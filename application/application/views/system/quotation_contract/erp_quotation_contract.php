                ...
                <div class="modal-body">
                    <table class="table table-bordered table-striped table-condesed" id="item_add_table">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="estimatedAmount[]" placeholder="0.00" value="0"
                    ...
                    <table class="table table-bordered table-striped table-condesed" id="edit_item_add_form">
                        ...
                        <tr>
                            ...
                            <td>&nbsp;<span id="edit_wac_cost" class="pull-right"
                                            style="font-size: 14px;text-align: right;margin-top: 8%;">0.00</span></td>
                            <td>
                                <input type="text" name="estimatedAmount" id="edit_estimatedAmount" placeholder="0.00"
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (jQuery.isEmptyObject(data['detail'])) {
                        $('#table_body').append('<tr class="danger"><td colspan="10" class="text-center"><b><?php echo $this->lang->line('common_no_records_found');?> </b></td></tr>');<!--No Records Found-->
                        $("#customerID").prop("disabled", false);
                        $("#transactionCurrencyID").prop("disabled", false);
                        currencyID = null;
                    } else {
   ...
   /* function clearitemAutoID(element){
    ...
    function clearitemAutoIDEdit(e,ths){
        ...
        if (keyCode == 9 || keyCode==13) {
            ...
            $(ths).closest('tr').find('#edit_net_unit_cost').html('0.00');
            $(ths).closest('tr').find('#edit_totalAmount').html('0.00');
        }
    ...
    function clearitemAutoID(e,ths) {
        ...
        if (keyCode == 9 || keyCode==13) {
            ...
            $(ths).closest('tr').find('.net_unit_cost').html('0.00');
            $(ths).closest('tr').find('.net_amount').html('0.00');
        }
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data['status']) {
                    ...
                    $(element).closest('tr').find('.wac_cost').text(parseFloat(data['amount']).formatMoney(2, '.', ','));
//                    $('#item_detail_form').bootstrapValidator('revalidateField', 'estimatedAmount');
    ...
    function LoaditemUnitPrice_againtsExchangerate_edit(LocalWacAmount) {
        ...
        $.ajax({
            ...
            success: function (data) {
                stopLoad();
                if (data['status']) {
                    $('#edit_wac_cost').text(parseFloat(data['amount']).formatMoney(2, '.', ','));
                }
                refreshNotifications(true);
            }, error: function () {
                alert('<?php echo $this->lang->line('common_an_error_occurred_Please_try_again');?>.');/*An Error Occurred! Please Try Again*/
    ...
    function item_detail_modal() {
        if (contractAutoID) {
            ...
            $('.net_amount,.net_unit_cost,.wac_cost').text('0.00');
                ...
                function () {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('#edit_net_unit_cost').text((unitAmount).formatMoney(2, '.', ','));
                            $('#edit_totalAmount').text((totAmount).formatMoney(2, '.', ','));
    ...
    function edit_net_amount() {
        ...
        if (qut == null || qut == 0) {
            ...
            $('#edit_totalAmount').text((((parseFloat(1) * parseFloat(amount)) - parseFloat(discoun)) * parseFloat(qut)).formatMoney(2, '.', ','));
            $('#edit_net_unit_cost').text(((parseFloat(1) * parseFloat(amount)) - parseFloat(discoun)).formatMoney(2, '.', ','));
        }
    }
    ...
    function add_more_item() {
        ...
        appendData.find('.number,.wac_cost,.net_unit_cost,.net_amount').text('0.00');
