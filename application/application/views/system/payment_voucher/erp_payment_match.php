...
<div class="m-b-md" id="wizardControl">
    <a class="btn btn-primary" href="#step1" data-toggle="tab"><?php echo $this->lang->line('accounts_payable_step_one');?><!--Step 1--> - <?php echo $this->lang->line('accounts_payable_trans_pm_payment_matching_header');?><!--Payment Matching Header--></a>
    <a class="btn btn-default btn-wizard" href="#step2" onclick="fetch_detail();" data-toggle="tab"><?php echo $this->lang->line('accounts_payable_step_two');?><!--Step 2--> - <?php echo $this->lang->line('accounts_payable_trans_pm_payment_matching_deail');?><!--Payment Matching Detail--></a>
    <a class="btn btn-default btn-wizard" href="#step3" onclick="load_conformation();" data-toggle="tab"><?php echo $this->lang->line('accounts_payable_step_three');?><!--Step 3--> - <?php echo $this->lang->line('accounts_payable_trans_pm_payment_matching_confirmation');?></a><!--Payment Matching Confirmation-->
</div><hr>
            ...
            <div class="row" >
                <div class="form-group col-sm-4">
                    ...
                    <div class="input-group datepic">
                        ...
                        <input type="text" name="matchDate" data-inputmask="'alias': '<?php echo $date_format_policy ?>'" value="<?php echo $current_date; ?>" id="matchDate"
        ...
        success : function(data){
            ...
            if(jQuery.isEmptyObject(data['payment'])){
                ...
                $.each(data['payment'], function (key, value) {
                    ...
                    if(balamntchk>0){
                        $('#match_ad_table').append('<tr><td>'+x+'</td><td>'+value['PVcode']+'</td><td>'+value['PVdate']+'</td><td>'+ad_detail+'</td><td class="text-right">'+parseFloat(value['transactionAmount']).formatMoney(currency_decimal, '.', ',')+'</td><td class="text-right">'+parseFloat((value['transactionAmount']-paid_amount)).formatMoney(currency_decimal, '.', ',')+'</td><td class="text-right"><select class="inv_drop" onchange="showBalanceAmount('+value['payVoucherDetailAutoID']+')"  id="inv_'+value['payVoucherDetailAutoID']+'"><option value=""><?php echo $this->lang->line('accounts_payable_trans_pm_select_invoice');?><!--Select Invoice--></option></select></td><td class="text-right" id="balamount_'+value['payVoucherDetailAutoID']+'"></td><td class="text-right"><input type="text" name="amount[]" style="width: 100px" id="amount_'+value['payVoucherDetailAutoID']+'" onkeyup="select_check_box(this,'+value['payVoucherDetailAutoID']+','+(value['transactionAmount']-paid_amount)+');" onkeypress="return validateFloatKeyPress(this,event);"  class="number"></td><td class="text-right" style="display:none;"><input class="checkbox" id="check_'+value['payVoucherDetailAutoID']+'" type="checkbox" value="'+value['payVoucherDetailAutoID']+'"></td></tr>');
                    }
                    x++;
                });
