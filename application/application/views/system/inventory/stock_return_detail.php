...
<table class="table table-bordered table-striped table-condesed">
    ...
    <tr>
        ...
        <th colspan="2"><?php echo $this->lang->line('common_qty');?> </th><!--Qty-->
        <th colspan="2">
            <button type="button" data-toggle="modal" onclick="openInvBasedModal()"
                    class="btn btn-primary pull-right btn-xs"><i class="fa fa-plus"></i><?php echo $this->lang->line('common_add_item');?>
            </button><!--Add Item-->
        </th>
    </tr>
    <tr>
        ...
        <th style="min-width: 10%"><?php echo $this->lang->line('transaction_common_return');?> </th><!--Return-->
                    ...
                    <div class="col-md-9">
                        <table class="table table-bordered table-striped table-condesed ">
                            ...
                            <tr>
                                ...
                                <th style="width: 15%"><?php echo $this->lang->line('transaction_item_cost');?> </th><!--Item Cost-->
                                <th style="width: 5%"><?php echo $this->lang->line('transaction_balance');?> </th><!--Balance-->
                                <th style="width: 5%"><?php echo $this->lang->line('transaction_common_return');?> </th><!--Return-->
                            </tr>
                            </thead>
            ...
            success: function (data) {
                ...
                if (!jQuery.isEmptyObject(data)) {
                    ...
                    $.each(data, function (val, text) {
                        $('#grv_table_body').append('<tr><td>' + x + '</td><td>' + text['grvPrimaryCode'] + ' - ' + text['transactionCurrency'] + ' - ' + text['grvDate'] + '</td><td>' + text['itemSystemCode'] + ' - ' + text['itemDescription'] + '</td><td class="text-right">' + text['unitOfMeasure'] + ' ' + parseFloat(text['receivedAmount']).formatMoney(text['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td>' + text['receivedQty'] + '</td><td class="text-center"><input type="text" class="number" size="15" id="qty_' + text['grvDetailsID'] + '" onkeyup="select_check_box(this,' + text['grvDetailsID'] + ',' + text['receivedQty'] + ')"></td><td class="text-right" style="display: none;"><input class="checkbox" id="check_' + text['grvDetailsID'] + '" type="checkbox" value="' + text['grvDetailsID'] + '"><input type="hidden" id="type_' + text['grvDetailsID'] + '"  value="' + text['type'] + '"></td></tr>');
                        x++;
                    });
                } else {
                    ...
                    $.each(data['detail'], function (key, value) {
                        ...
                        if (value['type'] == 'rrr') {
                            ...
                            if (value['isSubitemExist'] == 1) {
                                ...
                                string = '<tr><td>' + y + '</td><td>' + value['itemSystemCode'] + '</td><td>' + value['itemDescription'] + '</td><td class="text-center">' + value['unitOfMeasure'] + '</td><td class="text-center">' + value['currentlWacAmount'] + '</td><td class="text-center">' + value['return_Qty'] + '</td><td class="text-right">' + parseFloat(value['totalValue']).formatMoney(value['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td class="text-right"><a style="' + colour + '" onclick="load_itemMasterSub_config_modal(' + value['stockReturnDetailsID'] + ',\'SR\',' + value['wareHouseAutoID'] + ')"><i class="fa fa-list"></i></a> &nbsp;&nbsp; | &nbsp;&nbsp;  <a onclick="delete_item(' + value['stockReturnDetailsID'] + ');"><span style="color:rgb(209, 91, 71);" class="glyphicon glyphicon-trash"></span></a></td></tr>';
                            } else {
                                string = '<tr><td>' + y + '</td><td>' + value['itemSystemCode'] + '</td><td>' + value['itemDescription'] + '</td><td class="text-center">' + value['unitOfMeasure'] + '</td><td class="text-center">' + value['currentlWacAmount'] + '</td><td class="text-center">' + value['return_Qty'] + '</td><td class="text-right">' + parseFloat(value['totalValue']).formatMoney(value['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td class="text-right"><a onclick="delete_item(' + value['stockReturnDetailsID'] + ');"><span style="color:rgb(209, 91, 71);" class="glyphicon glyphicon-trash"></span></a></td></tr>';
