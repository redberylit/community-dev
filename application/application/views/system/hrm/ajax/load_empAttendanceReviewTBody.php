...
if (!empty($tempAttData)) {
    foreach ($tempAttData as $key => $row) {
        ...
        $empID = $row['empID'];
        /*      $tr_data = $empID;*/
        $tr_data = $row['ECode'] . ' ' . $row['Ename2'] . ' ' . $row['attendanceDate'] . ' ' . $row['floorDescription'];
        ...
        <tr data-code="<?php echo $datacode; ?>" style="white-space: nowrap;<?php echo $bg ?>"
            ...
            data-masterid="<?php echo $row['ID'] ?>" data-value="<?php echo $tr_data; ?>"
...
?>
    ...
    <script type="text/javascript">
        ...
        $('#attReview-searchItem').keyup(function () {
            ...
            console.log('dateSearch:' + dateSearch + ' ||| searchKey:' + searchKey);
