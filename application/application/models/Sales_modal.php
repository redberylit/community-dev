...
AND toTargetAmount))")->row_array();
    ...
    function get_sales_order_report()
    {
        ...
        if ($search) {
            $search = " AND (contractCode LIKE '%" . $search . "%' OR srp_erp_customermaster.customerName LIKE '%" . $search . "%' OR invoiceAmount LIKE '%" . $search . "%' OR receiptvAmount LIKE '%" . $search . "%' OR transactionAmount LIKE '%" . $search . "%')";
        } else {
            $search = "";
        }
        $qry = "SELECT DATE_FORMAT(contractDate,'" . $convertFormat . "') as documentDate, srp_erp_contractmaster.transactionAmount, a.invoiceAmount, a.nonTaxAmount, (IFNULL(a.receiptvAmount,0)+IFNULL(a.creditNoteAmount,0)+IFNULL(a.receiptMatchAmount,0)) as receiptAmount, transactionCurrency, transactionCurrencyDecimalPlaces, srp_erp_customermaster.customerName, contractCode, srp_erp_contractmaster.contractAutoID,srp_erp_contractmaster.documentID FROM srp_erp_contractmaster LEFT JOIN ( SELECT SUM(ab.invoiceAmount) AS invoiceAmount, SUM(ab.nonTaxAmount) AS nonTaxAmount, SUM(ab.receiptAmount) AS receiptvAmount,SUM(ab.creditNoteAmount) AS creditNoteAmount,SUM(ab.receiptMatchAmount) AS receiptMatchAmount, ab.contractAutoID FROM ( SELECT ( IFNULL( ( (tax.taxPercentage / 100) * SUM(civ.transactionAmount) ), 0 ) + SUM(civ.transactionAmount) ) AS invoiceAmount, SUM(civ.transactionAmount) AS nonTaxAmount, SUM(crv.transactionAmount) AS receiptAmount, SUM(IFNULL(crn.transactionAmount,0)) AS creditNoteAmount,SUM(IFNULL(rvd.transactionAmount,0)) AS receiptMatchAmount,tax.taxPercentage, contractAutoID, civ.invoiceAutoID FROM srp_erp_customerinvoicemaster LEFT JOIN ( SELECT SUM(transactionAmount) AS transactionAmount, contractAutoID, invoiceAutoID FROM srp_erp_customerinvoicedetails WHERE companyID = " . current_companyID() . " GROUP BY contractAutoID, invoiceAutoID ) civ ON srp_erp_customerinvoicemaster.invoiceAutoID = civ.invoiceAutoID LEFT JOIN ( SELECT SUM(IFNULL(taxPercentage, 0)) AS taxPercentage, invoiceAutoID FROM srp_erp_customerinvoicetaxdetails WHERE companyID = " . current_companyID() . " GROUP BY invoiceAutoID ) tax ON tax.invoiceAutoID = civ.invoiceAutoID LEFT JOIN ( SELECT SUM( srp_erp_customerreceiptdetail.transactionAmount ) AS transactionAmount, invoiceAutoID FROM srp_erp_customerreceiptdetail LEFT JOIN srp_erp_customerreceiptmaster ON srp_erp_customerreceiptmaster.receiptVoucherAutoId = srp_erp_customerreceiptdetail.receiptVoucherAutoId WHERE srp_erp_customerreceiptdetail.companyID = " . current_companyID() . " AND approvedYN = 1 GROUP BY invoiceAutoID ) crv ON crv.invoiceAutoID = civ.invoiceAutoID   LEFT JOIN (SELECT SUM( srp_erp_creditnotedetail.transactionAmount) AS transactionAmount,invoiceAutoID FROM srp_erp_creditnotedetail LEFT JOIN srp_erp_creditnotemaster ON srp_erp_creditnotemaster.creditNoteMasterAutoID = srp_erp_creditnotedetail.creditNoteMasterAutoID WHERE srp_erp_creditnotedetail.companyID = " . current_companyID() . " AND approvedYN = 1 GROUP BY invoiceAutoID ) crn ON crn.invoiceAutoID = civ.invoiceAutoID  LEFT JOIN (SELECT SUM(srp_erp_rvadvancematchdetails.transactionAmount) AS transactionAmount,InvoiceAutoID FROM srp_erp_rvadvancematchdetails LEFT JOIN srp_erp_rvadvancematch ON srp_erp_rvadvancematch.matchID = srp_erp_rvadvancematchdetails.matchID WHERE srp_erp_rvadvancematchdetails.companyID = " . current_companyID() . " AND confirmedYN = 1 GROUP BY InvoiceAutoID) rvd ON rvd.invoiceAutoID = civ.invoiceAutoID WHERE srp_erp_customerinvoicemaster.approvedYN = 1 AND contractAutoID IS NOT NULL GROUP BY contractAutoID, civ.invoiceAutoID ) ab GROUP BY ab.contractAutoID ) a ON a.contractAutoID = srp_erp_contractmaster.contractAutoID LEFT JOIN srp_erp_customermaster ON customerAutoID = customerID WHERE srp_erp_contractmaster.companyID = " . current_companyID() . " AND srp_erp_contractmaster.customerID IN (" . join(',', $customerID) . ") AND srp_erp_contractmaster.documentID = 'SO' AND approvedYN = 1 $search GROUP BY srp_erp_contractmaster.contractAutoID ORDER BY transactionCurrency,contractDate";
...
GROUP BY
	contractmaster.salesPersonID ) t1
	GROUP BY 
t1.salesPersonID";
        $output = $this->db->query($qry)->result_array();
        return $output;
