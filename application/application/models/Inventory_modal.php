...
class Inventory_modal extends ERP_Model
{
    ...
    function fetch_template_data($itemIssueAutoID)
    {
        ...
        return $data;*/
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,,DATE_FORMAT(issueDate,\'' . $convertFormat . '\') AS issueDate,(DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\')) AS approvedDate,(DATE_FORMAT(FYBegin,\'' . $convertFormat . '\')) AS FYbegining,(DATE_FORMAT(FYEnd,\'' . $convertFormat . '\')) AS FYend,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        $this->db->where('itemIssueAutoID', $itemIssueAutoID);
        $this->db->from('srp_erp_itemissuemaster');
        $data['master'] = $this->db->get()->row_array();
        $this->db->select('srp_erp_itemissuedetails.*,srp_erp_materialrequest.MRCode,srp_erp_chartofaccounts.systemAccountCode as systemAccountCode,srp_erp_chartofaccounts.GLDescription as costglname,srp_erp_chartofaccounts.GLDescription as GLDescription ');
        $this->db->where('itemIssueAutoID', $itemIssueAutoID);
        $this->db->join('srp_erp_materialrequest', 'srp_erp_materialrequest.mrAutoID = srp_erp_itemissuedetails.mrAutoID', 'left');
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(tranferDate,\'' . $convertFormat . '\') AS tranferDate,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(returnDate,\'' . $convertFormat . '\') AS returnDate,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        $this->db->where('stockReturnAutoID', $stockReturnAutoID);
        $this->db->from('srp_erp_stockreturnmaster');
        $data['master'] = $this->db->get()->row_array();
        $this->db->select('stockReturnDetailsID, stockReturnAutoID, type, itemAutoID, concat(IFNULL(srp_erp_stockreturndetails.documentSystemCode,""),IF(documentSystemCode IS NULL,""," - "),srp_erp_stockreturndetails.itemSystemCode ) as itemSystemCode, itemDescription, itemFinanceCategory, itemFinanceCategorySub, financeCategory, itemCategory, unitOfMeasureID, unitOfMeasure, defaultUOMID, defaultUOM conversionRateUOM, return_Qty, received_Qty, currentStock, currentWareHouseStock, currentlWacAmount, totalValue, PLGLAutoID, PLSystemGLCode, PLGLCode, PLDescription, PLType, BLGLAutoID, BLSystemGLCode, BLGLCode, BLDescription, BLType, comments ');
        $this->db->where('stockReturnAutoID', $stockReturnAutoID);
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(stockAdjustmentDate,\'' . $convertFormat . '\') AS stockAdjustmentDate,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
...
srp_erp_chartofaccounts.GLDescription as costglname
FROM
	`srp_erp_itemissuedetails`
...
WHERE
    ...
    function fetch_return_direct_details()
    {
        ...
        $this->db->select('concat(IFNULL(srp_erp_stockreturndetails.documentSystemCode,""),IF(documentSystemCode IS NULL,""," - "),srp_erp_stockreturndetails.itemSystemCode ) as itemSystemCode,srp_erp_stockreturndetails.itemDescription,srp_erp_stockreturndetails.unitOfMeasure,srp_erp_stockreturnmaster.transactionCurrencyDecimalPlaces,srp_erp_stockreturndetails.currentlWacAmount,srp_erp_stockreturndetails.return_Qty,srp_erp_stockreturndetails.totalValue,srp_erp_stockreturndetails.stockReturnDetailsID,srp_erp_stockreturndetails.type, ,srp_erp_itemmaster.isSubitemExist,srp_erp_stockreturnmaster.wareHouseAutoID');
        $this->db->from('srp_erp_stockreturndetails');
...
WHERE
            ...
            if (!empty($Confirmed)) {
                ...
                if ($validate) {
                    ...
                    if ($approvals_status == 1) {
                        ...
                        if($autoApproval==0) {
                            $result = $this->save_material_issue_approval(0, $app_data['itemIssueAutoID'], 1, 'Auto Approved');
                            if($result){
                                $this->db->trans_commit();
        ...
        if (empty($results)) {
            ...
            if (!empty($Confirmed)) {
                ...
                if ($validate) {
                    ...
                    if($autoApproval==0){
                        $approvals_status = $this->approvals->auto_approve($app_data['stockTransferAutoID'], 'srp_erp_stocktransfermaster','stockTransferAutoID', 'ST',$app_data['stockTransferCode'],$app_data['tranferDate']);
                    }elseif($autoApproval==1){
                        $approvals_status = $this->approvals->CreateApproval('ST', $app_data['stockTransferAutoID'], $app_data['stockTransferCode'], 'Stock Transfer', 'srp_erp_stocktransfermaster', 'stockTransferAutoID',0,$app_data['tranferDate']);
                    }else{
                        return array('error' => 1, 'message' => 'Approval levels are not set for this document');
                        exit;
                    }
                    ...
                    if ($approvals_status == 1) {
                        $autoApproval= get_document_auto_approval('ST');
                        if($autoApproval==0) {
                            $result = $this->save_stock_transfer_approval(0, $app_data['stockTransferAutoID'], 1, 'Auto Approved');
                            if($result){
                                return array('error' => 0, 'message' => 'Document Confirmed Successfully.');
                            }
                        }else{
            ...
            if (!empty($Confirmed)) {
                ...
                if ($isProductReference_completed == 0) {
                    ...
                    if ($validate) {
                        ...
                        $this->db->select('stockAdjustmentCode,companyFinanceYearID,DATE_FORMAT(stockAdjustmentDate, "%Y") as invYear,DATE_FORMAT(stockAdjustmentDate, "%m") as invMonth');
    ...
    function fetch_warehouse_item()
    {
        ...
        if (!empty($data)) {
            return array('status' => true, 'currentStock' => $currentStock, 'WacAmount' => $data['companyLocalWacAmount']);
        } else {
    ...
    function fetch_st_warehouse_item()
    {
        ...
        if (!empty($data)) {
            return array('error' => 0, 'message' => '', 'status' => true, 'currentStock' => $currentStock, 'WacAmount' => $data['companyLocalWacAmount']);
        } else {
...
FROM
	(
		SELECT
			...
			(
				srp_erp_grvdetails.receivedQty - SUM(IFNULL(return_Qty, 0))
			) AS receivedQty,
			`srp_erp_grvdetails`.`receivedAmount`,
		...
		UNION ALL
			SELECT
				...
				(
					srp_erp_paysupplierinvoicedetail.requestedQty - SUM(IFNULL(return_Qty, 0))
				) AS receivedQty,
...
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        ...
        $data['master'] = $this->db->get()->row_array();
        $this->db->select('srp_erp_salesreturndetails.*,srp_erp_customerinvoicemaster.invoiceCode');
        $this->db->from('srp_erp_salesreturndetails');
        $this->db->where('salesReturnAutoID', $salesReturnAutoID);
        $this->db->join('srp_erp_customerinvoicemaster', 'srp_erp_salesreturndetails.invoiceAutoID = srp_erp_customerinvoicemaster.invoiceAutoID');
        $data['detail'] = $this->db->get()->result_array();
        return $data;
    }
...
WHERE
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,,DATE_FORMAT(requestedDate,\'' . $convertFormat . '\') AS requestedDate,(DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\')) AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        $this->db->where('mrAutoID', $mrAutoID);
        $this->db->from('srp_erp_materialrequest');
        $data['master'] = $this->db->get()->row_array();
...
GROUP BY srp_erp_materialrequestdetails.mrDetailID")->result_array();
    ...
    function save_stock_adjustment_detail_buyback()
    {
        ...
        $data['deduction'] = trim($this->input->post('deduction'));
