...
class CommunityNgo_model extends ERP_Model
{
    ...
    function new_beneficiary_division()
    {
        ...
        if ($stateID) {
            ...
            $get_comJammiyah = $this->db->query("SELECT srp_erp_ngo_com_regionmaster.jammiyahDivisionID FROM srp_erp_ngo_com_regionmaster INNER JOIN srp_erp_statemaster ON srp_erp_ngo_com_regionmaster.jammiyahDivisionID=srp_erp_statemaster.stateID WHERE srp_erp_ngo_com_regionmaster.companyID = {$companyID} AND srp_erp_statemaster.divisionTypeCode = 'JD'");
            $rowJammiyah = $get_comJammiyah->row();
            if ($divisionTypeCode == "JD" && !empty($rowJammiyah)) {
                return array('e', 'Community Jammiyah Division is already assigned !');
            } else {
                $this->db->insert('srp_erp_statemaster', $data);
                if ($this->db->affected_rows() > 0) {
    ...
    function save_communityMember()
    {
        ...
        $ComMailId = trim($this->input->post('EmailID'));
        ...
        $defsProvince = '';
        $defsDistrict = '';
        $defsDistrictDivision = '';
        $defProvince = $default_data['ProvinceCode'];
        if ($defProvince) {
            $defsProvince = $default_data['ProvinceCode'];
        }
        $defDistrict = $default_data['DistrictCode'];
        if ($defDistrict) {
            $defsDistrict = $default_data['DistrictCode'];
        }

        $defDistrictDivision = $default_data['DDivisionCode'];
        if ($defDistrictDivision) {
        ...
        $CBC_Date = trim($this->input->post('CBC_Date'));
        $MrgRegisteredDate = trim($this->input->post('MrgRegisteredDate'));
        $CmPassportIssueDate = trim($this->input->post('CmPassportIssueDate'));
        $CmPassportExpiryDate = trim($this->input->post('CmPassportExpiryDate'));
        $CmDrvLicIssueDate = trim($this->input->post('CmDrvLicIssueDate'));
        $CmDrvLicExpiryDate = trim($this->input->post('CmDrvLicExpiryDate'));
        $countyID = trim($this->input->post('countyID'));
        $RegionID = trim($this->input->post('RegionID'));

        $data['CDOB'] = (!empty($CDOB)) ? input_format_date($CDOB, $date_format_policy) : null;
        $data['CountryOfOrigin'] = trim($this->input->post('CountryOfOrigin'));
        $data['CPlaceOfBirth'] = trim($this->input->post('CPlaceOfBirth'));
        $data['CBC_No'] = trim($this->input->post('CBC_No'));
        $data['CBC_Date'] = (!empty($CBC_Date)) ? input_format_date($CBC_Date, $date_format_policy) : null;
        ...
        $data['TP_mobileNo2'] = trim($this->input->post('TP_mobileNo2'));
        $data['CountryCodeMobile2'] = trim($this->input->post('CountryCodeMobile2'));
        $data['AreaCodeMobile2'] = trim($this->input->post('AreaCodeMobile2'));
        $data['EmailID'] = $ComMailId;
        $data['CmHeight'] = trim($this->input->post('CmHeight'));
        $data['CmWeight'] = trim($this->input->post('CmWeight'));
        ...
        $data['MrgCertificateNo'] = trim($this->input->post('MrgCertificateNo'));
        $data['MrgRegisteredDate'] = (!empty($MrgRegisteredDate)) ? input_format_date($MrgRegisteredDate, $date_format_policy) : null;
        ...
        $data['CmPassportNo'] = trim($this->input->post('CmPassportNo'));
        $data['CmPassportIssueDate'] = (!empty($CmPassportIssueDate)) ? input_format_date($CmPassportIssueDate, $date_format_policy) : null;
        $data['CmPassportExpiryDate'] = (!empty($CmPassportExpiryDate)) ? input_format_date($CmPassportExpiryDate, $date_format_policy) : null;
        $data['CmDrivingLicenseNo'] = trim($this->input->post('CmDrivingLicenseNo'));
        $data['CmDrvLicIssueDate'] = (!empty($CmDrvLicIssueDate)) ? input_format_date($CmDrvLicIssueDate, $date_format_policy) : null;
        $data['CmDrvLicExpiryDate'] = (!empty($CmDrvLicExpiryDate)) ? input_format_date($CmDrvLicExpiryDate, $date_format_policy) : null;

        $data['comVerifiApproved'] = '1';
        ...
        if ($Com_MasterID) {
            ...
            if (!empty($resComLogExist->LeaderID)) {
                ...
                $updateDB2 = $db->query("UPDATE ngo_family SET FamEmail='{$ComMailId}' WHERE companyID={$companyID} AND `LeaderID`='{$Com_MasterID}'");
            }
    ...
    function save_communityParent()
    {
        ...
        $data['cfComMasID'] = trim($this->input->post('cfComMasID'));
        $data['fatherIsAlive'] = trim($this->input->post('fatherIsAlive'));
        $data['cfFullName'] = trim($this->input->post('cfFullName'));
        $cFatherDOB = trim($this->input->post('cFatherDOB'));
        if ($this->input->post('fatherIsAlive') == '0') {
            $cfDateOfDeath = trim($this->input->post('cfDateOfDeath'));
        } else {
            $cfDateOfDeath = '';
        }
        $data['cfBornCountry'] = trim($this->input->post('cfBornCountry'));
        $data['cfBornArea'] = trim($this->input->post('cfBornArea'));
        $data['cfBC_No'] = trim($this->input->post('cfBC_No'));
        $cfBCDate = trim($this->input->post('cfBCDate'));
        $data['cfOccupationId'] = trim($this->input->post('cfOccupationId'));
        $data['cfNIC_No'] = trim($this->input->post('cfNIC_No'));
        $data['cfDC_No'] = trim($this->input->post('cfDC_No'));
        $cfDCDate = trim($this->input->post('cfDCDate'));
        $data['cfDateOfDeath'] = (!empty($cfDateOfDeath)) ? input_format_date($cfDateOfDeath, $date_format_policy) : null;
        $data['cFatherDOB'] = (!empty($cFatherDOB)) ? input_format_date($cFatherDOB, $date_format_policy) : null;
        $data['cfBCDate'] = (!empty($cfBCDate)) ? input_format_date($cfBCDate, $date_format_policy) : null;
        $data['cfDCDate'] = (!empty($cfDCDate)) ? input_format_date($cfDCDate, $date_format_policy) : null;

        $data['cmComMasID'] = trim($this->input->post('cmComMasID'));
        $data['motherIsAlive'] = trim($this->input->post('motherIsAlive'));
        $data['cmFullName'] = trim($this->input->post('cmFullName'));
        $cMotherDOB = trim($this->input->post('cMotherDOB'));
        if ($this->input->post('motherIsAlive') == '0') {
            $cmDateOfDeath = trim($this->input->post('cmDateOfDeath'));
        } else {
            $cmDateOfDeath = '';
        }
        $data['cmBornCountry'] = trim($this->input->post('cmBornCountry'));
        $data['cmBornArea'] = trim($this->input->post('cmBornArea'));
        $data['cmBC_No'] = trim($this->input->post('cmBC_No'));
        $cmBCDate = trim($this->input->post('cmBCDate'));
        $data['cmOccupationId'] = trim($this->input->post('cmOccupationId'));
        $data['cmNIC_No'] = trim($this->input->post('cmNIC_No'));
        $data['cmDC_No'] = trim($this->input->post('cmDC_No'));
        $cmDCDate = trim($this->input->post('cmDCDate'));
        $data['cMotherDOB'] = (!empty($cMotherDOB)) ? input_format_date($cMotherDOB, $date_format_policy) : null;
        $data['cmDateOfDeath'] = (!empty($cmDateOfDeath)) ? input_format_date($cmDateOfDeath, $date_format_policy) : null;
        $data['cmBCDate'] = (!empty($cmBCDate)) ? input_format_date($cmBCDate, $date_format_policy) : null;
        $data['cmDCDate'] = (!empty($cmDCDate)) ? input_format_date($cmDCDate, $date_format_policy) : null;
        ...
        if ($Com_MasterID) {
            ...
            if ($update) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    return array('e', 'Parent Update Failed ');
                } else {
                    $this->db->trans_commit();

                    return array('s', 'Parent Updated Successfully.', $Com_MasterID);
                }
            }
        }
    }
    ...
    function save_memFatherParents()
    {
        ...
        $data['cf_GdFComMasID'] = trim($this->input->post('cf_GdFComMasID'));
        $data['cf_GrandFIsAlive'] = trim($this->input->post('cf_GrandFIsAlive'));
        $data['cf_GrandFFullName'] = trim($this->input->post('cf_GrandFFullName'));
        $cf_GrandFDOB = trim($this->input->post('cf_GrandFDOB'));
        if ($this->input->post('cf_GrandFIsAlive') == '0') {
            $cf_GrandFDateOfDeath = trim($this->input->post('cf_GrandFDateOfDeath'));
        } else {
            $cf_GrandFDateOfDeath = '';
        }
        $data['cf_GrandFBornCountry'] = trim($this->input->post('cf_GrandFBornCountry'));
        $data['cf_GrandFBornArea'] = trim($this->input->post('cf_GrandFBornArea'));
        $data['cf_GrandFBC_No'] = trim($this->input->post('cf_GrandFBC_No'));
        $cf_GrandFBCDate = trim($this->input->post('cf_GrandFBCDate'));
        $data['cf_GrandFOccupationId'] = trim($this->input->post('cf_GrandFOccupationId'));
        $data['cf_GrandFNIC_No'] = trim($this->input->post('cf_GrandFNIC_No'));
        $data['cf_GrandFDC_No'] = trim($this->input->post('cf_GrandFDC_No'));
        $cf_GrandFDCDate = trim($this->input->post('cf_GrandFDCDate'));
        $data['cf_GrandFDOB'] = (!empty($cf_GrandFDOB)) ? input_format_date($cf_GrandFDOB, $date_format_policy) : null;
        $data['cf_GrandFDateOfDeath'] = (!empty($cf_GrandFDateOfDeath)) ? input_format_date($cf_GrandFDateOfDeath, $date_format_policy) : null;
        $data['cf_GrandFBCDate'] = (!empty($cf_GrandFBCDate)) ? input_format_date($cf_GrandFBCDate, $date_format_policy) : null;
        $data['cf_GrandFDCDate'] = (!empty($cf_GrandFDCDate)) ? input_format_date($cf_GrandFDCDate, $date_format_policy) : null;

        $data['cf_GdMComMasID'] = trim($this->input->post('cf_GdMComMasID'));
        $data['cf_GrandMIsAlive'] = trim($this->input->post('cf_GrandMIsAlive'));
        $data['cf_GrandMFullName'] = trim($this->input->post('cf_GrandMFullName'));
        $cf_GrandMDOB = trim($this->input->post('cf_GrandMDOB'));
        if ($this->input->post('cf_GrandMIsAlive') == '0') {
            $cf_GrandMDateOfDeath = trim($this->input->post('cf_GrandMDateOfDeath'));
        } else {
            $cf_GrandMDateOfDeath = '';
        }
        $data['cf_GrandMBornCountry'] = trim($this->input->post('cf_GrandMBornCountry'));
        $data['cf_GrandMBornArea'] = trim($this->input->post('cf_GrandMBornArea'));
        $data['cf_GrandMBC_No'] = trim($this->input->post('cf_GrandMBC_No'));
        $cf_GrandMBCDate = trim($this->input->post('cf_GrandMBCDate'));
        $data['cf_GrandMOccupationId'] = trim($this->input->post('cf_GrandMOccupationId'));
        $data['cf_GrandMNIC_No'] = trim($this->input->post('cf_GrandMNIC_No'));
        $data['cf_GrandMDC_No'] = trim($this->input->post('cf_GrandMDC_No'));
        $cf_GrandMDCDate = trim($this->input->post('cf_GrandMDCDate'));
        $data['cf_GrandMDOB'] = (!empty($cf_GrandMDOB)) ? input_format_date($cf_GrandMDOB, $date_format_policy) : null;
        $data['cf_GrandMDateOfDeath'] = (!empty($cf_GrandMDateOfDeath)) ? input_format_date($cf_GrandMDateOfDeath, $date_format_policy) : null;
        $data['cf_GrandMBCDate'] = (!empty($cf_GrandMBCDate)) ? input_format_date($cf_GrandMBCDate, $date_format_policy) : null;
        $data['cf_GrandMDCDate'] = (!empty($cf_GrandMDCDate)) ? input_format_date($cf_GrandMDCDate, $date_format_policy) : null;
        ...
        if ($Com_MasterID) {
            ...
            if ($update) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    return array('e', 'Father\'s Side Grandparent Update Failed ');
                } else {
                    $this->db->trans_commit();

                    return array('s', 'Father\'s Side Grandparent Updated Successfully.', $Com_MasterID);
                }
            }
        }
    }
    ...
    function save_memMotherParents()
    {
        ...
        $data['cm_GdFComMasID'] = trim($this->input->post('cm_GdFComMasID'));
        $data['cm_GrandFIsAlive'] = trim($this->input->post('cm_GrandFIsAlive'));
        $data['cm_GrandFFullName'] = trim($this->input->post('cm_GrandFFullName'));
        $cm_GrandFDOB = trim($this->input->post('cm_GrandFDOB'));
        if ($this->input->post('cm_GrandFIsAlive') == '0') {
            $cm_GrandFDateOfDeath = trim($this->input->post('cm_GrandFDateOfDeath'));
        } else {
            $cm_GrandFDateOfDeath = '';
        }
        $data['cm_GrandFBornCountry'] = trim($this->input->post('cm_GrandFBornCountry'));
        $data['cm_GrandFBornArea'] = trim($this->input->post('cm_GrandFBornArea'));
        $data['cm_GrandFBC_No'] = trim($this->input->post('cm_GrandFBC_No'));
        $cm_GrandFBCDate = trim($this->input->post('cm_GrandFBCDate'));
        $data['cm_GrandFOccupationId'] = trim($this->input->post('cm_GrandFOccupationId'));
        $data['cm_GrandFNIC_No'] = trim($this->input->post('cm_GrandFNIC_No'));
        $data['cm_GrandFDC_No'] = trim($this->input->post('cm_GrandFDC_No'));
        $cm_GrandFDCDate = trim($this->input->post('cm_GrandFDCDate'));
        $data['cm_GrandFDOB'] = (!empty($cm_GrandFDOB)) ? input_format_date($cm_GrandFDOB, $date_format_policy) : null;
        $data['cm_GrandFDateOfDeath'] = (!empty($cm_GrandFDateOfDeath)) ? input_format_date($cm_GrandFDateOfDeath, $date_format_policy) : null;
        $data['cm_GrandFBCDate'] = (!empty($cm_GrandFBCDate)) ? input_format_date($cm_GrandFBCDate, $date_format_policy) : null;
        $data['cm_GrandFDCDate'] = (!empty($cm_GrandFDCDate)) ? input_format_date($cm_GrandFDCDate, $date_format_policy) : null;

        $data['cm_GdMComMasID'] = trim($this->input->post('cm_GdMComMasID'));
        $data['cm_GrandMIsAlive'] = trim($this->input->post('cm_GrandMIsAlive'));
        $data['cm_GrandMFullName'] = trim($this->input->post('cm_GrandMFullName'));
        $cm_GrandMDOB = trim($this->input->post('cm_GrandMDOB'));
        if ($this->input->post('cm_GrandMIsAlive') == '0') {
            $cm_GrandMDateOfDeath = trim($this->input->post('cm_GrandMDateOfDeath'));
        } else {
            $cm_GrandMDateOfDeath = '';
        }
        $data['cm_GrandMBornCountry'] = trim($this->input->post('cm_GrandMBornCountry'));
        $data['cm_GrandMBornArea'] = trim($this->input->post('cm_GrandMBornArea'));
        $data['cm_GrandMBC_No'] = trim($this->input->post('cm_GrandMBC_No'));
        $cm_GrandMBCDate = trim($this->input->post('cm_GrandMBCDate'));
        $data['cm_GrandMOccupationId'] = trim($this->input->post('cm_GrandMOccupationId'));
        $data['cm_GrandMNIC_No'] = trim($this->input->post('cm_GrandMNIC_No'));
        $data['cm_GrandMDC_No'] = trim($this->input->post('cm_GrandMDC_No'));
        $cm_GrandMDCDate = trim($this->input->post('cm_GrandMDCDate'));
        $data['cm_GrandMDOB'] = (!empty($cm_GrandMDOB)) ? input_format_date($cm_GrandMDOB, $date_format_policy) : null;
        $data['cm_GrandMDateOfDeath'] = (!empty($cm_GrandMDateOfDeath)) ? input_format_date($cm_GrandMDateOfDeath, $date_format_policy) : null;
        $data['cm_GrandMBCDate'] = (!empty($cm_GrandMBCDate)) ? input_format_date($cm_GrandMBCDate, $date_format_policy) : null;
        $data['cm_GrandMDCDate'] = (!empty($cm_GrandMDCDate)) ? input_format_date($cm_GrandMDCDate, $date_format_policy) : null;
        ...
        if ($Com_MasterID) {
            ...
            if ($update) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    return array('e', 'Mother\'s Side Grandparent Update Failed ');
                } else {
                    $this->db->trans_commit();

                    return array('s', 'Mother\'s Side Grandparent Updated Successfully.', $Com_MasterID);
                }
            }
        }
    }
    ...
    function save_memGrtFatherParents()
    {
        ...
        $data['cf_grt_GdFComMasID'] = trim($this->input->post('cf_grt_GdFComMasID'));
        $data['cf_grt_GrandFIsAlive'] = trim($this->input->post('cf_grt_GrandFIsAlive'));
        $data['cf_grt_GrandFFullName'] = trim($this->input->post('cf_grt_GrandFFullName'));
        $cf_grt_GrandFDOB = trim($this->input->post('cf_grt_GrandFDOB'));
        if ($this->input->post('cf_grt_GrandFIsAlive') == '0') {
            $cf_grt_GrandFDateOfDeath = trim($this->input->post('cf_grt_GrandFDateOfDeath'));
        } else {
            $cf_grt_GrandFDateOfDeath = '';
        }
        $data['cf_grt_GrandFBornCountry'] = trim($this->input->post('cf_grt_GrandFBornCountry'));
        $data['cf_grt_GrandFBornArea'] = trim($this->input->post('cf_grt_GrandFBornArea'));
        $data['cf_grt_GrandFBC_No'] = trim($this->input->post('cf_grt_GrandFBC_No'));
        $cf_grt_GrandFBCDate = trim($this->input->post('cf_grt_GrandFBCDate'));
        $data['cf_grt_GrandFOccupationId'] = trim($this->input->post('cf_grt_GrandFOccupationId'));
        $data['cf_grt_GrandFNIC_No'] = trim($this->input->post('cf_grt_GrandFNIC_No'));
        $data['cf_grt_GrandFDC_No'] = trim($this->input->post('cf_grt_GrandFDC_No'));
        $cf_grt_GrandFDCDate = trim($this->input->post('cf_grt_GrandFDCDate'));
        $data['cf_grt_GrandFDOB'] = (!empty($cf_grt_GrandFDOB)) ? input_format_date($cf_grt_GrandFDOB, $date_format_policy) : null;
        $data['cf_grt_GrandFDateOfDeath'] = (!empty($cf_grt_GrandFDateOfDeath)) ? input_format_date($cf_grt_GrandFDateOfDeath, $date_format_policy) : null;
        $data['cf_grt_GrandFBCDate'] = (!empty($cf_grt_GrandFBCDate)) ? input_format_date($cf_grt_GrandFBCDate, $date_format_policy) : null;
        $data['cf_grt_GrandFDCDate'] = (!empty($cf_grt_GrandFDCDate)) ? input_format_date($cf_grt_GrandFDCDate, $date_format_policy) : null;

        $data['cf_grt_GdMComMasID'] = trim($this->input->post('cf_grt_GdMComMasID'));
        $data['cf_grt_GrandMIsAlive'] = trim($this->input->post('cf_grt_GrandMIsAlive'));
        $data['cf_grt_GrandMFullName'] = trim($this->input->post('cf_grt_GrandMFullName'));
        $cf_grt_GrandMDOB = trim($this->input->post('cf_grt_GrandMDOB'));
        if ($this->input->post('cf_grt_GrandMIsAlive') == '0') {
            $cf_grt_GrandMDateOfDeath = trim($this->input->post('cf_grt_GrandMDateOfDeath'));
        } else {
            $cf_grt_GrandMDateOfDeath = '';
        }
        $data['cf_grt_GrandMBornCountry'] = trim($this->input->post('cf_grt_GrandMBornCountry'));
        $data['cf_grt_GrandMBornArea'] = trim($this->input->post('cf_grt_GrandMBornArea'));
        $data['cf_grt_GrandMBC_No'] = trim($this->input->post('cf_grt_GrandMBC_No'));
        $cf_grt_GrandMBCDate = trim($this->input->post('cf_grt_GrandMBCDate'));
        $data['cf_grt_GrandMOccupationId'] = trim($this->input->post('cf_grt_GrandMOccupationId'));
        $data['cf_grt_GrandMNIC_No'] = trim($this->input->post('cf_grt_GrandMNIC_No'));
        $data['cf_grt_GrandMDC_No'] = trim($this->input->post('cf_grt_GrandMDC_No'));
        $cf_grt_GrandMDCDate = trim($this->input->post('cf_grt_GrandMDCDate'));
        $data['cf_grt_GrandMDOB'] = (!empty($cf_grt_GrandMDOB)) ? input_format_date($cf_grt_GrandMDOB, $date_format_policy) : null;
        $data['cf_grt_GrandMDateOfDeath'] = (!empty($cf_grt_GrandMDateOfDeath)) ? input_format_date($cf_grt_GrandMDateOfDeath, $date_format_policy) : null;
        $data['cf_grt_GrandMBCDate'] = (!empty($cf_grt_GrandMBCDate)) ? input_format_date($cf_grt_GrandMBCDate, $date_format_policy) : null;
        $data['cf_grt_GrandMDCDate'] = (!empty($cf_grt_GrandMDCDate)) ? input_format_date($cf_grt_GrandMDCDate, $date_format_policy) : null;
        ...
        if ($Com_MasterID) {
            ...
            if ($update) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    return array('e', 'Father\'s Side Great-Grandparent Update Failed ');
                } else {
                    $this->db->trans_commit();

                    return array('s', 'Father\'s Side Great-Grandparent Updated Successfully.', $Com_MasterID);
                }
            }
        }
    }
    ...
    function save_memGrtMotherParents()
    {
        ...
        $data['cm_grt_GdFComMasID'] = trim($this->input->post('cm_grt_GdFComMasID'));
        $data['cm_grt_GrandFIsAlive'] = trim($this->input->post('cm_grt_GrandFIsAlive'));
        $data['cm_grt_GrandFFullName'] = trim($this->input->post('cm_grt_GrandFFullName'));
        $cm_grt_GrandFDOB = trim($this->input->post('cm_grt_GrandFDOB'));
        if ($this->input->post('cm_grt_GrandFIsAlive') == '0') {
            $cm_grt_GrandFDateOfDeath = trim($this->input->post('cm_grt_GrandFDateOfDeath'));
        } else {
            $cm_grt_GrandFDateOfDeath = '';
        }
        $data['cm_grt_GrandFBornCountry'] = trim($this->input->post('cm_grt_GrandFBornCountry'));
        $data['cm_grt_GrandFBornArea'] = trim($this->input->post('cm_grt_GrandFBornArea'));
        $data['cm_grt_GrandFBC_No'] = trim($this->input->post('cm_grt_GrandFBC_No'));
        $cm_grt_GrandFBCDate = trim($this->input->post('cm_grt_GrandFBCDate'));
        $data['cm_grt_GrandFOccupationId'] = trim($this->input->post('cm_grt_GrandFOccupationId'));
        $data['cm_grt_GrandFNIC_No'] = trim($this->input->post('cm_grt_GrandFNIC_No'));
        $data['cm_grt_GrandFDC_No'] = trim($this->input->post('cm_grt_GrandFDC_No'));
        $cm_grt_GrandFDCDate = trim($this->input->post('cm_grt_GrandFDCDate'));
        $data['cm_grt_GrandFDOB'] = (!empty($cm_grt_GrandFDOB)) ? input_format_date($cm_grt_GrandFDOB, $date_format_policy) : null;
        $data['cm_grt_GrandFDateOfDeath'] = (!empty($cm_grt_GrandFDateOfDeath)) ? input_format_date($cm_grt_GrandFDateOfDeath, $date_format_policy) : null;
        $data['cm_grt_GrandFBCDate'] = (!empty($cm_grt_GrandFBCDate)) ? input_format_date($cm_grt_GrandFBCDate, $date_format_policy) : null;
        $data['cm_grt_GrandFDCDate'] = (!empty($cm_grt_GrandFDCDate)) ? input_format_date($cm_grt_GrandFDCDate, $date_format_policy) : null;

        $data['cm_grt_GdMComMasID'] = trim($this->input->post('cm_grt_GdMComMasID'));
        $data['cm_grt_GrandMIsAlive'] = trim($this->input->post('cm_grt_GrandMIsAlive'));
        $data['cm_grt_GrandMFullName'] = trim($this->input->post('cm_grt_GrandMFullName'));
        $cm_grt_GrandMDOB = trim($this->input->post('cm_grt_GrandMDOB'));
        if ($this->input->post('cm_grt_GrandMIsAlive') == '0') {
            $cm_grt_GrandMDateOfDeath = trim($this->input->post('cm_grt_GrandMDateOfDeath'));
        } else {
            $cm_grt_GrandMDateOfDeath = '';
        }
        $data['cm_grt_GrandMBornCountry'] = trim($this->input->post('cm_grt_GrandMBornCountry'));
        $data['cm_grt_GrandMBornArea'] = trim($this->input->post('cm_grt_GrandMBornArea'));
        $data['cm_grt_GrandMBC_No'] = trim($this->input->post('cm_grt_GrandMBC_No'));
        $cm_grt_GrandMBCDate = trim($this->input->post('cm_grt_GrandMBCDate'));
        $data['cm_grt_GrandMOccupationId'] = trim($this->input->post('cm_grt_GrandMOccupationId'));
        $data['cm_grt_GrandMNIC_No'] = trim($this->input->post('cm_grt_GrandMNIC_No'));
        $data['cm_grt_GrandMDC_No'] = trim($this->input->post('cm_grt_GrandMDC_No'));
        $cm_grt_GrandMDCDate = trim($this->input->post('cm_grt_GrandMDCDate'));
        $data['cm_grt_GrandMDOB'] = (!empty($cm_grt_GrandMDOB)) ? input_format_date($cm_grt_GrandMDOB, $date_format_policy) : null;
        $data['cm_grt_GrandMDateOfDeath'] = (!empty($cm_grt_GrandMDateOfDeath)) ? input_format_date($cm_grt_GrandMDateOfDeath, $date_format_policy) : null;
        $data['cm_grt_GrandMBCDate'] = (!empty($cm_grt_GrandMBCDate)) ? input_format_date($cm_grt_GrandMBCDate, $date_format_policy) : null;
        $data['cm_grt_GrandMDCDate'] = (!empty($cm_grt_GrandMDCDate)) ? input_format_date($cm_grt_GrandMDCDate, $date_format_policy) : null;
        ...
        if ($Com_MasterID) {
            ...
            if ($update) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    return array('e', 'Mother\'s Side Great-Grandparent Update Failed ');
                } else {
                    $this->db->trans_commit();

                    return array('s', 'Mother\'s Side Great-Grandparent Updated Successfully.', $Com_MasterID);
                }
            }
        }
    }
    ...
    function load_member()
    {
        ...
        $data = $this->db->query("select * ,DATE_FORMAT(CDOB,'{$convertFormat}') AS CDOB,DATE_FORMAT(MrgRegisteredDate,'{$convertFormat}') AS MrgRegisteredDate,DATE_FORMAT(CmPassportIssueDate,'{$convertFormat}') AS CmPassportIssueDate,DATE_FORMAT(CmPassportExpiryDate,'{$convertFormat}') AS CmPassportExpiryDate,DATE_FORMAT(CmDrvLicIssueDate,'{$convertFormat}') AS CmDrvLicIssueDate,DATE_FORMAT(CmDrvLicExpiryDate,'{$convertFormat}') AS CmDrvLicExpiryDate,DATE_FORMAT(deactivatedDate,'{$convertFormat}') AS deactivatedDate from srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND comVerifiApproved='1' ")->row_array();
    ...
    function saveOccupation()
    {
        ...
        if ($OccTypeID == 1) {
            ...
            $JobMonthlyIncome = $this->input->post('JobMonthlyIncome');
    ...
    function member_image_upload()
    {
        ...
        $Com_MasterID = trim($this->input->post('MemberID'));
        $companyid = current_companyID();

        $memImageexist = $this->db->query("SELECT CImage FROM `srp_erp_ngo_com_communitymaster` 
        WHERE companyID = $companyid AND Com_MasterID = $Com_MasterID")->row_array();
        if (!empty($memImageexist['CImage'])) {
            $this->s3->deleteMyObject($memImageexist['CImage']);
        }

        $info = new SplFileInfo($_FILES["files"]["name"]);
        $fileName = 'Member_' . $companyCode . '_' . $Com_MasterID . '.' . $info->getExtension();
        ...
        $prefix = $this->config->item('folder_prefix');

        $path = "$prefix/$companyCode/MemberImages/$fileName";
        $input = $this->s3->inputFile($file['tmp_name']);
        $s3Upload = $this->s3->putMyObject($input, $path);
        ...
        if (!$s3Upload) {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                return array('e', "Member Image Upload Failed." . $this->db->_error_message());
            } else {
                $this->db->trans_commit();

                return array('s', 'Member Image uploaded  Successfully.');
            }
        }
    }
    ...
    function save_collection_members()
    {
        ...
        $Com_MasterIDArr = $this->input->post('Com_MasterID2');
        $Amount = $this->input->post('Amount');
        ...
        if (empty($Com_MasterIDArr)) {
            ...
            $companyFinanceYearID = $CollectionDetails{
                'companyFinanceYearID'};
            $companyFinanceYear = $CollectionDetails{
                'companyFinanceYear'};
            $PeriodTypeID = $CollectionDetails{
                'PeriodTypeID'};
            $transactionCurrencyID = $CollectionDetails{
                'transactionCurrencyID'};
            $transactionCurrency = $CollectionDetails{
                'transactionCurrency'};
            $transactionCurrencyDecimalPlaces = $CollectionDetails{
                'transactionCurrencyDecimalPlaces'};
            $segmentID = $CollectionDetails{
                'segmentID'};
            $segmentCode = $CollectionDetails{
                'segmentCode'};
            ...
            foreach ($Com_MasterIDArr as $key => $Com_MasterID) {
                ...
                if (isset($isCustomerExist)) {
                    ...
                    $MemberCode = $MemberDetails{
                        'MemberCode'};
                    $CName_with_initials = $MemberDetails{
                        'CName_with_initials'};
                    $P_Address = $MemberDetails{
                        'P_Address'};
                    $C_Address = $MemberDetails{
                        'C_Address'};

                    $Membercountry = $this->db->query("SELECT CountryDes FROM srp_erp_countrymaster WHERE countryID = {$MemberDetails{
                        'countyID'}} ")->row_array();
                    $Country = $Membercountry{
                        'CountryDes'};

                    $Telephone = $MemberDetails{
                        'TP_Mobile'};
                    $EmailID = $MemberDetails{
                        'EmailID'};
                    ...
                    $customer_receivableAutoID = $CusConfigDetails{
                        'receivableAutoID'};
                    $customer_receivableSystemGLCode = $CusConfigDetails{
                        'receivableSystemGLCode'};
                    $customer_receivableGLAccount = $CusConfigDetails{
                        'receivableGLAccount'};
                    $customer_receivableDescription = $CusConfigDetails{
                        'receivableDescription'};
                    $customer_receivableType = $CusConfigDetails{
                        'receivableType'};
                    $partyCategoryID = $CusConfigDetails{
                        'partyCategoryID'};
                ...
                $company_curYr = $this->db->query("SELECT isActive FROM srp_erp_companyfinanceperiod WHERE companyID = {$companyID} AND companyFinanceYearID = {$companyFinanceYearID} AND MONTH(dateFrom)={$curMonth}")->row_array();
                $activeCheck = $company_curYr['isActive'];
                ...
                if ($PeriodTypeID == '1') { //Monthly
                    ...
                    foreach ($company_finance_year_period as $row_periods) {
                        ...
                        if ($activeCheck == 1) {
                            ...
                            return array('e', 'Current Invoice Month : In-active.');
                        }
                    }
                    ...
                    foreach ($company_finance_year_period as $row_periods) {
                        ...
                        if ($activeCheck == 1 && ($MonthID == 1 || $MonthID == 4 || $MonthID == 7 || $MonthID == 10)) {
                            $ValidPeriodArr[] = array(
                                'companyFinancePeriodID' => $row_periods['companyFinancePeriodID'],
                                'invoiceDate' => $row_periods['dateFrom'],
                                'customerInvoiceDate' => $row_periods['dateFrom'],
                    ...
                    foreach ($company_finance_year_period as $row_periods) {
                        ...
                        if ($activeCheck == 1 && ($MonthID == 1 || $MonthID == 6)) {
                            $ValidPeriodArr[] = array(
                                'companyFinancePeriodID' => $row_periods['companyFinancePeriodID'],
                                'invoiceDate' => $row_periods['dateFrom'],
                                'customerInvoiceDate' => $row_periods['dateFrom'],
                ...
                if (!empty($ValidPeriodArr)) {
                    ...
                    foreach ($ValidPeriodArr as $ValidPeriodID) {
                        ...
                        if (!empty($CollectionMaster)) {
                            ...
                            $revenueGLAutoID = $CollectionMaster{
                                'revenueGLAutoID'};
                            $revenueSystemGLCode = $CollectionMaster{
                                'revenueSystemGLCode'};
                            $revenueGLCode = $CollectionMaster{
                                'revenueGLCode'};
                            $revenueGLDescription = $CollectionMaster{
                                'revenueGLDescription'};
                            $revenueGLType = $CollectionMaster{
                                'revenueGLType'};

                            $receivableAutoID = $CollectionMaster{
                                'receivableAutoID'};
                            $receivableSystemGLCode = $CollectionMaster{
                                'receivableSystemGLCode'};
                            $receivableGLAccount = $CollectionMaster{
                                'receivableGLAccount'};
                            $receivableDescription = $CollectionMaster{
                                'receivableDescription'};
                            $receivableType = $CollectionMaster{
                                'receivableType'};

                            $Description = $CollectionMaster{
                                'Description'};
                            $CollectionDes = $CollectionMaster{
                                'CollectionDes'};
...
HAVING
    ...
    function create_invoice($itemIssueAutoID)
    {
        ...
        if (isset($isCustomerExist)) {
            ...
            $customerTelephone = $isCustomerExist['customerTelephone'];
            $customerEmail = $isCustomerExist['customerEmail'];
            $customerreceivableAutoID = $isCustomerExist{
                'receivableAutoID'};
            $customerreceivableSystemGLCode = $isCustomerExist{
                'receivableSystemGLCode'};
            $customerreceivableGLAccount = $isCustomerExist{
                'receivableGLAccount'};
            $customerreceivableDescription = $isCustomerExist{
                'receivableDescription'};
            $customerreceivableType = $isCustomerExist{
                'receivableType'};
        } else {
            ...
            //customer acc details
            $CusConfigDetails = $this->db->query("SELECT * FROM srp_erp_ngo_com_customerconfig WHERE srp_erp_ngo_com_customerconfig.companyID = {$companyID} AND isActive = 1 ")->row_array();
            $receivableAutoID = $CusConfigDetails{
                'receivableAutoID'};
            $receivableSystemGLCode = $CusConfigDetails{
                'receivableSystemGLCode'};
            $receivableGLAccount = $CusConfigDetails{
                'receivableGLAccount'};
            $receivableDescription = $CusConfigDetails{
                'receivableDescription'};
            $receivableType = $CusConfigDetails{
                'receivableType'};
            $partyCategoryID = $CusConfigDetails{
                'partyCategoryID'};
            ...
            $customerTelephone = $data['customerTelephone'];
            $customerEmail = $data['customerEmail'];
            $customerreceivableAutoID = $data{
                'receivableAutoID'};
            $customerreceivableSystemGLCode = $data{
                'receivableSystemGLCode'};
            $customerreceivableGLAccount = $data{
                'receivableGLAccount'};
            $customerreceivableDescription = $data{
                'receivableDescription'};
            $customerreceivableType = $data{
                'receivableType'};
        }
    ...
    function save_familyHouseEnroll()
    {
        ...
        $data['rentHouseOwnerId'] = $this->input->post('rentHouseOwnerId');
        ...
        $data['drinkSourceId'] = $this->input->post('drinkSourceId');
        $data['isAgriWaterSup'] = $this->input->post('isAgriWaterSup');
        $data['agriculSourceId'] = $this->input->post('agriculSourceId');
        $data['isHmToilet'] = $this->input->post('isHmToilet');
        $data['SanitationStateID'] = $this->input->post('SanitationStateID');
        ...
        $data['isAffByDisaster'] = $this->input->post('isAffByDisaster');
        $data['DisasterTypeID'] = $this->input->post('DisasterTypeID');
        $data['DisasterYear'] = $this->input->post('DisasterYear');
        $data['DisasterRemarks'] = $this->input->post('DisasterRemarks');
        ...
        if ($FamHouseCn == '1') {
            ...
            $qhouseExitChk = $this->db->where('hEnrollingID', ($hEnrollingID))
                ->select('*')
                ->from('srp_erp_ngo_com_house_enrolling')
                ->get();
            $houseExitChk = $qhouseExitChk->row();

            if (!empty($houseExitChk)) {
                $delhEnrollingID = $this->db->delete('srp_erp_ngo_com_house_enrolling', array('hEnrollingID' => $hEnrollingID));
            }
        }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            if ($FamHouseCn == '1') {
                ...
                return array('e', 'Community Family House Enrolling Removed Successfully.', $last_id);
            }
        }
    }
    ...
    function save_familyMaster()
    {
        ...
        if (($FamMasterID == '' || $FamMasterID == NULL) && isset($isCommExist)) {
            ...
            if ($FamMasterID == '') {
                //get member mahallah
                $qryMember = $this->db->query('SELECT RegionID,countyID FROM srp_erp_ngo_com_communitymaster WHERE companyID="' . $companyID . '" AND Com_MasterID="' . $LeaderID . '"');
                $rowMember = $qryMember->row();
                $getAreaCode = $this->db->query("SELECT memArea.masterID,memArea.shortCode FROM srp_erp_statemaster memArea WHERE memArea.stateID={$rowMember->RegionID} AND memArea.countyID = {$rowMember->countyID}");
                $memAreaCode = $getAreaCode->row();
                $rowAreaCode = $memAreaCode->shortCode;
 ...
 FROM srp_erp_ngo_com_familydetails INNER JOIN srp_erp_ngo_com_communitymaster ON srp_erp_ngo_com_familydetails.Com_MasterID=srp_erp_ngo_com_communitymaster.Com_MasterID WHERE srp_erp_ngo_com_familydetails.FamMasterID ={$FamMasterID} AND srp_erp_ngo_com_familydetails.relationshipID='4'");
            ...
            if (!empty($rowFSide)) {
                ...
                if (!empty($rowFJob)) {
                    $fatherJob = $rowFJob->specializationID;
                } else {
                    $fatherJob = '';
                }
            }
 ...
 FROM srp_erp_ngo_com_familydetails INNER JOIN srp_erp_ngo_com_communitymaster ON srp_erp_ngo_com_familydetails.Com_MasterID=srp_erp_ngo_com_communitymaster.Com_MasterID WHERE srp_erp_ngo_com_familydetails.FamMasterID ={$FamMasterID} AND srp_erp_ngo_com_familydetails.relationshipID='5'");
            ...
            if (!empty($rowMSide)) {
                ...
                if (!empty($rowMJob)) {
                    $motherJob = $rowMJob->specializationID;
                } else {
                    $motherJob = '';
                }
            }
    ...
    function save_famMembers_detail()
    {
        ...
        $cfComMasID = $this->input->post('cfComMasID');
        $cmComMasID = $this->input->post('cmComMasID');
        $date_format_policy = date_format_policy();
        ...
        foreach ($Com_MasterID as $key => $femMemId) {
            ...
            if (empty($chkExist)) {
                ...
                if (!empty($cfComMasID[$key])) {
                    $fatherJob = '';
                    $get_fatherSide = $this->db->query("SELECT srp_erp_ngo_com_communitymaster.Com_MasterID,CFullName,CDOB,CNIC_No,countyID,RegionID,CBC_No,CBC_Date,cfComMasID,cmComMasID,cf_GdFComMasID,cf_GdMComMasID,
 fatherIsAlive,cfDateOfDeath,cfFullName,cFatherDOB,cfBornCountry,cfBornArea,cfBC_No,cfBCDate,cfOccupationId,cfNIC_No,cfDC_No,cfDCDate,
 motherIsAlive,cmDateOfDeath,cmFullName,cMotherDOB,cmBornCountry,cmBornArea,cmBC_No,cmBCDate,cmOccupationId,cmNIC_No,cmDC_No,cmDCDate,
 cf_GrandFIsAlive,cf_GrandFDateOfDeath,cf_GrandFFullName,cf_GrandFDOB,cf_GrandFBornCountry,cf_GrandFBornArea,cf_GrandFBC_No,cf_GrandFBCDate,cf_GrandFOccupationId,cf_GrandFNIC_No,cf_GrandFDC_No,cf_GrandFDCDate,
 cf_GrandMIsAlive,cf_GrandMDateOfDeath,cf_GrandMFullName,cf_GrandMDOB,cf_GrandMBornCountry,cf_GrandMBornArea,cf_GrandMBC_No,cf_GrandMBCDate,cf_GrandMOccupationId,cf_GrandMNIC_No,cf_GrandMDC_No,cf_GrandMDCDate
 FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.Com_MasterID ={$cfComMasID[$key]} AND srp_erp_ngo_com_communitymaster.companyID={$companyID}");
                    $rowFSide = $get_fatherSide->row();
                    if (!empty($rowFSide)) {
                        $queryFJob = $this->db->query("SELECT DISTINCT MemJobID,specializationID FROM srp_erp_ngo_com_memjobs WHERE srp_erp_ngo_com_memjobs.Com_MasterID={$rowFSide->Com_MasterID} AND srp_erp_ngo_com_memjobs.companyID={$companyID} ");
                        $rowFJob = $queryFJob->row();
                        ...
                        //update father
                        $ftrSdData['fatherIsAlive'] = 1;
                        $ftrSdData['cfComMasID'] = $cfComMasID[$key];
                        $ftrSdData['cfFullName'] = $rowFSide->CFullName;
                        $ftrSdData['cFatherDOB'] = $rowFSide->CDOB;
                        $ftrSdData['cfBornCountry'] = $rowFSide->countyID;
                        $ftrSdData['cfBornArea'] = $rowFSide->RegionID;
                        ...
                        $ftrSdData['cfOccupationId'] = $fatherJob;
                        $ftrSdData['cfNIC_No'] = $rowFSide->CNIC_No;
                        $ftrSdData['cf_GdFComMasID'] = $rowFSide->cfComMasID;
                        $ftrSdData['cf_GrandFIsAlive'] = $rowFSide->fatherIsAlive;
                        $ftrSdData['cf_GrandFDateOfDeath'] = $rowFSide->cfDateOfDeath;
                        $ftrSdData['cf_GrandFFullName'] = $rowFSide->cfFullName;
                        $ftrSdData['cf_GrandFDOB'] = $rowFSide->cFatherDOB;
                        ...
                        $ftrSdData['cf_GrandFDC_No'] = $rowFSide->cfDC_No;
                        $ftrSdData['cf_GrandFDCDate'] = $rowFSide->cfDCDate;
                        $ftrSdData['cf_GdMComMasID'] = $rowFSide->cmComMasID;
                        $ftrSdData['cf_GrandMIsAlive'] = $rowFSide->motherIsAlive;
                        $ftrSdData['cf_GrandMDateOfDeath'] = $rowFSide->cmDateOfDeath;
                        $ftrSdData['cf_GrandMFullName'] = $rowFSide->cmFullName;
                        $ftrSdData['cf_GrandMDOB'] = $rowFSide->cMotherDOB;
                        ...
                        $ftrSdData['cf_GrandMDC_No'] = $rowFSide->cmDC_No;
                        $ftrSdData['cf_GrandMDCDate'] = $rowFSide->cmDCDate;
                        $ftrSdData['cf_grt_GdFComMasID'] = $rowFSide->cf_GdFComMasID;
                        $ftrSdData['cf_grt_GrandFIsAlive'] = $rowFSide->cf_GrandFIsAlive;
                        $ftrSdData['cf_grt_GrandFDateOfDeath'] = $rowFSide->cf_GrandFDateOfDeath;
                        $ftrSdData['cf_grt_GrandFFullName'] = $rowFSide->cf_GrandFFullName;
                        $ftrSdData['cf_grt_GrandFDOB'] = $rowFSide->cf_GrandFDOB;
                        ...
                        $ftrSdData['cf_grt_GrandFDC_No'] = $rowFSide->cf_GrandFDC_No;
                        $ftrSdData['cf_grt_GrandFDCDate'] = $rowFSide->cf_GrandFDCDate;
                        $ftrSdData['cf_grt_GdMComMasID'] = $rowFSide->cf_GdMComMasID;
                        $ftrSdData['cf_grt_GrandMIsAlive'] = $rowFSide->cf_GrandMIsAlive;
                        $ftrSdData['cf_grt_GrandMDateOfDeath'] = $rowFSide->cf_GrandMDateOfDeath;
                        $ftrSdData['cf_grt_GrandMFullName'] = $rowFSide->cf_GrandMFullName;
                        $ftrSdData['cf_grt_GrandMDOB'] = $rowFSide->cf_GrandMDOB;
                        ...
                        $this->db->where('Com_MasterID', $Com_MasterID[$key]);
                        $update = $this->db->update('srp_erp_ngo_com_communitymaster', $ftrSdData);
                    }
                }
                if (!empty($cmComMasID[$key])) {
                    $motherJob = '';
                    $get_motherSide = $this->db->query("SELECT srp_erp_ngo_com_communitymaster.Com_MasterID,CFullName,CDOB,CNIC_No,countyID,RegionID,CBC_No,CBC_Date,cfComMasID,cmComMasID,cm_GdFComMasID,cm_GdMComMasID,
 fatherIsAlive,cfDateOfDeath,cfFullName,cFatherDOB,cfBornCountry,cfBornArea,cfBC_No,cfBCDate,cfOccupationId,cfNIC_No,cfDC_No,cfDCDate,
 motherIsAlive,cmDateOfDeath,cmFullName,cMotherDOB,cmBornCountry,cmBornArea,cmBC_No,cmBCDate,cmOccupationId,cmNIC_No,cmDC_No,cmDCDate,
 cm_GrandFIsAlive,cm_GrandFDateOfDeath,cm_GrandFFullName,cm_GrandFDOB,cm_GrandFBornCountry,cm_GrandFBornArea,cm_GrandFBC_No,cm_GrandFBCDate,cm_GrandFOccupationId,cm_GrandFNIC_No,cm_GrandFDC_No,cm_GrandFDCDate,
 cm_GrandMIsAlive,cm_GrandMDateOfDeath,cm_GrandMFullName,cm_GrandMDOB,cm_GrandMBornCountry,cm_GrandMBornArea,cm_GrandMBC_No,cm_GrandMBCDate,cm_GrandMOccupationId,cm_GrandMNIC_No,cm_GrandMDC_No,cm_GrandMDCDate
 FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.Com_MasterID ={$cmComMasID[$key]} AND srp_erp_ngo_com_communitymaster.companyID={$companyID}");
                    $rowMSide = $get_motherSide->row();
                    if (!empty($rowMSide)) {
                        $queryMJob = $this->db->query("SELECT DISTINCT MemJobID,specializationID FROM srp_erp_ngo_com_memjobs WHERE srp_erp_ngo_com_memjobs.Com_MasterID={$rowMSide->Com_MasterID} AND srp_erp_ngo_com_memjobs.companyID={$companyID} ");
                        $rowMJob = $queryMJob->row();
                        ...
                        //update mother
                        $mtrSdData['motherIsAlive'] = 1;
                        $mtrSdData['cmComMasID'] = $cmComMasID[$key];
                        $mtrSdData['cmFullName'] = $rowMSide->CFullName;
                        $mtrSdData['cMotherDOB'] = $rowMSide->CDOB;
                        $mtrSdData['cmBornCountry'] = $rowMSide->countyID;
                        $mtrSdData['cmBornArea'] = $rowMSide->RegionID;
                        ...
                        $mtrSdData['cmOccupationId'] = $motherJob;
                        $mtrSdData['cmNIC_No'] = $rowMSide->CNIC_No;
                        $mtrSdData['cm_GdFComMasID'] = $rowMSide->cfComMasID;
                        $mtrSdData['cm_GrandFIsAlive'] = $rowMSide->fatherIsAlive;
                        $mtrSdData['cm_GrandFDateOfDeath'] = $rowMSide->cfDateOfDeath;
                        $mtrSdData['cm_GrandFFullName'] = $rowMSide->cfFullName;
                        $mtrSdData['cm_GrandFDOB'] = $rowMSide->cFatherDOB;
                        ...
                        $mtrSdData['cm_GrandFDC_No'] = $rowMSide->cfDC_No;
                        $mtrSdData['cm_GrandFDCDate'] = $rowMSide->cfDCDate;
                        $mtrSdData['cm_GdMComMasID'] = $rowMSide->cmComMasID;
                        $mtrSdData['cm_GrandMIsAlive'] = $rowMSide->motherIsAlive;
                        $mtrSdData['cm_GrandMDateOfDeath'] = $rowMSide->cmDateOfDeath;
                        $mtrSdData['cm_GrandMFullName'] = $rowMSide->cmFullName;
                        $mtrSdData['cm_GrandMDOB'] = $rowMSide->cMotherDOB;
                        ...
                        $mtrSdData['cm_GrandMDC_No'] = $rowMSide->cmDC_No;
                        $mtrSdData['cm_GrandMDCDate'] = $rowMSide->cmDCDate;
                        $mtrSdData['cm_grt_GdFComMasID'] = $rowMSide->cm_GdFComMasID;
                        $mtrSdData['cm_grt_GrandFIsAlive'] = $rowMSide->cm_GrandFIsAlive;
                        $mtrSdData['cm_grt_GrandFDateOfDeath'] = $rowMSide->cm_GrandFDateOfDeath;
                        $mtrSdData['cm_grt_GrandFFullName'] = $rowMSide->cm_GrandFFullName;
                        $mtrSdData['cm_grt_GrandFDOB'] = $rowMSide->cm_GrandFDOB;
                        ...
                        $mtrSdData['cm_grt_GrandFDC_No'] = $rowMSide->cm_GrandFDC_No;
                        $mtrSdData['cm_grt_GrandFDCDate'] = $rowMSide->cm_GrandFDCDate;
                        $mtrSdData['cm_grt_GdMComMasID'] = $rowMSide->cm_GdMComMasID;
                        $mtrSdData['cm_grt_GrandMIsAlive'] = $rowMSide->cm_GrandMIsAlive;
                        $mtrSdData['cm_grt_GrandMDateOfDeath'] = $rowMSide->cm_GrandMDateOfDeath;
                        $mtrSdData['cm_grt_GrandMFullName'] = $rowMSide->cm_GrandMFullName;
                        $mtrSdData['cm_grt_GrandMDOB'] = $rowMSide->cm_GrandMDOB;
    ...
    function update_comFem_member_details()
    {
        ...
        $this->db->update('srp_erp_ngo_com_familydetails', $data, array('FamDel_ID' => $FamDel_ID));
        $last_id = 0; //$this->db->insert_id();
 ...
 cf_GrandFIsAlive,cf_GrandFDateOfDeath,cf_GrandFFullName,cf_GrandFDOB,cf_GrandFBornCountry,cf_GrandFBornArea,cf_GrandFBC_No,cf_GrandFBCDate,cf_GrandFOccupationId,cf_GrandFNIC_No,cf_GrandFDC_No,cf_GrandFDCDate,
 cf_GrandMIsAlive,cf_GrandMDateOfDeath,cf_GrandMFullName,cf_GrandMDOB,cf_GrandMBornCountry,cf_GrandMBornArea,cf_GrandMBC_No,cf_GrandMBCDate,cf_GrandMOccupationId,cf_GrandMNIC_No,cf_GrandMDC_No,cf_GrandMDCDate
 FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.Com_MasterID ={$cfComMasID} AND srp_erp_ngo_com_communitymaster.companyID={$companyID}");
            $rowFSide = $get_fatherSide->row();
            if (!empty($rowFSide)) {
                $queryFJob = $this->db->query("SELECT DISTINCT MemJobID,specializationID FROM srp_erp_ngo_com_memjobs WHERE srp_erp_ngo_com_memjobs.Com_MasterID={$rowFSide->Com_MasterID} AND srp_erp_ngo_com_memjobs.companyID={$companyID} ");
                $rowFJob = $queryFJob->row();
                ...
                //update father
                $ftrSdData['fatherIsAlive'] = 1;
                $ftrSdData['cfComMasID'] = $cfComMasID;
                $ftrSdData['cfFullName'] = $rowFSide->CFullName;
                $ftrSdData['cFatherDOB'] = $rowFSide->CDOB;
                $ftrSdData['cfBornCountry'] = $rowFSide->countyID;
                $ftrSdData['cfBornArea'] = $rowFSide->RegionID;
 ...
 cm_GrandFIsAlive,cm_GrandFDateOfDeath,cm_GrandFFullName,cm_GrandFDOB,cm_GrandFBornCountry,cm_GrandFBornArea,cm_GrandFBC_No,cm_GrandFBCDate,cm_GrandFOccupationId,cm_GrandFNIC_No,cm_GrandFDC_No,cm_GrandFDCDate,
 cm_GrandMIsAlive,cm_GrandMDateOfDeath,cm_GrandMFullName,cm_GrandMDOB,cm_GrandMBornCountry,cm_GrandMBornArea,cm_GrandMBC_No,cm_GrandMBCDate,cm_GrandMOccupationId,cm_GrandMNIC_No,cm_GrandMDC_No,cm_GrandMDCDate
 FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.Com_MasterID ={$cmComMasID} AND srp_erp_ngo_com_communitymaster.companyID={$companyID}");
            $rowMSide = $get_motherSide->row();
            if (!empty($rowMSide)) {
                $queryMJob = $this->db->query("SELECT DISTINCT MemJobID,specializationID FROM srp_erp_ngo_com_memjobs WHERE srp_erp_ngo_com_memjobs.Com_MasterID={$rowMSide->Com_MasterID} AND srp_erp_ngo_com_memjobs.companyID={$companyID} ");
                $rowMJob = $queryMJob->row();
                ...
                //update mother
                $mtrSdData['motherIsAlive'] = 1;
                $mtrSdData['cmComMasID'] = $cmComMasID;
                $mtrSdData['cmFullName'] = $rowMSide->CFullName;
                $mtrSdData['cMotherDOB'] = $rowMSide->CDOB;
                $mtrSdData['cmBornCountry'] = $rowMSide->countyID;
                $mtrSdData['cmBornArea'] = $rowMSide->RegionID;
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->session->set_flashdata('s', 'Member Detail : Records updated successfully.');
            $this->db->trans_commit();

            return array('s', 'Member Detail : Records updated successfully. ');
        }
    }

    function fetch_commMember_confirmation($Com_MasterID)
    {
        ...
        $where = "tCom.companyID = " . $companyID . " AND Com_MasterID = '" . $Com_MasterID . "' AND tCom.comVerifiApproved='1'";

        $data['master'] = $this->db->query("SELECT *,divisionc.stateID,divisionc.Description AS GS_DivisionSt
,DATE_FORMAT(cfDateOfDeath,'{$convertFormat}') AS cfDateOfDeath,cfFullName,DATE_FORMAT(cFatherDOB,'{$convertFormat}') AS cFatherDOB,DATE_FORMAT(cfBCDate,'{$convertFormat}') AS cfBCDate,DATE_FORMAT(cfDCDate,'{$convertFormat}') AS cfDCDate,
DATE_FORMAT(cmDateOfDeath,'{$convertFormat}') AS cmDateOfDeath,cmFullName,DATE_FORMAT(cMotherDOB,'{$convertFormat}') AS cMotherDOB,DATE_FORMAT(cmBCDate,'{$convertFormat}') AS cmBCDate,DATE_FORMAT(cmDCDate,'{$convertFormat}') AS cmDCDate,
t2.CountryDes AS fCountry,t3.description AS fArea,t8.CountryDes AS mCountry,t9.description AS mArea,t6.description AS fJobDes,t11.description AS mJobDes
...
,DATE_FORMAT(cm_GrandMDateOfDeath,'{$convertFormat}') AS cm_GrandMDateOfDeath,DATE_FORMAT(cm_GrandMDOB,'{$convertFormat}') AS cm_GrandMDOB,p12.CountryDes AS mgmCountry,p10.description AS mgmArea,p11.description AS mgmJobDes,DATE_FORMAT(cm_GrandMBCDate,'{$convertFormat}') AS cm_GrandMBCDate,DATE_FORMAT(cm_GrandMDCDate,'{$convertFormat}') AS cm_GrandMDCDate
,DATE_FORMAT(cf_grt_GrandFDateOfDeath,'{$convertFormat}') AS cf_grt_GrandFDateOfDeath,DATE_FORMAT(cf_grt_GrandFDOB,'{$convertFormat}') AS cf_grt_GrandFDOB,g2.CountryDes AS fgptCountry,g3.description AS fgptArea,g6.description AS fgptJobDes,DATE_FORMAT(cf_grt_GrandFBCDate,'{$convertFormat}') AS cf_grt_GrandFBCDate,DATE_FORMAT(cf_grt_GrandFDCDate,'{$convertFormat}') AS cf_grt_GrandFDCDate
,DATE_FORMAT(cf_grt_GrandMDateOfDeath,'{$convertFormat}') AS cf_grt_GrandMDateOfDeath,DATE_FORMAT(cf_grt_GrandMDOB,'{$convertFormat}') AS cf_grt_GrandMDOB,g8.CountryDes AS fgmtCountry,g9.description AS fgmtArea,g1.description AS fgmtJobDes,DATE_FORMAT(cf_grt_GrandMBCDate,'{$convertFormat}') AS cf_grt_GrandMBCDate,DATE_FORMAT(cf_grt_GrandMDCDate,'{$convertFormat}') AS cf_grt_GrandMDCDate
,DATE_FORMAT(cm_grt_GrandFDateOfDeath,'{$convertFormat}') AS cm_grt_GrandFDateOfDeath,DATE_FORMAT(cm_grt_GrandFDOB,'{$convertFormat}') AS cm_grt_GrandFDOB,g4.CountryDes AS mgptCountry,g5.description AS mgptArea,g7.description AS mgptJobDes,DATE_FORMAT(cm_grt_GrandFBCDate,'{$convertFormat}') AS cm_grt_GrandFBCDate,DATE_FORMAT(cm_grt_GrandFDCDate,'{$convertFormat}') AS cm_grt_GrandFDCDate
,DATE_FORMAT(cm_grt_GrandMDateOfDeath,'{$convertFormat}') AS cm_grt_GrandMDateOfDeath,DATE_FORMAT(cm_grt_GrandMDOB,'{$convertFormat}') AS cm_grt_GrandMDOB,g12.CountryDes AS mgmtCountry,g10.description AS mgmtArea,g11.description AS mgmtJobDes,DATE_FORMAT(cm_grt_GrandMBCDate,'{$convertFormat}') AS cm_grt_GrandMBCDate,DATE_FORMAT(cm_grt_GrandMDCDate,'{$convertFormat}') AS cm_grt_GrandMDCDate
,tCom.createdDateTime,tCom.createdUserName,tCom.modifiedDateTime FROM srp_erp_ngo_com_communitymaster AS tCom
LEFT JOIN srp_titlemaster ON srp_titlemaster.TitleID = tCom.TitleID LEFT JOIN srp_erp_gender ON srp_erp_gender.genderID = tCom.GenderID 
LEFT JOIN srp_erp_statemaster divisionc ON divisionc.stateID = tCom.GS_Division LEFT JOIN srp_erp_statemaster area ON area.stateID = tCom.RegionID 
LEFT JOIN srp_erp_bloodgrouptype ON srp_erp_bloodgrouptype.BloodTypeID = tCom.BloodGroupID LEFT JOIN srp_erp_ngo_com_maritalstatus ON srp_erp_ngo_com_maritalstatus.maritalstatusID = tCom.CurrentStatus 
LEFT JOIN srp_erp_ngo_com_schools ON srp_erp_ngo_com_schools.schoolComID=tCom.SchoolAttended LEFT JOIN srp_erp_countrymaster ON srp_erp_countrymaster.countryID=tCom.CountryOfResidentID 
LEFT JOIN srp_erp_countrymaster AS t2 ON t2.countryID = tCom.cfBornCountry LEFT JOIN srp_erp_statemaster AS t3 ON t3.stateID = tCom.cfBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS t6 ON t6.specializationID = tCom.cfOccupationId
LEFT JOIN srp_erp_countrymaster AS t8 ON t8.countryID = tCom.cmBornCountry LEFT JOIN srp_erp_statemaster AS t9 ON t9.stateID = tCom.cmBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS t11 ON t11.specializationID = tCom.cmOccupationId   
LEFT JOIN srp_erp_countrymaster AS p2 ON p2.countryID = tCom.cf_GrandFBornCountry LEFT JOIN srp_erp_statemaster AS p3 ON p3.stateID = tCom.cf_GrandFBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS p6 ON p6.specializationID = tCom.cf_GrandFOccupationId
LEFT JOIN srp_erp_countrymaster AS p8 ON p8.countryID = tCom.cf_GrandMBornCountry LEFT JOIN srp_erp_statemaster AS p9 ON p9.stateID = tCom.cf_GrandMBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS p1 ON p1.specializationID = tCom.cf_GrandMOccupationId                     
LEFT JOIN srp_erp_countrymaster AS p4 ON p4.countryID = tCom.cm_GrandFBornCountry LEFT JOIN srp_erp_statemaster AS p5 ON p5.stateID = tCom.cm_GrandFBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS p7 ON p7.specializationID = tCom.cm_GrandFOccupationId
LEFT JOIN srp_erp_countrymaster AS p12 ON p12.countryID = tCom.cm_GrandMBornCountry LEFT JOIN srp_erp_statemaster AS p10 ON p10.stateID = tCom.cm_GrandMBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS p11 ON p11.specializationID = tCom.cm_GrandMOccupationId    
LEFT JOIN srp_erp_countrymaster AS g2 ON g2.countryID = tCom.cf_grt_GrandFBornCountry LEFT JOIN srp_erp_statemaster AS g3 ON g3.stateID = tCom.cf_grt_GrandFBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS g6 ON g6.specializationID = tCom.cf_grt_GrandFOccupationId
LEFT JOIN srp_erp_countrymaster AS g8 ON g8.countryID = tCom.cf_grt_GrandMBornCountry LEFT JOIN srp_erp_statemaster AS g9 ON g9.stateID = tCom.cf_grt_GrandMBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS g1 ON g1.specializationID = tCom.cf_grt_GrandMOccupationId                     
LEFT JOIN srp_erp_countrymaster AS g4 ON g4.countryID = tCom.cm_grt_GrandFBornCountry LEFT JOIN srp_erp_statemaster AS g5 ON g5.stateID = tCom.cm_grt_GrandFBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS g7 ON g7.specializationID = tCom.cm_grt_GrandFOccupationId
LEFT JOIN srp_erp_countrymaster AS g12 ON g12.countryID = tCom.cm_grt_GrandMBornCountry LEFT JOIN srp_erp_statemaster AS g10 ON g10.stateID = tCom.cm_grt_GrandMBornArea LEFT JOIN srp_erp_ngo_com_jobspecialization AS g11 ON g11.specializationID = tCom.cm_grt_GrandMOccupationId   
 WHERE $where ")->row_array();

        $data['cmQualification'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_qualifications AS t1
        ...
        $data['cmLanguage'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberlanguage AS t1
                                        JOIN srp_erp_lang_languages AS t2 ON t2.languageID = t1.LanguageID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "'")->result_array();

        $data['cmOccupation'] = $this->db->query("SELECT *,t6.description AS Medium,t8.description AS Specialization FROM srp_erp_ngo_com_memjobs AS t1
        ...
        $data['cmSickness'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberpersickness AS t1
                                        JOIN srp_erp_ngo_com_permanent_sickness AS t2 ON t2.sickAutoID = t1.sickAutoID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "'")->result_array();

        $data['cmVehicleConfig'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_membervehicles AS t1
                                        LEFT JOIN srp_erp_ngo_com_propertytypes AS t3 ON t3.PropertyID = t1.PropertyID
                                        LEFT JOIN srp_erp_ngo_com_vehicles_master AS t2 ON t2.vehicleAutoID = t1.vehicleAutoID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "'")->result_array();

        $data['cmHelpReqConGv'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberhelprequirements AS t1
        ...
        $data['cmHelpReqConPv'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberhelprequirements AS t1
                                        JOIN srp_erp_ngo_com_helprequirements AS t2 ON t2.helpRequireID = t1.helpRequireID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "' AND helpRequireType='PVT' ")->result_array();

        $data['cmHelpReqConCs'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberhelprequirements AS t1
                                        JOIN srp_erp_ngo_com_helprequirements AS t2 ON t2.helpRequireID = t1.helpRequireID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "' AND helpRequireType='CONS' ")->result_array();

        $data['cmHelpReqComOther'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberhelprequirements AS t1
                                        JOIN srp_erp_ngo_com_helprequirements AS t2 ON t2.helpRequireID = t1.helpRequireID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = {$Com_MasterID} AND helpRequireType='OTHER' ")->result_array();

        $data['cmWillingToHelp'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberwillingtohelp AS t1
                                        JOIN srp_erp_ngo_com_helpcategories AS t2 ON t2.helpCategoryID = t1.helpCategoryID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "' ")->result_array();

        $data['cmFamilyDetails'] = $this->db->query("select srp_erp_family_relationship.relationship,srp_erp_ngo_com_familyeconomicstatemaster.EconStateDes,femMaster.FamAncestory,srp_erp_ngo_com_ancestrycategory.AncestryDes,femMaster.monthlyExpenses,srp_erp_ngo_com_house_ownership_master.ownershipDescription,srp_erp_ngo_com_house_type_master.hTypeDescription,srp_erp_ngo_com_house_enrolling.FamHouseSt,exitHouseFam.FamilyName as exitFamilyName from srp_erp_ngo_com_familydetails INNER JOIN srp_erp_ngo_com_familymaster femMaster ON femMaster.FamMasterID=srp_erp_ngo_com_familydetails.FamMasterID LEFT JOIN srp_erp_family_relationship ON srp_erp_family_relationship.relationshipID=srp_erp_ngo_com_familydetails.relationshipID LEFT JOIN srp_erp_ngo_com_ancestrycategory ON femMaster.AncestryCatID=srp_erp_ngo_com_ancestrycategory.AncestryCatID LEFT JOIN srp_erp_ngo_com_familyeconomicstatemaster ON femMaster.ComEconSteID=srp_erp_ngo_com_familyeconomicstatemaster.EconStateID LEFT JOIN srp_erp_ngo_com_house_enrolling ON srp_erp_ngo_com_house_enrolling.FamMasterID=femMaster.FamMasterID LEFT JOIN srp_erp_ngo_com_house_ownership_master ON srp_erp_ngo_com_house_enrolling.ownershipAutoID=srp_erp_ngo_com_house_ownership_master.ownershipAutoID LEFT JOIN srp_erp_ngo_com_house_type_master ON srp_erp_ngo_com_house_enrolling.hTypeAutoID=srp_erp_ngo_com_house_type_master.hTypeAutoID LEFT JOIN srp_erp_ngo_com_house_enrolling exitHouse ON srp_erp_ngo_com_house_enrolling.Link_hEnrollingID=exitHouse.hEnrollingID LEFT JOIN srp_erp_ngo_com_familymaster exitHouseFam ON exitHouse.FamMasterID=exitHouseFam.FamMasterID
WHERE srp_erp_ngo_com_familydetails.companyID={$companyID} AND Com_MasterID = '" . $Com_MasterID . "' AND srp_erp_ngo_com_familydetails.isLeader='1'")->row_array();
    ...
    function delete_family_master()
    {
        ...
        if (empty($isFamInChk)) {
            ...
            $data = array(
                'isDeleted' => 1, 'deletedDate' => $this->common_data['current_date'],
    ...
    function familyCreate_confirmation()
    {
        ...
        if (!empty($data['fam_memRela'])) {
            ...
            $data = array(
                'confirmedYN' => 1, 'confirmedDate' => $this->common_data['current_date'],
                'confirmedByEmpID' => $this->common_data['current_userID'],
                'confirmedByName' => $this->common_data['current_user']
            );
...
WHERE hEnr.FamMasterID=$FamMasterID ORDER BY hEnr.hEnrollingID")->row_array();
            ...
            if ($last_id) {
                ...
                if ($Com_MasterID != NULL || $Com_MasterID != '') {
                    ...
                    if (!empty($rowFam) || $rowFam != NULL) {
                        $queryFamAdd = $this->db->query("SELECT FamMasterID,srp_erp_ngo_com_familydetails.Com_MasterID,CFullName,relationshipID,CDOB,GenderID,CNIC_No,CImage FROM srp_erp_ngo_com_familydetails INNER JOIN srp_erp_ngo_com_communitymaster ON srp_erp_ngo_com_communitymaster.Com_MasterID=srp_erp_ngo_com_familydetails.Com_MasterID WHERE FamMasterID ={$rowFam->FamMasterID} ");
    ...
    function save_comBeneficiary_familyDel()
    {
        $data = array(
            ...
            "image" => trim($this->input->post('CImage')),
            "createdUserID" => current_userID(),
            "createdPCid" => current_pc(),
            "timestamp" => $this->common_data['current_date']
        );
...
WHERE dds.DocDesID
    ...
    function delete_comBeneficiary_multipleImage()
    {
        ...
        if (!empty($itemimageExist['beneficiaryImage'])) {

            $this->s3->deleteMyObject('uploads/ngo/beneficiaryImage/' . $itemimageExist['beneficiaryImage']);
        }
    ...
    function fetch_LogFamilies_list($fam_key)
    {
        ...
        if (!empty($resLgFm)) {
            ...
            foreach ($resLgFm as $rowLgFm) {
                ...
                $communityimage = get_all_community_images($rowLgFm->CImage, 'Community/' . $companyCode . '/MemberImages/', 'commMemNoImg');
    ...
    function delete_project_proposal_image()
    {
        ...
        if (!empty($myFileName)) {
            ...
            $this->s3->deleteMyObject('uploads/ngo/projectProposalImage/' . $myFileName);
        }
  ...
  LEFT JOIN srp_erp_ngo_com_familymaster fm ON fm.FamMasterID=fd.FamMasterID LEFT JOIN srp_erp_ngo_com_communitymaster cm ON cm.Com_MasterID=fd.Com_MasterID WHERE fd.companyID='" . $companyID . "' AND fd.FamMasterID='" . $rowPbene->FamMasterID . "' AND cm.isActive='1' AND fd.isMove='0' AND cm.comVerifiApproved='1'");
            $rowag = $queryFD->row();
    ...
    function save_com_proj_proposal_attachments()
    {
    ...
    function delete_com_proj_proposal_attachment()
    {
        ...
        if (!empty($itemimageExist['myFileName'])) {
            $this->s3->deleteMyObject('uploads/ngo/attachments/' . $itemimageExist['myFileName']);
        }
    ...
    function update_beneficiary_edit()
    {
        ...
        $this->db->update(
            'srp_erp_ngo_projectproposalbeneficiaries',
            $data,
    ...
    function update_zakatSet_edit()
    {
        ...
        $this->db->update(
            'srp_erp_ngo_com_projectproposalzakatsetup',
            $data,
    ...
    public function saveUploadData_masCom()
    {
        ...
        if ($ComUploadType == 3) {
            if ($ComUploadDescription == FALSE) {
                ...
                $this->db->where('ComUploadType', 3);
                $num = $this->db->get('srp_erp_ngo_com_uploads')->result_array();
                $file_name = 'Upload_Document_' . (count($num) + 1);
                ...
                if (!$this->upload->do_upload("document_file")) {
                    ...
                    $ComUploadFileName = $file_name . $upload_data["file_ext"];
                    $ComUploadFileType = trim($upload_data["file_ext"]);
                    $ComUploadFileSize = trim($upload_data["file_size"]);
                }
            }
        } else {
            $ComUploadFileName = '';
        ...
        if (empty($isUpExist)) {
            ...
            $data_upload = array(
                ...
                'ComUploadFileName' => $ComUploadFileName,
                'ComUploadFileType' => $ComUploadFileType,
                'ComUploadFileSize' => $ComUploadFileSize,
    ...
    function delete_comUpload()
    {
        ...
        $ComUploadType = trim($this->input->post('ComUploadType'));
        ...
        if ($ComUploadType == 3) {
            $myFileName = $this->input->post('ComUploadFileName');
        ...
        foreach ($res as $row) {
            echo json_encode(
                array(
                    ...
                    "document_file" => $row->ComUploadFileName,
        ...
        foreach ($res as $row) {
            echo json_encode(
                array(
                    ...
                    "ComUploadSubject" => $row->ComUploadSubject,
                    "ComUpload_url" => $row->ComUpload_url,
                    "ComUploadFileName" => $row->ComUploadFileName,
                    "ComUploadDescription" => $row->ComUploadDescription,
                    "ComUploader" => $row->ComUploader,
    ...
    function com_donor_image_upload()
    {
        ...
        $info = new SplFileInfo($_FILES["files"]["name"]);
        $fileName = 'Community_donor_' . $this->common_data['company_data']['company_code'] . '_' . trim($this->input->post('contactID')) .  '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['files'];
        if ($file['error'] == 1) {
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
    ...
    function memDoc_app_detail($appMemID, $appMemName, $VerificationDocID, $memDocDescription)
    {
        ...
        $query = $this->db->query('SELECT FamMasterID,LeaderID,VerificationDocID,VerifyDocAttachmentFront,VerifyDocAttachmentBack,VerifyDocApprovalRemark,isVerifyDocApproved FROM srp_erp_ngo_com_familymaster WHERE srp_erp_ngo_com_familymaster.companyID="' . $companyID . '" AND LeaderID ="' . $appMemID . '" AND VerificationDocID ="' . $VerificationDocID . '"');
        $row = $query->row();

        if (empty($row)) {
            echo '
                 <div class="well well-sm"  style="position: relative; height: 400px; margin-bottom: 0px;">
                    <span style="text-align:center; color:#CCC; position: absolute; left: 42%;top: 35%;font-size: 14px;">Empty Gallery<br> <i class="fa fa-camera fa-5x" style="margin-top: 5px;"></i></span>
                </div>';
        } else {

            $memApprovalFrontImg = get_all_community_images($row->VerifyDocAttachmentFront, 'Community/family_registration/', 'communityNoImg');
            $memApprovalBackImg = get_all_community_images($row->VerifyDocAttachmentBack, 'Community/family_registration/', 'communityNoImg');

            echo '<div class="col-sm-12"><div class="col-md-2"></div><div class="col-md-4">
                                        <div class="thumbnail">
                                                <img style="width: 150px;height: 100px;" src="' . $memApprovalFrontImg . '"  alt="Nature"  onclick="open_ApproveDocViewer(1,' . $row->FamMasterID . ');">
                                                <div class="caption text-center">
                                                    <h5><b>Front Image</b></h5>
                                            </div>
                                        </div>
                                    </div>';
            ...
            echo ' <div class="col-md-4">
                                        <div class="thumbnail">
                                                <img style="width: 150px;height: 100px;" src="' . $memApprovalBackImg . '"  alt="Nature"  onclick="open_ApproveDocViewer(2,' . $row->FamMasterID . ');">
                                                <div class="caption text-center">
                                                    <h5><b>Back Image</b></h5>
                                            </div>
                                        </div>
            ...
            echo ' <div class="col-sm-12" style="margin-top: 10px;">
                ...
                <textarea class="form-control" id="description" name="description" rows="3" required>' . $row->VerifyDocApprovalRemark . '</textarea>
                    <span class="input-req-inner"></span>
                        ...
                        <div  class="col-sm-12">
                            ...
                            <div class="form-group col-md-10">
                              <select class="select2 isVerifyDocApproved" name="isVerifyDocApproved" id="isVerifyDocApproved" style="width: 100%;">';
            echo '<option></option>';

            if ($row->isVerifyDocApproved == 1) {
                echo '<option value="1" selected>' . $this->lang->line('communityngo_approved') . '</option>
                              <option value="0">' . $this->lang->line('communityngo_pending_approvals') . '</option>
                              <option value="4">' . $this->lang->line('communityngo_not_approved') . '</option>
                              <option value="5">' . $this->lang->line('communityngo_pending_clarification') . '</option>';
            } elseif ($row->isVerifyDocApproved == 4) {
                echo '<option value="1">' . $this->lang->line('communityngo_approved') . '</option>
                              <option value="0">' . $this->lang->line('communityngo_pending_approvals') . '</option>
                              <option value="4" selected>' . $this->lang->line('communityngo_not_approved') . '</option>
                              <option value="5">' . $this->lang->line('communityngo_pending_clarification') . '</option>';
            } elseif ($row->isVerifyDocApproved == 5) {
                echo '<option value="1" selected>' . $this->lang->line('communityngo_approved') . '</option>
                              <option value="0">' . $this->lang->line('communityngo_pending_approvals') . '</option>
                              <option value="4">' . $this->lang->line('communityngo_not_approved') . '</option>
                              <option value="5" selected>' . $this->lang->line('communityngo_pending_clarification') . '</option>';
            } else {
                echo '<option value="1">' . $this->lang->line('communityngo_approved') . '</option>
                              <option value="0" selected>' . $this->lang->line('communityngo_pending_approvals') . '</option>
                              <option value="4">' . $this->lang->line('communityngo_not_approved') . '</option>
                              <option value="5">' . $this->lang->line('communityngo_pending_clarification') . '</option>';
            }
    ...
    function memApprovalBody($docType, $FamMasterID)
    {

        $companyID = current_companyID();

        $query = $this->db->query('SELECT FamMasterID,LeaderID,VerificationDocID,VerifyDocAttachmentFront,VerifyDocAttachmentBack,VerifyDocApprovalRemark FROM srp_erp_ngo_com_familymaster WHERE srp_erp_ngo_com_familymaster.companyID="' . $companyID . '" AND FamMasterID ="' . $FamMasterID . '"');
        $res = $query->result();
        $a = '0';
        $b = '0';
        echo '<div id="carousel-example-generic" class="carousel slide" data-ride="carousel" style="height: auto;">
                 <ol class="carousel-indicators">';
        foreach ($res as $row) {
            if ($a == '0') {
                echo '<li data-target="#carousel-example-generic" data-slide-to="0" class="active"></li>';
            } else {
                echo '<li data-target="#carousel-example-generic" data-slide-to="' . $a . '" class=""></li>';
            }
            $a++;
        }
        echo '</ol>
        ...
        foreach ($res as $row) {
            ...
            $memApprovalBackImg = get_all_community_images($row->VerifyDocAttachmentBack, 'Community/family_registration/', 'communityNoImg');
            if ($docType == 1) {
                $first_slide = $memApprovalFrontImg;
                $second_slide = $memApprovalBackImg;
                $doc_desc = 'Front Image';
                $docBk_desc = 'Back Image';
            } else {
                $first_slide = $memApprovalBackImg;
                $second_slide = $memApprovalFrontImg;
                $doc_desc = 'Back Image';
                $docBk_desc = 'Front Image';
            }

            echo '<div class="item active">
                                <img src="' . $first_slide . '" alt="First slide">
                                <div class="carousel-caption" style="background-color: rgba(0, 0, 0, 0.5);padding: 0px;">' . $doc_desc . '</div>
                             </div>';

            echo '<div class="item">
                                <img src="' . $second_slide . '" alt="Other slide" >
                                <div class="carousel-caption" style="background-color: rgba(0, 0, 0, 0.5);padding: 0px;">' . $docBk_desc . '</div>
                            </div>';
    ...
    function save_mApproval_del()
    {
        ...
        $isVerifyDocApproved =  trim($this->input->post('isVerifyDocApproved'));
        $IsActive =  trim($this->input->post('IsActive'));
        $appLeaderId =  trim($this->input->post('appLeaderId'));
        $phone_no =   $this->input->post('phone_no');
        $message =   $this->input->post('description');

        $this->db->select('isVerifyDocApproved');
        $this->db->from('srp_erp_ngo_com_familymaster');
        $this->db->where('companyID', current_companyID());
        $this->db->where('LeaderID', $appLeaderId);
        $this->db->where('isVerifyDocApproved', 1);
        $result = $this->db->get()->row_array();
        if (!empty($result) && $isVerifyDocApproved == 1) {
            return array('e', 'The Family is already approved');
        } else {

            $data['VerifyDocApprovalRemark'] = $message;
            $data['isVerifyDocApproved'] = $this->input->post('isVerifyDocApproved');
            $data['VerifyDocApprovedBy'] = $this->common_data['current_userID'];
            $data['VerifyDocApprovedDate'] = $this->common_data['current_date'];
            $this->db->set('timestamp', current_date(true));
            $this->db->where('LeaderID', $appLeaderId);
            $result = $this->db->update('srp_erp_ngo_com_familymaster', $data);

            $updateComApp = $this->db->query("UPDATE srp_erp_ngo_com_communitymaster SET comVerifiApproved='{$this->input->post('isVerifyDocApproved')}' WHERE companyID={$companyID} AND `Com_MasterID`='{$appLeaderId}'");

            if ($IsActive == 1) {
                $this->load->library('send_sms');
                $this->send_sms->send($phone_no, $message);
            }
            if ($result) {
                return array('s', 'Member Approval Updated successfully');
            } else {
                return array('e', 'Member Approval alternation is Failed');
            }
        }
    }

    //Advertisement Member Approval
    function addDoc_app_detail($advertiseId, $adMemName, $adType_id, $sub_category_name)
    {
        $primaryLanguage = getPrimaryLanguage();
        $this->lang->load('communityngo', $primaryLanguage);
        ...
        $query = $this->db->query("SELECT advertise.id advertiseId,advertise.FamMasterID,comMas.Com_MasterID,comMas.MemberCode,comMas.CName_with_initials,adSub_category.advertisement_category_id AS Gender,comMas.CountryCodePrimary,advertise.mobile,stateAr.Description AS stateArDes ,stateProvince.Description AS provinceAr,
                                    comMas.C_Address, TitleDescription,comMas.EmailID, comMas.CNIC_No,DATE_FORMAT(insert_date, '{$convertFormat}') AS insert_dates,DATE_FORMAT(published_date, '{$convertFormat}') AS published_dates,DATE_FORMAT(expire_date,  '{$convertFormat}') AS expire_dates,ad_category.id,ad_category.category_name,stateProvince.Description, CImage,FamilySystemCode,FamilyName,
                                    adSub_category.sub_category_name,advertise.type_id,advertisementtypes.advertisementType,advertise.is_public,advertise.amount,advertise.upload_url,advertise.title AS upload_title,advertise.description AS upload_description,adApproval_remark,is_approved,advertise.expire_days
                                    ...
                                    WHERE femMas.companyID='" . $companyID . "' AND advertise.id ='" . $advertiseId . "'");
        $row = $query->row();

        if (empty($row)) {
            echo '
                 <div class="well well-sm"  style="position: relative; height: 400px; margin-bottom: 0px;">
                    <span style="text-align:center; color:#CCC; position: absolute; left: 42%;top: 35%;font-size: 14px;">No Data Found<br> <i class="fa fa-camera fa-5x" style="margin-top: 5px;"></i></span>
                </div>';
        } else {
            if ($row->is_public == '1') {
            ...
            if ($row->published_dates == null) {
                $publishedDates = date("Y-m-d");
            } else {
                $publishedDates = $row->published_dates;
            }
            echo '<div class="col-sm-12"><div class="col-md-1"></div><div class="col-md-4">
                                        <div class="thumbnail" style="height:200px;background-color:lightcyan;">
                                                <div class="caption">
                                                    <h5 class="text-center"><b>Advertised By</b></h5>
                                                <p><span>' . $this->lang->line('CommunityNgo_familyName') . ': <b>' . $row->FamilyName . '</b></span></p>
                                                <p><span>' . $this->lang->line('communityngo_TP_Mobile') . ': <b>' . $row->mobileNo . '</b></span></p>
                                                <p><span>' . $this->lang->line('communityngo_Province') . ': <b>' . $row->provinceAr . '</b></span></p>
                                                <p><span>' . $this->lang->line('communityngo_primary_email') . ': <b>' . $row->mailID . '</b></span></p>
            ...
            echo ' <div class="col-md-6">
                                        <div class="thumbnail" style="height:200px;background-color:lavender;">
                                                <div class="caption">
                                                    <h5 class="text-center"><b>Advertisement</b></h5>
                                                 
                                                 <p><span>' . $this->lang->line('common_title') . ': <b>' . $row->upload_title . '</b></span></p>
                                                 <p style="line-height: 0.8em;"><span>' . $this->lang->line('common_description') . ': <b>' . $row->upload_description . '</b></span></p>
                                                 <p style="line-height: 0.8em;"><span>' . $this->lang->line('communityngo_Category') . ': <b>' . $row->categoryStr . '</b></span></p>
                                                <p style="line-height: 0.8em;"><span>' . $this->lang->line('communityngo_sub_category') . ': <b>' . $row->sub_category_name . '</b></span></p>
                                                <p style="line-height: 0.8em;"><span>' . $this->lang->line('CommunityNgo_famAddedDate') . ': <b>' . $row->insert_dates . '</b></span><span style="float: right;">' . $this->lang->line('common_amount') . ' : <b>' . $row->amount . '</b></span></p>
                                                <p style="line-height: 0.8em;"><span>' . $this->lang->line('communityngo_adIs_public') . ': <b>' . $is_publicStatus . '</b></span><span style="float: right;">' . $this->lang->line('communityngo_adExpire_in_days') . ' : <b><span class="badge" style="font-size: 13px;">' . $row->expire_days . '</span></b></span></p>
            ...
            echo ' <div class="col-sm-12" style="margin-top: 10px;">
                            <div class="form-group col-md-2">
                                <label class="title">' . $this->lang->line('common_remarks') . '</label>
                            </div>
                            <div class="form-group col-md-10">
                <span class="input-req" title="Required Field">
                <textarea class="form-control" id="adApproval_remarks" name="adApproval_remarks" rows="3" required>' . $row->adApproval_remark . '</textarea>
                    <span class="input-req-inner"></span>
                </span>
                            </div>
                        </div>
                        <div class="col-sm-12" style="margin-top: 10px;">
                            <div class="form-group col-md-2">
                                    <label class="title">' . $this->lang->line('communityngo_adPublished_date') . '</label>
                            </div>
                            <div class="form-group col-md-10">
                <div class="input-group dateAdpic">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                    <input onchange="$(\'#published_date\').val(this.value);" type="text" name="published_date"
                                           data-inputmask="\'alias\': \'' . $date_format_policy . '\'"
                                           value="' . $publishedDates . '" id="published_date" class="form-control" required>
                                </div>
                            </div>
                        </div>
                        <div  class="col-sm-12">
                            <div class="form-group col-md-2">
                                <label class="title">' . $this->lang->line('communityngo_approval_status') . '</label>
                            </div>
                            <div class="form-group col-md-10">
                              <select class="select2 is_approved" name="is_approved" id="is_approved" style="width: 100%;">';
            echo '<option></option>';
    ...
    function addApprovalBody($advertiseId)
    {

        $companyID = current_companyID();

        $query = $this->db->query('SELECT upload_url,title FROM srp_erp_ngo_com_advertisments advertise INNER JOIN srp_erp_ngo_com_familymaster femMas ON femMas.FamMasterID = advertise.FamMasterID  WHERE femMas.companyID="' . $companyID . '" AND advertise.id ="' . $advertiseId . '"');
        $res = $query->result();
        $a = '0';
        $b = '0';
        echo '<div id="carousel-example-generic" class="carousel slide" data-ride="carousel" style="height: auto;">
        ...
        echo '</ol>
                 <div class="carousel-inner">';
        foreach ($res as $row) {
            $memAddImgs = get_all_community_images($row->upload_url, 'Community/advertisment/', 'communityNoImg');
            $Add_slide = $memAddImgs;
            $docAd_desc = $row->title;

            echo '<div class="item active">
                                <img src="' . $Add_slide . '" alt="First slide">
                                <div class="carousel-caption" style="background-color: rgba(0, 0, 0, 0.5);padding: 0px;">' . $docAd_desc . '</div>
                             </div>';
    ...
    function save_addApproval_del()
    {
        ...
        $IsActive =  trim($this->input->post('IsActive'));
        $advertiseId =  trim($this->input->post('advertiseId'));
        $phoneAd_no =   $this->input->post('phoneAd_no');
        $expire_days =   $this->input->post('expire_days');
        $message =   $this->input->post('adApproval_remarks');
        $published_date =   $this->input->post('published_date');
        $published_dates =  input_format_date($published_date, $date_format_policy);

        $time_original = strtotime($published_dates);
        $time_add      = $time_original + (3600 * 24) * $expire_days; //add expire in days

        $expire_date      = date("Y-m-d", $time_add);

        $this->db->select('is_approved');
        $this->db->from('srp_erp_ngo_com_advertisments');
        $this->db->where('id', $advertiseId);
        $this->db->where('is_approved', 1);
        $result = $this->db->get()->row_array();
        if (!empty($result) && $is_approved == 1) {
            return array('e', 'The Advertisement is already approved');
        } else {

            $data['adApproval_remark'] = $message;
            $data['published_date'] = $published_dates;
            $data['expire_date'] = $expire_date;
            $data['is_approved'] = $this->input->post('is_approved');
            $data['approved_by'] = $this->common_data['current_userID'];
            $this->db->where('id', $advertiseId);
            $result = $this->db->update('srp_erp_ngo_com_advertisments', $data);

            if ($IsActive == 1) {
                $this->load->library('send_sms');
                $this->send_sms->send($phoneAd_no, $message);
            }
            if ($result) {
                return array('s', 'Advertisement Approval Updated successfully');
            } else {
                return array('e', 'Advertisement Approval alternation is Failed');
            }
        }
    }
