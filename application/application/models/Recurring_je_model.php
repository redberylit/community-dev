...
class Recurring_je_model extends ERP_Model
{
    ...
    function fetch_Journal_entry_template_data($RJVMasterAutoId)
    {
        ...
        $this->db->select('*,DATE_FORMAT(RJVStartDate,\''.$convertFormat.'\') AS RJVStartDate,DATE_FORMAT(RJVEndDate,\''.$convertFormat.'\') AS RJVEndDate,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
    ...
    function recurring_journal_entry_confirmation()
    {
        ...
        $approvals_status = $this->approvals->CreateApproval('RJV', $app_data['RJVMasterAutoId'], $app_data['RJVcode'], 'Recurring Journal Voucher', 'srp_erp_recurringjvmaster', 'RJVMasterAutoId',0,$app_data['RJVStartDate']);
