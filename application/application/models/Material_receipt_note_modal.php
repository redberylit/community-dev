...
class Material_receipt_note_modal extends ERP_Model
{
    ...
    function load_material_receipt_header()
    {
        ...
        $this->db->select('*,DATE_FORMAT(receivedDate,\'' . $convertFormat . '\') AS receivedDate');
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,,DATE_FORMAT(receivedDate,\'' . $convertFormat . '\') AS receivedDate,(DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\')) AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        ...
        $this->db->select('srp_erp_materialreceiptdetails.*,srp_erp_itemissuemaster.itemIssueCode');
        $this->db->where('mrnAutoID', $mrnAutoID);
...
")->row_array();
        ...
        $data['totalValue'] = ($data['unitCost'] * ($data['qtyReceived'] / $data['conversionRateUOM']));
        $data['modifiedPCID'] = $this->common_data['current_pc'];
        $data['modifiedUserID'] = $this->common_data['current_userID'];
        $data['modifiedUserName'] = $this->common_data['current_user'];
        $data['modifiedDateTime'] = $this->common_data['current_date'];
    ...
    {
        $this->db->select('srp_erp_materialreceiptdetails.*,srp_erp_itemmaster.isSubitemExist,srp_erp_materialreceiptmaster.wareHouseAutoID,srp_erp_itemissuemaster.itemIssueCode');
        $this->db->from('srp_erp_materialreceiptdetails');
            ...
            if (!empty($Confirmed)) {
                ...
                if ($isProductReference_completed == 0) {
                    ...
                    if ($approvals_status == 1) {
                        ...
                        if($autoApproval==0) {
                            $result = $this->save_material_receipt_approval(0,$app_data['mrnAutoID'], 1, 'Auto Approved');
                            if($result){
        ...
        if ($approvals_status == 1) {
            ...
            for ($i = 0; $i < count($details_arr); $i++) {
                if ($details_arr[$i]['itemCategory'] == 'Inventory' or $details_arr[$i]['itemCategory'] == 'Non Inventory') {
                    ...
                    $item_arr[$i]['companyLocalWacAmount'] = round(((($item['currentStock'] * $item['companyLocalWacAmount']) + ($localUnitCost * $qty)) / $item_arr[$i]['currentStock']), $master['companyLocalCurrencyDecimalPlaces']);
                    $item_arr[$i]['companyReportingWacAmount'] = round(((($item['currentStock'] * $item['companyReportingWacAmount']) + ($reportingUnitCost * $qty)) / $item_arr[$i]['currentStock']), $master['companyReportingCurrencyDecimalPlaces']);
    ...
    function add_sub_itemMaster_tmpTbl($qty = 0, $itemAutoID, $masterID, $detailID, $code = 'MRN', $itemCode = null, $data = array())
    {
    ...
    function edit_sub_itemMaster_tmpTbl($qty = 0, $itemAutoID, $masterID, $detailID, $code = 'MRN', $itemCode = null, $data = array())
    {
	...
	mrm.issueDate,
	mrm.employeeName,
IFNULL(SUM(mrqdetail.qtyReceived),0) as qtyReceived,
IFNULL(SUM(mrqdetail.mrQty),0) as mrQty
FROM
	srp_erp_itemissuemaster mrm
LEFT JOIN (
	SELECT
		mrd.itemIssueDetailID,
		mrd.itemIssueAutoID,
		mrd.qtyReceived,
		sum(
	...
	FROM
	srp_erp_materialreceiptdetails mrd
	LEFT JOIN srp_erp_itemissuedetails ON srp_erp_itemissuedetails.itemIssueDetailID= mrd.itemIssueDetailID
	GROUP BY
		itemIssueDetailID
) mrqdetail ON mrqdetail.itemIssueAutoID = mrm.itemIssueAutoID
WHERE
	mrm.issueDate <= '$receivedDate'
AND mrm.itemType = '$itemType'
AND mrm.issueType = 'Material Request'
AND mrm.requestedWareHouseAutoID = '$wareHouseAutoID'
AND mrm.companyID = '$companyID'
AND mrm.approvedYN = 1
GROUP BY
		mrm.itemIssueAutoID")->result_array();
        return $data;*/
	...
	`srp_erp_itemissuedetails`.*, `det`.`qtyReceived` AS `qtyReceived`,
	`srp_erp_warehouseitems`.`currentStock` AS `stock`
FROM
...
LEFT JOIN `srp_erp_warehouseitems` ON `srp_erp_warehouseitems`.`itemAutoID` = `srp_erp_itemissuedetails`.`itemAutoID` AND `srp_erp_warehouseitems`.`wareHouseAutoID` = $warehouseID
WHERE
    ...
    function save_material_issue_note_base_items()
    {
        ...
        foreach ($itemIssueDetailID as $key => $itemIssueDetailID) {
            ...
            if ($qty[$key] != 0) {
                ...
                $data['totalValue'] = ($data['currentlWacAmount'] * ($data['qtyReceived'] / $data['conversionRateUOM']));
                $data['modifiedPCID'] = $this->common_data['current_pc'];
                $data['modifiedUserID'] = $this->common_data['current_userID'];
                $data['modifiedUserName'] = $this->common_data['current_user'];
                $data['modifiedDateTime'] = $this->common_data['current_date'];
