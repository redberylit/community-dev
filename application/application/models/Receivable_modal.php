...
class Receivable_modal extends ERP_Model
{
    function fetch_credit_note_template_data($creditNoteMasterAutoID)
    {
        ...
        $this->db->select('*,DATE_FORMAT(srp_erp_creditnotemaster.creditNoteDate,\''.$convertFormat.'\') AS creditNoteDate,DATE_FORMAT(srp_erp_creditnotemaster.approvedDate,\''.$convertFormat.' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        ...
        $this->db->select('GLCode,GLDescription,segmentCode,transactionAmount,companyLocalAmount,customerAmount,description,isFromInvoice,invoiceSystemCode');
