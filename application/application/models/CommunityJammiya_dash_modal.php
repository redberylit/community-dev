...
class CommunityJammiya_dash_modal extends ERP_Model
{
    ...
    function communityDashSum_Count()
    {
        ...
        $deleted = " srp_erp_ngo_com_familymaster.isDeleted = '0' ";
        $verifyDocApp = " AND srp_erp_ngo_com_familymaster.isVerifyDocApproved='1'";

        $where = $deleted . $verifyDocApp;
        ...
        $family_count = $this->db->query("SELECT COUNT(*) as familiesCount FROM srp_erp_ngo_com_familymaster INNER JOIN srp_erp_ngo_com_communitymaster ON srp_erp_ngo_com_communitymaster.Com_MasterID=srp_erp_ngo_com_familymaster.LeaderID WHERE (srp_erp_ngo_com_familymaster.isDeleted IS NULL OR srp_erp_ngo_com_familymaster.isDeleted = '' OR srp_erp_ngo_com_familymaster.isDeleted = 0) AND  srp_erp_ngo_com_familymaster.isVerifyDocApproved='1' $where_clsDsh " . " $areaMemIdS  " . " $gsDivitnIdS")->row_array();

        $committee_count = $this->db->query("SELECT COUNT(*) as committeesCount FROM srp_erp_ngo_com_committeesmaster WHERE (isDeleted IS NULL OR isDeleted = '' OR isDeleted = 0) AND isActive='1'")->row_array();
