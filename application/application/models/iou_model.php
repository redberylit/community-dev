...
class iou_model extends ERP_Model
{
    ...
    function iou_Voucher_confirmation()
    {
        ...
        if (empty($results)) {
            ...
            if (!empty($Confirmed)) {
                ...
                $this->db->select('voucherAutoID, documentID,iouCode');
                $this->db->where('voucherAutoID', $iouvoucherid);
                $this->db->from('srp_erp_iouvouchers');
    ...
    function save_iou_catergory()
    {
        ...
        if (($expenceclaimid)) {
            ...
            $isExist = $this->db->query("SELECT expenseClaimCategoriesAutoID FROM srp_erp_expenseclaimcategories WHERE claimcategoriesDescription = '$description' AND expenseClaimCategoriesAutoID != $expenceclaimid And companyID = $companyid")->row('expenseClaimCategoriesAutoID');
            if (isset($isExist)) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    $this->db->trans_rollback();
                    return array('e', 'IOU Category Update ' . $this->db->_error_message());

                } else {
                    $this->db->trans_commit();
                    return array('s', 'IOU Category Updated Successfully.');
                }
            }
        } else {
            ...
            if (isset($isExist)) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    $this->db->trans_rollback();
                    return array('e', 'IOU Category Saved failed ' . $this->db->_error_message());
                } else {
                    $this->db->trans_commit();
                    return array('s', 'IOU Category  saved Successfully.');
                }
            }
        }
    ...
    function load_iou_cat_header()
    {
        ...
        $data = $this->db->query("Select * from srp_erp_expenseclaimcategories where companyID = $comanyid And expenseClaimCategoriesAutoID = $ioucatid ")->row_array();
        ...
        if ($ioumasterbookingautoid) {
            ...
            if (!empty($voucheridexist)) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    $this->db->trans_commit();
                    return array('s', 'IOU Expense  Updated successfully.', $ioumasterbookingautoid);
                }
            }
            ...
            if (!empty($voucheridexist) || $voucheridexist != '') {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    $this->db->trans_commit();
                    return array('s', '' . $data['bookingCode'] . ' IOU Expense created successfully.', $last_id);
...
srp_erp_ioubookingdetails ioubookdet LEFT join(SELECT sum(transactionAmount) as totaltransactionamt,bookingDetailsID from srp_erp_ioubookingdetails WHERE iouVoucherAutoID = $iouVoucherAutoID  AND companyid = $companyid) iouboookdet on  iouboookdet.bookingDetailsID = ioubookdet.bookingDetailsID LEFT JOIN srp_erp_iouvouchers vouchermaster ON ioubookdet.iouVoucherAutoID = vouchermaster.voucherAutoID LEFT JOIN srp_erp_segment segment on segment.segmentID = ioubookdet.segmentID  WHERE ioubookdet.bookingDetailsID = $ioubookigdetailid AND ioubookdet.companyID = $companyid AND ioubookdet.iouVoucherAutoID = $iouVoucherAutoID")->row_array();
    ...
    function fetch_iou_booking($ioubookingid)
    {
        ...
        $data['detail'] = $this->db->query("select *,bookingdet.description AS bookingdescription,bookingdet.categoryDescription, segment.segmentCode,vouchers.iouCode,bookingdet.transactionAmount AS bookingAmount,bookingdet.transactionCurrencyDecimalPlaces AS transactionCurrencyDecimalPlacesbooking 
    ...
    function ioubooking_confirmation()
    {
        ...
        if (empty($results)) {
            ...
            if (!empty($Confirmed)) {
                ...
                $this->load->library('approvals');
                $this->db->select('bookingMasterID, documentID,bookingCode');
                $this->db->where('bookingMasterID', $ioubookingmasterid);
                $this->db->from('srp_erp_ioubookingmaster');
                $app_data = $this->db->get()->row_array();
...
GROUP BY payVoucherAutoId")->row_array();
    ...
    function close_iou_voucher()
    {
        ...
        $detailexist = $this->db->query("select bookingMasterID from srp_erp_ioubookingmaster where iouVoucherAutoID = $voucherid AND companyID = $comapnyid")->row_array();
