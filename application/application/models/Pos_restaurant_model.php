...
class Pos_restaurant_model extends ERP_Model
{
    ...
    function update_menuSalesServiceCharge($menuSalesID)
    {
        ...
        $this->db->query("UPDATE srp_erp_pos_menusalesmaster SET serviceCharge = ( SELECT sum(serviceChargeAmount) FROM srp_erp_pos_menusalesservicecharge WHERE menuSalesID = '" . $menuSalesID . "' AND wareHouseAutoID='" . $outletId . "'   )  , is_sync = 0 WHERE menuSalesID = '" . $menuSalesID . "'");
    ...
    {
        $outletId = get_outletID();
        $q1 = "SELECT IF (netTotal IS NULL, 0, netTotal) + IF (totalTaxAmount IS NULL, 0, totalTaxAmount) + IF (serviceCharge IS NULL, 0, serviceCharge) AS totalTmp, deliveryCommission FROM srp_erp_pos_menusalesmaster WHERE menuSalesID = '" . $menuSalesID . "' AND isDelivery =1 AND wareHouseAutoID='" . $outletId . "' ";
 ...
 FROM srp_erp_pos_menusalesmaster salesMaster JOIN srp_employeesdetails employees ON employees.EIdNo = salesMaster.createdUserID WHERE salesMaster.companyID=" . current_companyID() . " and warehouseAutoID IN($warehouse) GROUP BY salesMaster.createdUserID ";
    ...
    function fetch_pos_customer_details()
    {
        ...
        $data = $this->db->query('SELECT posCustomerAutoID,CustomerName,CustomerAddress1,customerTelephone,CONCAT(customerTelephone, "- ", CustomerName) AS cusdet FROM srp_erp_pos_customermaster WHERE (customerTelephone LIKE "' . $search_string . '" OR CustomerName LIKE "' . $search_string . '") AND companyID="' . $companyID . '"')->result_array();
        if (!empty($data)) {
            foreach ($data as $val) {
                $dataArr[] = array('value' => $val["cusdet"], 'data' => $val['customerTelephone'], 'posCustomerAutoID' => $val["posCustomerAutoID"], 'CustomerName' => $val['CustomerName'], 'CustomerAddress1' => $val['CustomerAddress1'], 'customerTelephone' => $val['customerTelephone']);
            }
