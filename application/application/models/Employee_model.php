...
class Employee_model extends ERP_Model
{
        ...
        if ($imgData[0] != 's' || $imgDataSignature[0] != 's') {
            ...
            if ($this->db->trans_status() == true) {
                ...
                if ($updateID == current_userID() && !empty($_FILES['empImage']['name'])) { //If current users profile image updated
                    ...
                    if (!empty($emp_image) && $this->s3->getMyObjectInfo($emp_image)) {
                        $this->session->empImage = $this->s3->getMyAuthenticatedURL($emp_image, 86400);
                    } else {
                        $emp_image = ($emp_data['Gender'] == 1) ? 'male' : 'female';
                        $this->session->empImage = $this->s3->getMyAuthenticatedURL("images/users/{$emp_image}.png", 86400);
                    }
                }
    ...
    function image_upload_s3($ECode)
    {
        ...
        $s3Upload = $this->s3->putMyObject($input, $fileName);
    ...
    function employee_details($empID = null)
    {
        ...
        $this->db->select('DATE_FORMAT(contractStartDate,\'' . $convertFormat . '\') AS contractStartDate, DATE_FORMAT(contractEndDate,\'' . $convertFormat . '\') AS contractEndDate,contractRefNo,
            EmployeeConType,ECode, EmpTitleId, EmpDesignationId, EmpShortCode, EmpImage,empSignature, Gender, EcMobile, EEmail,DATE_FORMAT(EDOB,\'' . $convertFormat . '\') AS EDOB ,
            ...
            DesDescription, empTB.EIdNo AS thisEmpID, isSystemAdmin, gradeID, medicalInfo');
    ...
    function contactDetails_update()
    {
        ...
        $data = array(
            ...
            'EpTelephone' => $telNo1, 'EcTel' => $telNo2, 'EcMobile' => $emp_mobile,
    ...
    function save_employmentData()
    {
        ...
        $data = array(
            ...
            'pos_barCode' => $pos_barCode, 'EDOJ' => $empDoj, 'DateAssumed' => $dateAssumed, 'gratuityID' => $gratuityID,
    ...
    function save_employmentData_envoy()
    {
        ...
        $data = array(
            ...
            'EmployeeConType' => $employeeConType, 'EPassportNO' => $pass_portNo, 'AirportDestination' => $airport_destination, 'EPassportExpiryDate' => $passPort_expiryDate,
            'EVisaExpiryDate' => $visa_expiryDate, 'EDOJ' => $empDoj, 'DateAssumed' => $dateAssumed, 'gratuityID' => $gratuityID, 'payee_emp_type' => $payee_emp_type,
            'ModifiedUserName' => current_employee(), 'ModifiedPC' => current_pc(), 'Timestamp' => $dateTime
        );
    ...
    function saveDesignation()
    {
        ...
        $isExist = $this->db->query("SELECT DesDescription FROM `srp_designation` WHERE DesDescription IN " . $whereIN . " AND Erp_companyID={$companyID} AND isDeleted=0")->result_array();
    ...
    function delete_empDocument($hiddenID)
    {
        ...
        $this->s3->deleteMyObject($docDetails['FileName']);
    ...
    function save_attendanceData()
    {
        ...
        $data = array(
            ...
            'floorID' => $floorID, 'overTimeGroup' => $overTimeGroup,
    ...
    function save_employeeAsTemp()
    {
        ...
        if ($additionType != 0) { /*If Variable pay*/
            ...
            $declarations = $this->db->query("SELECT empID, detailID, amount FROM srp_erp_variablepaydeclarationdetails                                   
                                  WHERE companyID = {$companyID} AND salaryCategoryID = {$additionType} 
            ...
            if (empty($this->input->post('empHiddenID'))) {
                ...
                if ($this->input->post('isConform') == 1) {
                    if ($additionType == 0) {
                        ...
                        $this->form_validation->set_rules('unit[]', 'No Of Units', 'trim|required');
                    }
                }
                    ...
                    if ($masterUpdate[0] == 's') {
                        ...
                        foreach ($empHiddenID as $empID) {
                            ...
                            if ($additionType == 0) { // not variable pay
                                ...
                                $data[$i]['GLCode'] = $variableGL;
                                $data[$i]['noOfUnits'] = $unit[$i];
                                $data[$i]['categoryID'] = $additionType;
                                $data[$i]['declarationDetID'] = $decDetID[$i];
                                $data[$i]['declarationAmount'] = $decAmount[$i];
                            }
    ...
    function edit_monthAddition($type, $id = 0)
    {
        ...
        if ($type == 'MA' || $type == 'MD') {
            ...
            $query = $this->db->select("{$monthlyCode}, description, confirmedYN, approvedYN, isNonPayroll, isProcessed, {$dateField} {$sysType}")
                ->from($tableName)
                ->where($monthID, $id)
                ->get();
...
//        return false;
    ...
    function get_attendanceData2()
    {
        ...
        $companyID = current_companyID();
        $qry = "SELECT isWeekEndDay,isHoliday,isCheckin,empID,ECode,Ename1,normalTime,normalDay, Ename2,empMachineID, floorDescription, ID, machineID, srp_erp_pay_empattendancereview.floorID, attendanceDate,
                ...
                lateHours, earlyHours, OTHours,mustCheck, weekend, holiday, NDaysOT, weekendOTHours, holidayOTHours,realTime,approvedComment 
                ...
                WHERE attendanceDate BETWEEN '{$frmDate}' AND '{$toDate}' AND srp_erp_pay_empattendancereview.companyID = {$companyID}  AND approvedYN=0 
                AND srp_erp_pay_empattendancereview.floorID IN($floorID) AND confirmedYN = 0 ";
        $data = $this->db->query($qry)->result_array();
    ...
    function save_employeesLeave()
    {
        ...
        if (!empty($_FILES['document_file']['name'])) {
            ...
            $input = $this->s3->inputFile($file['tmp_name']);
            $s3Upload = $this->s3->putMyObject($input, $file_name);
    ...
    {
        $this->db->select("*,srp_erp_family_details.name as name,r.relationship as relationshipDesc,c.Nationality as countryName,
                          g.name as genderDesc, i.description as insuranDesc, cn_mas.CountryDes AS flag_img");
        $this->db->from("srp_erp_family_details");
    ...
    function delete_familydetail()
    {
        ...
        if (!empty($attachments)) {
            foreach ($attachments as $row) {
                $this->s3->deleteMyObject($row['myFileName']);
            }
        }
    ...
    function delete_family_attachment()
    {
        ...
        $this->s3->deleteMyObject($filename);
    ...
    {
        $companyID = current_companyID();
        $detail = $this->db->query("SELECT id, CONCAT(ECode, ' - ',  Ename2) AS empName, lv_type.description, 
    ...
    {
        $companyID = current_companyID();
        $detail = $this->db->query("SELECT id, CONCAT(ECode, ' - ',  Ename2) AS empName, gross_amount, amount, 
    ...
    function update_user_type_usermanagement()
    {
        $companyID = current_companyID();
        $noofusers = $this->db->query("SELECT noOfUsers FROM srp_erp_company WHERE company_id = $companyID")->row_array();
        $currentusercount = $this->db->query("SELECT count(EIdNo) as usercount from srp_employeesdetails where Erp_companyID = $companyID AND userType = 1")->row_array();
