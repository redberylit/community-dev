    ...
    function save_customer()
    {
       ...
       /* $this->db->select('companyGroupID');
        ...
        if (trim($this->input->post('groupCustomerAutoID'))) {
            ...
            $code = (current_companyCode() . '/CUS'. str_pad($number["serialNo"]+1, 6, '0', STR_PAD_LEFT));
            $data['groupcustomerSystemCode'] = $code;
        ...
        foreach($companyid as $key => $val){
            ...
            if($i==0){
                if(empty($linkexsist) && empty($CurrentCOAexsist)){
                    ...
                    $data['customerSystemCode'] = $this->sequence->sequence_generator_group('CUS', 0, $val, $companyCode['company_code']);
