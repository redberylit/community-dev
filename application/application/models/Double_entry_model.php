...
class Double_entry_model extends CI_Model
{
    ...
    function fetch_double_entry_supplier_invoices_data($InvoiceAutoID, $code = null)
    {
           ...
           // echo '<pre>';print_r($detail); echo '</pre>'; die();
            for ($i = 0; $i < count($detail); $i++) {
                if ($detail[$i]['type'] == 'Item') {
                    ...
                    $data_arr['amount_type'] = 'dr';
                    if($disciunt>0){
                        $amount=$detail[$i]['transactionAmount']-($detail[$i]['transactionAmount']/$totalsum['transactionAmount'])*$disciunt;
                    }else{
        ...
        $data_arr['segment'] = $master['segmentCode'];
        $data_arr['gl_dr'] = 0;
        $data_arr['gl_cr'] = $cr_total+$taxttlitm;
        $data_arr['amount_type'] = 'cr';
        $data_arr['isAddon'] = 0;
        $data_arr['taxMasterAutoID'] = null;
        $data_arr['partyVatIdNo'] = null;
    ...
    function fetch_double_entry_credit_note_data($creditNoteMasterAutoID, $code = null)
    {
        ...
        $companyReporting_total = 0;
        $gl_array['gl_detail'] = array();
        $this->db->select('srp_erp_creditnotemaster.*,srp_erp_customermaster.customerName as customerNamemaster');
...
WHERE
    ...
    function fetch_double_entry_customer_invoice_data_insurance($invoiceAutoID, $code = null)
    {
        ...
        $sumamnt = "SELECT sum(transactionAmount) as transactionAmount
FROM
	`srp_erp_customerinvoicedetails`
WHERE
	`invoiceAutoID` = $invoiceAutoID
...
WHERE
    ...
    function fetch_double_entry_delivery_order($orderID){
        ...
        $gl_array = array(); $tax_total = 0; $cr_total = 0;
        $gl_array['gl_detail'] = array();
        $cr_p_arr = array(); $cr_m_arr = array();
        $e_cr_p_arr = array(); $e_cr_m_arr = array();
        $item_arr = array(); $item_tax_arr = array();
        ...
        $item_tax_detail = $this->db->query("SELECT *, SUM(taxAmount * requestedQty) AS tottaxAmount FROM srp_erp_deliveryorderdetails
    ...
    /*Sales Return Buyback Double Entry - Done By Safeena Ansar*/
    function fetch_double_entry_sales_return_buyback_data($salesReturnAutoID, $documentID) /*, $code = null*/
    {
        ...
        $GLEntry_revenue = $this->db->query("SELECT
                                    ...
                                    detailTbl.revenueGLType AS GLType,
                                    0 AS credit,
                                    ABS(SUM( detailTbl.return_Qty * ((invoiceDetail.totalAfterTax / invoiceDetail.requestedQty) + detailTbl.salesPrice) ) )  as debit,
                                    ABS(SUM( detailTbl.return_Qty * ((invoiceDetail.totalAfterTax / invoiceDetail.requestedQty) + detailTbl.salesPrice) ) )   AS transactionAmount,
                                    masterTbl.transactionCurrencyDecimalPlaces as transactionDecimal,
                                    masterTbl.approvedYN as approvedYN,
                                detailTbl.salesReturnAutoID as auto_id
                                FROM srp_erp_salesreturnmaster masterTbl
        ...
        $GLEntry_receivable = $this->db->query("SELECT
                                    ...
                                    detailTbl.segmentCode AS segmentCode,
                                    coa.subCategory AS GLType,
                                    ABS(SUM(detailTbl.return_Qty * ((invoiceDetail.totalAfterTax / invoiceDetail.requestedQty) + detailTbl.salesPrice) )) * -1  AS credit,
                                    0 AS debit,
                                    ABS(SUM(detailTbl.return_Qty * ((invoiceDetail.totalAfterTax / invoiceDetail.requestedQty) + detailTbl.salesPrice) ) ) * -1  AS transactionAmount,
                                    masterTbl.transactionCurrencyDecimalPlaces as transactionDecimal,
                                    masterTbl.approvedYN as approvedYN,
                                detailTbl.salesReturnAutoID as auto_id
                                FROM srp_erp_salesreturnmaster masterTbl
    ...
    /*Receipt Voucher SUOM Double Entry - Done By Safeena Ansar*/
