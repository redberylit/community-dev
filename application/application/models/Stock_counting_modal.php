...
class Stock_counting_modal extends ERP_Model
{
    ...
    function save_stock_counting_detail_multiple()
    {
        ...
        foreach ($itemAutoIDs as $key => $itemAutoID) {
            ...
            $dtl = $this->db->query("SELECT currentStock FROM srp_erp_warehouseitems WHERE wareHouseAutoID='{$stockadjustmentMaster['wareHouseAutoID']}' and itemAutoID='{$itemAutoID}'")->row_array(); //get warehouse stock of the item by location
            $previousWareHouseStock = $this->db->query('SELECT IFNULL(SUM(transactionQTY/convertionRate),0) as currentStock FROM `srp_erp_itemledger` where wareHouseAutoID="' . $stockadjustmentMaster['wareHouseAutoID'] . '" AND itemAutoID="' . $itemAutoID . '" ')->row_array();
            ...
            $data['previousStock'] = isset($item_data['currentStock']) && !empty($item_data['currentStock']) ? $item_data['currentStock'] : 0;
            ...
            $data['previousWareHouseStock'] = isset($dtl['currentStock']) && !empty($dtl['currentStock']) ? $dtl['currentStock'] : 0;
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(stockCountingDate,\'' . $convertFormat . '\') AS stockCountingDate,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
            ...
            if (!empty($Confirmed)) {
                ...
                if ($isProductReference_completed == 0) {
                    ...
                    if ($validate) {
                        ...
                        $this->db->select('stockCountingAutoID, stockCountingCode,stockCountingDate');
