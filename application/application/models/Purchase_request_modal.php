...
class Purchase_request_modal extends ERP_Model
{
    ...
    function fetch_pqr_detail_table()
    {
        ...
        $this->db->select('*,DATE_FORMAT(expectedDeliveryDate,\'' . $convertFormat . '\') AS expectedDeliveryDate,CONCAT_WS(\' - Part No : \',IF(LENGTH(srp_erp_purchaserequestdetails.itemDescription),srp_erp_purchaserequestdetails.itemDescription,NULL),IF(LENGTH(srp_erp_itemmaster.partNo),srp_erp_itemmaster.partNo,NULL))as Itemdescriptionpartno');
        $this->db->where('purchaseRequestID', trim($this->input->post('purchaseRequestID')));
        $this->db->from('srp_erp_purchaserequestdetails');
    ...
    {
        $convertFormat = convert_date_format_sql();
        $this->db->select('purchaseRequestID,createdUserName,transactionCurrency,transactionCurrencyDecimalPlaces,purchaseRequestCode, DATE_FORMAT(createdDateTime,\'' . $convertFormat . '\') AS createdDateTime,referenceNumber,requestedByName,narration,DATE_FORMAT(expectedDeliveryDate,\'' . $convertFormat . '\') AS expectedDeliveryDate,confirmedByName,confirmedYN,DATE_FORMAT(confirmedDate,\'' . $convertFormat . '\') AS confirmedDate,approvedbyEmpID,approvedbyEmpName,approvedYN,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,DATE_FORMAT(documentDate,\'' . $convertFormat . '\') AS documentDate,segmentCode,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
        ...
        $this->db->select('srp_erp_itemmaster.itemSystemCode,srp_erp_itemmaster.itemDescription,unitOfMeasure,requestedQty,unitAmount,discountAmount,comment, totalAmount, discountPercentage,DATE_FORMAT(expectedDeliveryDate,\'' . $convertFormat . '\') AS expectedDeliveryDate,CONCAT_WS(\'- \',IF(LENGTH(srp_erp_purchaserequestdetails.comment),srp_erp_purchaserequestdetails.comment,NULL),\' Part No : \',IF(LENGTH(srp_erp_itemmaster.partNo),srp_erp_itemmaster.partNo,NULL))as Itemdescriptionpartno');
        $this->db->where('purchaseRequestID', $purchaseRequestID);
        $this->db->join('srp_erp_itemmaster','srp_erp_itemmaster.itemAutoID = srp_erp_purchaserequestdetails.itemAutoID');
        $this->db->from('srp_erp_purchaserequestdetails');
...
AND period.isActive = 1")->row_array();
                ...
                if($autoApproval==0){
                    ...
                    $approvals_status = $this->approvals->CreateApproval('PRQ', $po_data['purchaseRequestID'], $po_data['purchaseRequestCode'], 'Purchase Request', 'srp_erp_purchaserequestmaster', 'purchaseRequestID',0,$po_data['documentDate']);
                }else{
