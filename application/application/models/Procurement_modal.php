...
class Procurement_modal extends ERP_Model
{
    ...
    function fetch_po_detail_table()
    {
        ...
        $this->db->select('srp_erp_purchaseorderdetails.*,	CONCAT_WS(\' - Part No : \',IF ( LENGTH( srp_erp_purchaseorderdetails.`itemDescription` ), `srp_erp_purchaseorderdetails`.`itemDescription`, NULL ),IF( LENGTH( srp_erp_itemmaster.partNo ), `srp_erp_itemmaster`.`partNo`, NULL )) AS Itemdescriptionpartno ');
        $this->db->where('purchaseOrderID', trim($this->input->post('purchaseOrderID')));
        $this->db->from('srp_erp_purchaseorderdetails');
                ...
                if ($approvals_status == 1) {
                    ...
                    if($autoApproval==0) {
                        $data = array(
                            ...
                            'transactionAmount' => round($po_total-$discountVal, $po_data['transactionCurrencyDecimalPlaces']),
                            'companyLocalAmount' => round((($po_total-$discountVal) / $po_data['companyLocalExchangeRate']), $po_data['companyLocalCurrencyDecimalPlaces']),
                            'companyReportingAmount' => round((($po_total-$discountVal) / $po_data['companyReportingExchangeRate']), $po_data['companyReportingCurrencyDecimalPlaces']),
                            'supplierCurrencyAmount' => round((($po_total-$discountVal) / $po_data['supplierCurrencyExchangeRate']), $po_data['supplierCurrencyDecimalPlaces']),
                            'isReceived' => 0,
                        );
                        $this->db->where('purchaseOrderID', trim($this->input->post('purchaseOrderID')));
                        $this->db->update('srp_erp_purchaseordermaster', $data);
