...
if (!function_exists('editDelFunction')) {
    function editDelFunction($id, $confirmedYN = 0, $templateDescription)
    {
        $view = '';
        $edit = '';
        $delete = '';
        ...
        if ($confirmedYN != 1) {
            $edit = '<a onclick="templateLoad(' . $id . ', 1 )"><span title="Edit" rel="tooltip" class="glyphicon glyphicon-pencil"></span></a>';
            $delete = '&nbsp;&nbsp; | &nbsp;&nbsp;<a onclick="templateDelete(' . $id . ')"><span title="Delete" rel="tooltip" class="glyphicon glyphicon-trash" style="color:rgb(209, 91, 71);"></span></a>';
        } else if ($confirmedYN == 1) {
            $referBack = '&nbsp;<a onclick="referBackConformation(' . $id . ')"><span style="color:rgb(209, 91, 71);" title="Refer Back" rel="tooltip" class="glyphicon glyphicon-repeat"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;';
            $view = '<a onclick="templateLoad(' . $id . ', 0 )"><i title="View" rel="tooltip" class="fa fa-fw fa-eye"></i></a>';
            $clone = ' | <a onclick="templateClone(' . $id . ', \''.$templateDescription.'\')"><i title="Clone" rel="tooltip" class="fa fa-copy" aria-hidden="true"></i></a>';
        }

        return '<span class="pull-right">' . $referBack . '' . $view . ''.$clone.'' . $edit . '' . $delete . ' </span>';
    }
}
...
if (!function_exists('paySheetAction')) {
    function paySheetAction($id, $confirmedYN, $approvedYN, $year, $month, $isNonPayroll, $templateID)
    {
        ...
        if ($confirmedYN != 1) {
            $edit = ' | <a onclick="payroll_details(' . $fType2 . ', ' . $id . ')"><span title="Edit" rel="tooltip" class="glyphicon glyphicon-pencil"></span></a>';
            $delete = '&nbsp; | &nbsp;<a onclick="payroll_delete(' . $id . ')"><span title="Delete" rel="tooltip" class="glyphicon glyphicon-trash" style="color:rgb(209, 91, 71);"></span></a>';
        }
        if ($confirmedYN == 1 && $approvedYN != 1) {
            $referBack = '<a onclick="referBackConformation(' . $id . ' ,' . $det . ')"><span style="color:rgb(209, 91, 71);" title="Refer Back" rel="tooltip" class="glyphicon glyphicon-repeat"></span></a>&nbsp; | ';
        }

        $view = '<a onclick="payroll_details(' . $fType1 . ', ' . $id . ')"><i title="View" rel="tooltip" class="fa fa-fw fa-eye"></i></a>';
       /* if( $isNonPayroll == 'Y' ){
            $templateID = getPolicyValues('NPT', 'SPN');
        }else{ $templateID = getPolicyValues('PT', 'SP'); }*/
        ...
        $print = ' | <a target="_blank" href="'.site_url('template_paySheet/paySheet_print/').'/'.$id.'/'.$templateID.'/'.$isNonPayroll.'/'.$det2.'" >';
...
if (!function_exists('actionBankProcess')) {
    function actionBankProcess($bankTransferID, $confirmedYN = 0, $documentCode, $isNonPayroll)
    {
        $view = '';
        $edit = '';
        $delete = '';
        ...
        $companyID = current_companyID();
        $CI =& get_instance();
        $CI->db->select("bankTransferID,payVoucherAutoId,PVcode")
            ->from('srp_erp_paymentvouchermaster')
            ->where('companyID', $companyID)
            ->where('bankTransferID', $bankTransferID);
        $paymentvoucher = $CI->db->get()->result_array();
            ...
            if(!empty($paymentvoucher)){
                if(count($paymentvoucher)>1){
                    ...
                    $CI->db->select("bankTransferID,payVoucherAutoId,PVcode")
                        ...
                        ->where('companyID', $companyID)
                        ->where('bankTransferID', $bankTransferID);
                    $PVcode = $CI->db->get()->row_array();

                    $view .= ' <a target="_blank" style="cursor: pointer;" onclick="documentPageView_modal(\'PV\',' . $PVcode['payVoucherAutoId'] . ')" >';
                    $view .= '<i class="fa fa-file"></i></a> &nbsp;&nbsp; | &nbsp;&nbsp;';
                }
            }
...
if (!function_exists('formulaBuilder_to_sql')) {
    function formulaBuilder_to_sql($ssoRow = array(), $salary_categories_arr = array(), $payDateMin, $payGroupID, $getBalancePay='Y')
    {
        $CI =& get_instance();
        $companyID = current_companyID();
                ...
                if (in_array($formula_row, $operand_arr)) { //validate is a operand
                    ...
                    if ($elementType == '_') {
                        ...
                        if (trim($monthlyADArr[1]) == '0') {
                            ...
                            /*** Monthly Addition or Monthly Deduction ***/
                            $formulaText .= 'Fixed Payment | Gross Salary';
                            $MD_MD_Description = $monthlyADArr[1] . '_' . $n;

                            $formulaDecode_arr[] = 'SUM(' . $MD_MD_Description . ')';
                            $salaryCatID[$n]['cat'] = $monthlyADArr[1];
                            $salaryCatID[$n]['description'] = 'Fixed Payment | Gross Salary';
...
if (!function_exists('getNoPaySystemTableRecords')) {
    function getNoPaySystemTableRecords()
    {
        ...
        $result = $CI->db->query("SELECT *, masterTB.id AS masterID,srp_erp_pay_salarycategories.salaryDescription as salaryDescription, fromulaTB.id AS formulaTBID
                                  ...
                                  AND fromulaTB.companyID={$companyID}")->result_array();
