...
class Inventory extends ERP_Controller
{
    ...
    function fetch_stock_adjustment_table()
    {
        ...
        $where = "companyID = " . $companyid . $location_filter . $date . $status_filter . "";
        $this->datatables->select("stockAdjustmentAutoID,confirmedYN,approvedYN,createdUserID,stockAdjustmentCode,comment,DATE_FORMAT(stockAdjustmentDate,'$convertFormat') AS stockAdjustmentDate ,wareHouseCode,wareHouseLocation,wareHouseDescription,isDeleted,confirmedByEmpID");
        $this->datatables->where($where);
    ...
    function fetch_stock_return_table()
    {
        ...
        $this->datatables->select("stockReturnAutoID,confirmedYN,approvedYN,createdUserID,stockReturnCode,comment,DATE_FORMAT(returnDate,'$convertFormat') as returnDate,wareHouseCode,wareHouseLocation,transactionCurrency,wareHouseDescription,supplierName,isDeleted,confirmedByEmpID");
        $this->datatables->where($where);
    ...
    function fetch_material_issue_approval_mc()
    {
        ...
        if($approvedYN == 0)
        {
            ...
            //$this->datatables->add_column('total_value', '<div class="pull-right"><b>Issued : </b> $1 </div>', 'tot_value');
            $this->datatables->edit_column('total_value', '<div class="pull-right"><b>$2 : </b> $1 </div>', 'number_format(tot_value,companyLocalCurrencyDecimalPlaces),companyLocalCurrency');
            $this->datatables->add_column('itemIssueCode', '$1', 'approval_change_modal_buyback(itemIssueCode,itemIssueAutoID,documentApprovedID,approvalLevelID,approvedYN,MI,0,mc)');
            $this->datatables->add_column('detail', '$1 - $2 ( $3 )', 'wareHouseDescription,wareHouseLocation,wareHouseCode');
            $this->datatables->add_column('confirmed', "<center>Level $1</center>", 'approvalLevelID');
            $this->datatables->add_column('approved', '$1', 'document_approval_drilldown(approvedYN, "MI", itemIssueAutoID)');
    ...
    function fetch_stock_adjustment_approval_buyback()
    {
        ...
        * */
        ...
        if($approvedYN == 0)
        {
            ...
            $this->datatables->where('srp_erp_stockadjustmentmaster.companyID', $companyID);
            $this->datatables->where('srp_erp_approvalusers.companyID', $companyID);
            $this->datatables->add_column('stockAdjustmentCode', '$1', 'approval_change_modal_buyback(stockAdjustmentCode,stockAdjustmentAutoID,documentApprovedID,approvalLevelID,approvedYN,SA,0,buy)');
            $this->datatables->add_column('detail', '$1 - $2 ( $3 )', 'wareHouseDescription,wareHouseLocation,wareHouseCode');
            $this->datatables->add_column('confirmed', "<center>Level $1</center>", 'approvalLevelID');
            $this->datatables->add_column('approved', '$1', 'document_approval_drilldown(approvedYN, "SA", stockAdjustmentAutoID)');
    ...
    function fetch_stock_transfer_approval_buyback()
    {
        ...
        if($approvedYN == 0)
        {
            ...
            $this->datatables->where('srp_erp_stocktransfermaster.companyID', $companyID);
            $this->datatables->where('srp_erp_approvalusers.companyID', $companyID);
            $this->datatables->add_column('stockTransferCode', '$1', 'approval_change_modal_buyback(stockTransferCode,stockTransferAutoID,documentApprovedID,approvalLevelID,approvedYN,ST,0,buy)');
            $this->datatables->add_column('detail', '<b>From : </b> $1 - $2 - $3 | <b> To : </b> $4 - $5 - $6', 'form_wareHouseCode, form_wareHouseLocation, form_wareHouseDescription, to_wareHouseCode ,to_wareHouseLocation ,to_wareHouseDescription');
            //$this->datatables->add_column('detail', '$1 - $2 ( $3 )', 'wareHouseDescription,wareHouseLocation,wareHouseCode');
            $this->datatables->add_column('confirmed', "<center>Level $1</center>", 'approvalLevelID');
            $this->datatables->add_column('approved', '$1', 'document_approval_drilldown(approvedYN, "ST", stockTransferAutoID)');
    ...
    function fetch_sales_return_table()
    {
        ...
        $this->datatables->select("salesReturnAutoID,confirmedYN,approvedYN,createdUserID,salesReturnCode,comment,DATE_FORMAT(returnDate,'$convertFormat') as returnDate,wareHouseCode,wareHouseLocation,transactionCurrency,wareHouseDescription,customerName,isDeleted,confirmedByEmpID");
        $this->datatables->where($where);
    ...
    function fetch_material_request()
    {
        ...
        $where = "companyID = " . $companyid . $location_filter . $date . $status_filter . "";
        $this->datatables->select("srp_erp_materialrequest.mrAutoID as mrAutoID,companyCode,MRCode,comment,employeeName,DATE_FORMAT(confirmedDate,'%Y-%m-%d') AS confirmedDate,confirmedYN,approvedYN,createdUserID,wareHouseDescription,DATE_FORMAT(requestedDate,'.$convertFormat.') AS requestedDate,itemType,det.totalValue as tot_value,isDeleted,companyLocalCurrencyDecimalPlaces,companyLocalCurrency,srp_erp_materialrequest.confirmedByEmpID as confirmedByEmp");
        ...
        $this->datatables->add_column('edit', '$1', 'load_material_request_action(mrAutoID,confirmedYN,approvedYN,createdUserID,isDeleted,confirmedByEmp)');
        $this->datatables->edit_column('DT_RowClass', '$1', 'set_deleted_class(isDeleted)');
        echo $this->datatables->generate();
    }
    ...
    function save_stock_transfer_detail_edit_all_multiple_buyback()
    {
        ...
        foreach ($searches as $key => $search) {
            ...
            $this->form_validation->set_rules("noOfItems[{$key}]", 'No Of Item', 'trim|required');
            $this->form_validation->set_rules("grossQty[{$key}]", 'Gross Qty', 'trim|required');
    ...
    function fetch_sales_return_approval_buyback()
    {
        ...
        * */
        ...
        $approvedYN = trim($this->input->post('approvedYN'));
        $this->datatables->select('masterTbl.salesReturnAutoID as masterAutoID, salesReturnCode as documentCode, `comment` as narration,srp_erp_customermaster.customerName as customerName, confirmedYN, srp_erp_documentapproved.approvedYN as approvedYN, documentApprovedID, approvalLevelID, DATE_FORMAT(returnDate,\'' . $convertFormat . '\') AS documentDate, det.totalValue as total_value, ROUND(det.totalValue, 2) as total_value_search,transactionCurrencyDecimalPlaces, transactionCurrency', false);
        $this->datatables->join('(SELECT SUM(return_Qty * ((invoiceDetail.totalAfterTax / invoiceDetail.requestedQty) + salesPrice)) as totalValue,salesReturnAutoID
         FROM srp_erp_salesreturndetails detailTbl
         LEFT JOIN srp_erp_customerinvoicedetails invoiceDetail ON invoiceDetail.invoiceAutoID = detailTbl.invoiceAutoID AND invoiceDetail.itemAutoID = detailTbl.itemAutoID
          GROUP BY salesReturnAutoID) det', '(det.salesReturnAutoID = masterTbl.salesReturnAutoID)', 'left');
        $this->datatables->from('srp_erp_salesreturnmaster masterTbl');
        $this->datatables->join('srp_erp_customermaster', 'srp_erp_customermaster.customerAutoID = masterTbl.customerID', 'left');
        $this->datatables->join('srp_erp_documentapproved', 'srp_erp_documentapproved.documentSystemCode = masterTbl.salesReturnAutoID AND srp_erp_documentapproved.approvalLevelID = masterTbl.currentLevelNo');
        $this->datatables->join('srp_erp_approvalusers', 'srp_erp_approvalusers.levelNo = masterTbl.currentLevelNo');
