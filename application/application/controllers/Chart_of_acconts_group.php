...
class Chart_of_acconts_group extends ERP_Controller
{
    ...
    function load_master_ofAccount()
    {
        ...
        $details = $this->db->query("SELECT srp_erp_groupchartofaccounts.* FROM srp_erp_groupchartofaccounts  WHERE srp_erp_groupchartofaccounts.groupID = {$Grpid} GROUP BY srp_erp_groupchartofaccounts.GLAutoID")->result_array();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->input->post('controlAccountUpdate')==0) {
                if ($this->input->post('GLAutoID')) {
                 ...
                 $subAccountExist= $this->db->query("SELECT * FROM `srp_erp_chartofaccounts` WHERE masterAutoID = (SELECT GLAutoID FROM `srp_erp_groupchartofaccounts` WHERE `GLAutoID` = '{$GLAutoID}' AND `masterAccountYN` = '1') limit 1")->row_array();
