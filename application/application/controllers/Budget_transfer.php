...
class Budget_transfer extends ERP_Controller
{
    ...
    function save_budget_transfer_header()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['createdDate'] = current_date(false);
    ...
    function fetch_budget_transfer_detail(){
        ...
        $this->datatables->edit_column('total_value', '<div class="pull-right"> $1 </div>', 'number_format(transferAmount,'.$company_reporting_DecimalPlaces.')');
...
AND confirmedYN = 1')->row_array();
    ...
    function fetch_budget_transfer_approval(){
        ...
        $convertFormat = convert_date_format_sql();
        $this->datatables->select("budgetTransferAutoID,srp_erp_budgettransfer.documentSystemCode as documentSystemCode,DATE_FORMAT(srp_erp_budgettransfer.createdDate,'$convertFormat') AS createdDate,srp_erp_budgettransfer.comments as comments,srp_erp_budgettransfer.confirmedYN as confirmedYN,srp_erp_budgettransfer.approvedYN as approvedYN,CONCAT(srp_erp_companyfinanceyear.beginingDate,' - ',srp_erp_companyfinanceyear.endingDate) AS financeYear,srp_erp_budgettransfer.createdUserID as createdUserID,srp_erp_documentapproved.approvalLevelID as approvalLevelID");
        $this->datatables->from('srp_erp_budgettransfer');
        $this->datatables->join('srp_erp_companyfinanceyear ', 'srp_erp_companyfinanceyear.companyFinanceYearID = srp_erp_budgettransfer.financeYearID');
        $this->datatables->join('srp_erp_documentapproved', 'srp_erp_documentapproved.documentSystemCode = srp_erp_budgettransfer.budgetTransferAutoID AND srp_erp_documentapproved.documentID="BDT"');
        $this->datatables->where('srp_erp_budgettransfer.companyID', current_companyID());
        $this->datatables->where('srp_erp_budgettransfer.approvedYN', $approvedYN);
        $this->datatables->add_column('approved', '$1', 'confirm_aproval_BDT(approvedYN,confirmedYN,"BDT",budgetTransferAutoID)');
    ...
    function get_budget_master(){
        $convertFormat = convert_date_format_sql();
        $budgetTransferAutoID=$this->input->post('budgetTransferAutoID');
        $this->db->select('documentSystemCode,srp_erp_budgettransfer.comments as comments,srp_erp_companyfinanceyear.beginingDate as beginingDate,srp_erp_companyfinanceyear.endingDate as endingDate,DATE_FORMAT(srp_erp_budgettransfer.createdDate,\''.$convertFormat.'\') AS createdDate');
        $this->db->where('budgetTransferAutoID', trim($budgetTransferAutoID));
