...
class Fund_management extends ERP_Controller
{
    ...
    public function fetch_investment()
    {
        ...
        $this->datatables->select('id, documentCode, invTypeID, invDes, currencyID, CurrencyCode, invDate, DecimalPlaces, trAmount, disburseAmount, narration, company_name', false)
            ->from("({$query}) t1")
    ...
    function finance_template_view(){
        ...
        $templateID = $masterData['templateID'];
        $dPlace = $masterData['trCurrencyDPlace'];
        echo load_fm_template($templateID, $fin_ID, $dPlace);
    }
    ...
    function update_financialData(){
        ...
        if(!empty($recordExists)){
            $amount_type = 'cr';
            if($recordExists['GLType'] == 'PLI'){ //Income
                $amount_type = ($amount > 0)? $amount_type: 'dr';
                $amount = $amount * -1;
            }
            else{ //Expense (PLE)
                $amount_type = ($amount > 0)? 'cr': 'dr';
                $amount = $amount * -1;
            }
            ...
            $amount_type = 'cr';
            if($accountData['subCategory'] == 'PLI'){ //Income
                $amount_type = ($amount > 0)? $amount_type: 'dr';
                $amount = $amount * -1;
            }
            else{ //Expense (PLE)
        ...
        if($this->db->trans_status() == true){
            ...
            foreach ($group_glData as $key=>$item){
                ...
                $amount = $this->db->query("SELECT  FORMAT( SUM(IFNULL(transactionAmount,0) * -1), transactionCurrencyDecimalPlaces) trAmount
    ...
    function finance_submission_print(){
        ...
        $tBody = load_fm_template($templateID, $fin_ID, $dPlace, $print=1);
        $data['tBody'] = $tBody;
