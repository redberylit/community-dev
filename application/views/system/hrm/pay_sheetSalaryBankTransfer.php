...
<script>
    ...
    function getTotalCheckedValue(){
        var tot = 0;
        $('.transCheck:checked').each(function(){
            //console.log( parseFloat($(this).attr('data-local-amount')) );
    ...
    function validate_wps(bankTransID){
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {'isValidate' : 'Y'},
            url: "<?php echo site_url('Template_paysheet/WPS/'); ?>"+bankTransID,
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                ...
                if( data[0] == 's'){
                    var url = "<?=site_url('Template_paysheet/WPS/')?>"+bankTransID;
