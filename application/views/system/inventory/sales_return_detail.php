            ...
            success: function (data) {
                ...
                if (!jQuery.isEmptyObject(data)) {
                    ...
                    $.each(data, function (val, text) {
                        ...
                        $('#item_cinv_table_body').append('<tr><td>' + x + '</td><td>' + text['invoiceCode'] + ' - ' + text['transactionCurrency'] + ' - ' + text['invoiceDate'] + '</td><td>' + text['itemSystemCode'] + ' - ' + text['itemDescription'] + '</td><td class="text-right">' + text['unitOfMeasure'] + ' ' + parseFloat(text['transactionAmount']).formatMoney(text['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td>' + text['requestedQty'] + '</td><td class="text-center"><input type="text" class="number" size="15" id="qty_' + text['invoiceDetailsAutoID'] + '" onkeyup="select_check_box(this,' + text['invoiceDetailsAutoID'] + ',' + text['requestedQty'] + ')"></td><td class="text-right" style="display: none;"><input class="checkbox" id="check_' + text['invoiceDetailsAutoID'] + '" type="checkbox" value="' + text['invoiceDetailsAutoID'] + '"></td></tr>');
                        x++;
                    });
                } else {
                    $('#item_cinv_table_body').append('<tr class="danger"><td colspan="6" class="text-center"><b><?php echo $this->lang->line('common_no_records_found');?></b></td></tr>');
                }
                /*No Records Found*/
            }, error: function () {
            ...
            success: function (data) {
                ...
                if (jQuery.isEmptyObject(data['detail'])) {
                    ...
                    $.each(data['detail'], function (key, value) {
                        ...
                        var row = '<tr><td>' + x + '</td><td>' + value['itemSystemCode'] + '</td><td>' + value['itemDescription'] + '</td><td>' + value['unitOfMeasure'] + '</td><td class="text-center">' + value['salesPrice'] + '</td><td class="text-center">' + value['return_Qty'] + '</td><td class="text-right">' + parseFloat(value['totalValue']).formatMoney(value['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td class="text-right"><a onclick="delete_item(' + value['salesReturnDetailsID'] + ');"><span style="color:rgb(209, 91, 71);" class="glyphicon glyphicon-trash"></span></a></td></tr>';
                    ...
                    $('#invoice_table_tfoot').html('<tr> <td class="text-right" colspan="6"> Total</td><td class="text-right total">' + total.formatMoney(currency_decimal, '.', ',') + '</td><td></td></tr>')
    ...
    function save_sales_return_detail() {
        ...
        if (!jQuery.isEmptyObject(selected)) {
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data: {'invoiceDetailsAutoID': selected, 'salesReturnAutoID': salesReturnAutoID, 'qty': qty},
