...
<div class="box box-danger">
    ...
    <div class="box-body" style="display: block;width: 100%">
        <div id="performanceSummary<?php echo $userDashboardID; ?>" style="height: 250px"></div>
    </div>
...
if (!empty($performanceSummary)) {
    foreach ($performanceSummary as $val) {
        ...
        if ($val["description"] == 'Revenue') {
            $color = '#3C8DBC';
            $revenueArr[] = "{name:'" . $val["description"] . "',y:100,x:" . abs($val["amount"]) . ",color: '$color'}";
        }
        ...
        if ($val["description"] != 'Revenue') {
            $y=0;
            if($totalRevenue) {
                $y = (abs($val["amount"]) / $totalRevenue) * 100;
            }
            $performanceSummaryArr[] = "{name:'" . $val["description"] . "',y:" . round($y, 2) . ",x:" . abs($val["amount"]) . ",color: '$color'}";
        }
    }
}
if ($amount == 0) {
    ...
    $(function () {
        chartPerformanceSummary<?php echo $userDashboardID; ?> = Highcharts.chart('performanceSummary<?php echo $userDashboardID; ?>', {
            ...
            series: [{
                ...
                name: 'Types',
                colorByPoint: true,
                data: [<?php echo join($performanceSummaryArr, ",") ?>], center: [300, 100],
                size: 150,
                innerSize: 105,
                depth: 45,
                //showInLegend: false
