                    ...
                    <div class="row">
                        <table id="skipTB" class="<?php echo table_class(); ?>" align="center">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th><?php echo $this->lang->line('hrms_loan_int_no');?><!--Int.No--></th>
                                    <th><?php echo $this->lang->line('hrms_loan_schedule_date');?><!--Schedule Date--></th>
                                    <th width="124px"><?php echo $this->lang->line('hrms_loan_new_date');?><!--New Date--></th>
                                </tr>
                            </thead>
                            <tbody>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success :function(data){
                ...
                $.each(data, function(elm, val){
                    ...
                    $('#loanSetupDet').append( '<input type="hidden" class="'+setupDet+'" value="'+isIntBase+'" data-int="'+intPercent+'"/>' );
                });
            },
            error: function () {
                stopLoad();
    ...
    $('#loanTypeID').change(function() {
        ...
        //alert(loanID +'|'+ isIntBase +' | '+ '.loanTypCls_'+loanID);
        if(isIntBase == 1){
    ...
    function loadSkipProcess(scheduleDet){
        ...
        $('.skipCheckbox:checked').each(function(){
            ...
            var skipDate = '<div class="input-group "><div class="input-group-addon myInputGroup"><i class="fa fa-calendar myFa-calender"></i></div>';
            ...
            skipDet += '<tr><td>'+j+'</td><td>'+intNo+'</td><td align="center">'+convertDate+' </td><td>'+skipDate+' '+skipHidden+'</td></tr>';
            j++;
    ...
    $(document).on("focus", ".dynamic_datePicker", function(){
        $(this).datepicker({
            ...
            if( newDate < nextDate  ){
                ...
                //console.log('month.length:'+ month.toString().length +' ||'+  monthLeadZero +' // date.length:'+ date.toString().length +' ||'+  dateLeadZero);
