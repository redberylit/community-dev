...
<div class="m-b-md" id="wizardControl">
    <a class="btn btn-primary" href="#step1" data-toggle="tab">
        <?php echo $this->lang->line('profile_Step1_expense_claim_header'); ?><!--Step 1 - Expense Claim Header--></a>
    <a class="btn btn-default btn-wizard" href="#step2" onclick="fetch_EC_detail_table()" data-toggle="tab">
        <?php echo $this->lang->line('profile_Step2_expense_claim_detail'); ?><!--Step 2 - Expense Claim Detail-->
    </a>
    <a class="btn btn-default btn-wizard" href="#step3" onclick="load_conformation();" data-toggle="tab">
        <?php echo $this->lang->line('profile_Step3_expense_claim_confirmation'); ?><!--Step 3 - Expense Claim Confirmation-->
    </a>
</div>
<hr>
                ...
                success: function (data) {
                    ...
                    if (jQuery.isEmptyObject(data['detail'])) {
                        ...
                        $.each(data['detail'], function (key, value) {
                            $('#table_body').append('<tr><td>' + x + '</td><td>' + value['claimcategoriesDescription'] + '</td><td>' + value['description'] + '</td><td >' + value['referenceNo'] + '</td><td >' + value['segmentCode'] + '</td><td class="text-center">' + value['transactionCurrency'] + '</td><td class="text-right">' + parseFloat(value['transactionAmount']).formatMoney(+value['transactionCurrencyDecimalPlaces'] + ',', '.') + '</td><td class="text-right"><a onclick="edit_item(' + value['expenseClaimDetailsID'] + ');"><span class="glyphicon glyphicon-pencil"></span></a> &nbsp;&nbsp; |&nbsp;&nbsp; <a onclick="delete_item(' + value['expenseClaimDetailsID'] + ');"><span style="color:rgb(209, 91, 71);" class="glyphicon glyphicon-trash"></span></a></td></tr>');
                            x++;
                        });
                    }
                    stopLoad();
