                                              ...
                                              class="sign"><i
                                                ...
                                                if (empty($default_data['DD'])) {
                                                   ...
                                                   // $stageFour = $this->db->query("SELECT * FROM srp_erp_statemaster WHERE type = 3 AND countyID = {$level1['countryID']} AND masterID = {$level3['stateID']} AND ( stateID = {$default_data['DD']} OR divisionTypeCode = 'JD' )ORDER BY Description ASC")->result_array();
                                                    $where = "srp_erp_statemaster.type = 3 AND srp_erp_statemaster.countyID = " . $level1['countryID'] . " AND srp_erp_statemaster.masterID = " . $level3['stateID'] . " AND srp_erp_statemaster.stateID = " . $default_data['DD'] . "";
                                                    $this->db->select('srp_erp_statemaster.divisionTypeCode,srp_erp_statemaster.stateID,srp_erp_statemaster.countyID,srp_erp_statemaster.Description');
                                                    $this->db->from('srp_erp_statemaster');
                                                    $this->db->where($where);
                                                    $this->db->get();
                                                    $query1 = $this->db->last_query();

                                                    $where2 = "srp_erp_ngo_com_regionmaster.companyID=".$companyID." AND srp_erp_statemaster.type = 3 AND srp_erp_statemaster.countyID = " . $level1['countryID'] . " AND srp_erp_statemaster.masterID = " . $level3['stateID'] . " AND srp_erp_statemaster.divisionTypeCode ='JD' ";
                                                    $this->db->select('srp_erp_statemaster.divisionTypeCode,srp_erp_statemaster.stateID,srp_erp_statemaster.countyID,srp_erp_statemaster.Description');
                                                    $this->db->from('srp_erp_statemaster');
                                                    $this->db->join('srp_erp_ngo_com_regionmaster', 'srp_erp_statemaster.stateID = srp_erp_ngo_com_regionmaster.jammiyahDivisionID', 'inner' );
