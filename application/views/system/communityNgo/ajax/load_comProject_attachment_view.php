                ...
                <div class="box-body" style="text-align: center; background: #ffffff;">
                    ...
                    if (!empty($docDet)) {
                        ...
                        foreach ($docDet as $doc) {
                            ...
                            $link = $this->s3->getMyAuthenticatedURL($doc['myFileName'], 3600);
    ...
    $(document).ready(function () {
        $('#Doc_form_attachment').bootstrapValidator({
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                data: formData,
                url: '<?php echo site_url('CommunityNgo/save_com_proj_proposal_attachments'); ?>',
                contentType: false,
                cache: false,
                processData: false,
                beforeSend: function () {
    ...
    function delete_attachment(attachmentid, FileName) {
        swal({
            ...
            function () {
                $.ajax({
                    async: true,
                    url: "<?php echo site_url('CommunityNgo/delete_com_proj_proposal_attachment'); ?>",
                    type: 'post',
                    dataType: 'json',
