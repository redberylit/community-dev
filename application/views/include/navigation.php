...
$this->session->set_userdata("familyDocUploadPath", trim($familyDocUploadPath));
...
if ($companyType == 1) {
    ...
    $sql = "SELECT srp_erp_navigationmenus.languageID,srp_erp_navigationmenus.accessControlYN,srp_erp_companysubgroupnavigationsetup.*,template.TempPageNameLink FROM srp_erp_companysubgroupnavigationsetup LEFT JOIN srp_erp_navigationmenus ON srp_erp_navigationmenus.navigationMenuID = srp_erp_companysubgroupnavigationsetup.navigationMenuID
...
<aside class="main-sidebar">
    <section class="sidebar">
        ...
        <form action="#" method="get" class="sidebar-form">
            ...
            echo form_dropdown('company', drill_down_navigation_dropdown(), $companyID . '-' . $companyTp .'-'. $isGroupUser.'-'. $userID, 'id="parentCompanyID" onchange="change_fetchcompany($(\'#parentCompanyID option:selected\').val(),$(\'#parentCompanyID option:selected\').text())" class="form-control select2", required'); ?>
        </form>
        <!-- /.search form -->
                                                    ...
                                                    <ul class="treeview-menu" style="display: none;">
                                                        <?php foreach ($data as $child2) {
                                                            if ($child2['levelNo'] == 2 && $child['navigationMenuID'] == $child2['masterID']) {
                                                                ...
                                                                <li class="">
                                                                    <a class="navMenu"
                                                                       onclick="fetchPage('<?php echo $child2['TempPageNameLink'] ?>','<?php echo $child2['pageID'] ?>','<?php echo $child2['pageTitle'] ?>');control_staff_access('<?php echo $child2['pageID'] ?>','<?php echo $child2['TempPageNameLink'] ?>',1,'<?php echo $child2['pageTitle'] ?>','<?php echo $child2['accessControlYN'] ?>');"><i
