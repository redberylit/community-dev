    ...
    function fetchPage(page_url, page_id, page_name, policy_id, data_arr, master_page_url=null) {
        ...
        $.ajax({
            ...
            success: function (page_html) {
                ...
                $('.community_master_styles').attr("disabled", "disabled");
...
/*third_party_model*/
    ...
    function documentPageView_modal(documentID, para1, para2, approval=0) {
        ...
        switch (documentID) {
            ...
            case "SA": // Stock Adjustment - ShahmyDocument ID is not set .
                if(para2 == 'buy')
                {
            ...
            case "SD-C": // Salary Declaration - Nasik (in Salary declaration master)
                ...
                /*Salary Declaration*/
                break;
            case "SD-C2": // Salary Declaration - Nasik (in Salary declaration master)
            ...
            case "JP": // Aflal
                ...
                break;
            case "BDT": // Budget Transfer - mushtaq
    ...
    function control_staff_access(page_id, page_url, status,pageTitle,accessControlYN) {

        $.ajax({
            type: 'POST',
            url: 'Access_menu/control_staff_access',
            data: {
                'page_url': page_url
            },
            success: function (access) {
                ...
                if (accessControlYN== 1 && (myArray.includes('1'))) {

                    //menu control
                    if (status == 1) {
                        $('#' + page_id).attr("onclick", "open_access_denied_alertMod()");
                        open_access_denied_alertMod();
                        fetchPage('system/erp_mo_defAccessControl', '', '');
                    }
                    ...
                    // menu control
                    if (status == 1) {
                        $('#' + page_id).removeAttr("onclick").attr("href",  page_url );
                        // window.location.href = page_url;
                       // fetchPage(page_url, '', pageTitle);

                    }

                    if (myArray.includes('Add')) {

                        $('.CA_Alter_btn').removeAttr("onclick").attr("onclick", "open_access_denied_alertMod()"); //control for add , edit an delete buttons
                        $('.CA_Submit_btn').hide();   //control for submit button
                        $('.CA_select2_disable').prop('disabled', true);   //control for select2
                        $('.CA_text_field_disable').prop('disabled', true);   //control for text field n textarea
                        $("input[type=radio]").attr('disabled', true); // for radio buttons
                        $('.CA_Checkbox_disable').each(function() { // for checkbox
                            $(this).prop('disabled', true);
                        });

                        //    $('input,textarea').attr('disabled', 'disabled'); //control for others
                        //   $('input,textarea').prop('readonly', true);

                    }

                    if (myArray.includes('View')) {
                    }

                    if (myArray.includes('Print')) {

                        $('.CA_Print_Excel_btn').removeAttr("onclick").attr("onclick", "open_access_denied_alertMod()"); //control for print and exel
                    }
                }
