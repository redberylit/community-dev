...
if (!function_exists('get_all_pos_customers')) {
    function get_all_pos_customers($status = true)/*Load all Customer*/
    {
        $CI =& get_instance();
        $CI->db->select("posCustomerAutoID,CustomerName");
        $CI->db->from('srp_erp_pos_customermaster');
        $CI->db->where('companyID', current_companyID());
        $customer_arr = array();
        $customer = $CI->db->get()->result_array();
        if (isset($customer)) {
            foreach ($customer as $row) {
                $customer_arr[trim($row['posCustomerAutoID'])] = (trim($row['posCustomerAutoID']) ? trim($row['CustomerName']) : '');
            }
        }
        return $customer_arr;
    }
