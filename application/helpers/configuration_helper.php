...
$actual_link = "images/logo/";
$actual_link_html = base_url('images/logo/');
...
if (!function_exists('table_class')) {
    ...
    {
        return 'table table-responsive table-bordered table-striped table-condensed table-row-select gradient-tr';
    }
}
...
if (!function_exists('familyDocUploadPath')) {
    function familyDocUploadPath()
    {
        $CI =& get_instance();

        return trim($CI->common_data['familyDocUploadPath']);
    }
}
...
if (!function_exists('confirm_ap_user')) {
    function confirm_ap_user($approved_status, $confirmed_status, $code, $autoID, $isFromLeave = null,$collectedStatus=null)
    {
        $status = '<center>';
...
if (!function_exists('load_invoice_action')) {
    function load_invoice_action($poID, $POConfirmedYN, $approved, $createdUserID, $confirmedYN, $isDeleted, $tempInvoiceID,$confirmedByEmpID)
    {
        $CI =& get_instance();
...
if (!function_exists('load_contract_action')) {
    function load_contract_action($poID, $POConfirmedYN, $approved, $createdUserID, $documentID, $confirmedYN, $isDeleted,$confirmedbyempid)
    {
        $CI =& get_instance();
        $CI->load->library('session');
        $status = '<span class="pull-right">';
...
if (!function_exists('load_rv_action')) {
    function load_rv_action($poID, $POConfirmedYN, $approved, $createdUserID, $isDeleted,$confirmedByEmp)
    {
        $CI =& get_instance();
        $CI->load->library('session');
        $status = '<span class="pull-right">';
...
if (!function_exists('bank_transaction_edit')) {
    function bank_transaction_edit($transactionID, $confirmYN, $approvedYN, $createduserID, $transferType, $fromBankGLAutoID,$confirmedByEmp)
    {
        ...
        $status .= '<a onclick=\'attachment_modal(' . $transactionID . ',"Bank Transfer","BT",' . $confirmYN . ');\'><span title="Attachment" rel="tooltip" class="glyphicon glyphicon-paperclip"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;';
...
if (!function_exists('journal_entry_action')) {
    function journal_entry_action($JVMasterAutoId, $confirmedYN, $approvedYN, $createdUserID, $isDeleted, $JVType,$confirmedByEmpID)
    {
        $CI =& get_instance();
        $CI->load->library('session');
        $status = '<span class="pull-right">';
...
if (!function_exists('drill_down_navigation_dropdown')) {
    function drill_down_navigation_dropdown()
    {
        ...
        if($isGroupUser==1){
            ...
            $group_company = $db2->query("select companyID as companyID,CONCAT(srp_erp_company.company_code,' | ',srp_erp_company.company_name) as company,empID as eid FROM `groupusercompanies` LEFT JOIN srp_erp_company ON srp_erp_company.company_id=groupusercompanies.companyID WHERE EidNo={$idno} ")->result_array();
            foreach ($group_company as $row) {
            ...
            if (!empty($group_company)) {
                foreach ($group_company as $row) {
                    $group_company_arr[trim($row['companyID']) . '-1-0-'. $userID] = trim($row['company']);
                }
            } else {
...
if (!function_exists('editcustomerotherlang')) {
    function editcustomerotherlang($customerAutoID)
    {
        // echo $to;
        $CI =& get_instance();
        $status = '<span class="pull-right">';
                ...
                if (in_array($formula_row, $operand_arr)) { //validate is a operand
                    ...
                    if ($elementType == '_') {
                        ...
                        if (trim($monthlyADArr[1]) == '0') {
                            ...
                            $formulaText .= '<li class="formula-li" data-value="!' . $monthlyADArr[1] . '" onclick="addSelectedClass(this)">';
                            $formulaText .= '<span class="formula-remove close" onclick="removeFormulaItem(this)"><i class="fa fa-times"></i></span>';
                            $formulaText .= '<span class="formula-text-value">Fixed Payment | Gross Salary</span></li>';
                        }
...
if (!function_exists('gl_description_template')) {
    function gl_description_template($templateID, $requestType='s', $id)
    {
        $CI =& get_instance();
        ...
        if($requestType == 'S'){ /*For sub category*/
        ...
        else{ /*For Group Total*/
            /*$template = $CI->db->query("SELECT CONCAT(description, ' | sub category') AS desStr, CONCAT(detID, '|SC') AS GLAutoID
                        FROM srp_erp_companyreporttemplatedetails detTB
                        WHERE companyReportTemplateID = {$templateID} AND itemType=1 AND NOT EXISTS (
                           SELECT subCategory FROM srp_erp_companyreporttemplatelinks lkTB
                           WHERE templateDetailID = {$templateID} AND lkTB.subCategory = detTB.detID
                        )
                        UNION ALL
                        SELECT CONCAT(systemAccountCode, ' | ', GLSecondaryCode,  ' | ', GLDescription) desStr,
                        CONCAT(GLAutoID, '|GL') AS GLAutoID
                        FROM srp_erp_chartofaccounts chTB
                        WHERE masterCategory = '{$masterCategory}' AND companyID = {$companyID}")->result_array();*/
...
if (!function_exists('drill_down_emp_language')) {
    ...
    {
        $CI =& get_instance();
        $data = $CI->db->query("SELECT *,CASE 
    WHEN description = \"Arabic\" THEN \"ع\" 
	WHEN description = \"English\" THEN \"ENG\" 
	WHEN description = \"Tamil\" THEN \"தமிழ்\" 
	WHEN description = \"Sinhala\" THEN \"සිංහල\"
	END `languageshortcode` 
FROM
srp_erp_lang_languages
Where isActive = 1 
  ORDER BY
languageID 
DESC")->result_array();
...
if (!function_exists('get_fm_templateDetails')) {
    function get_fm_templateDetails($masterID){
        ...
        foreach ($data as $row){
            ...
            if($masterType == 'H'){
                $returnData .= '<button type="button" class="btn btn-xs btn-default" onclick="new_header_or_group('.$masterID.')">';
            ...
            $returnData .= '<button type="button" class="btn btn-xs btn-default" onclick="edit_title('.$masterID.', \''.$row['description'].'\')">'.$edit_span.'</button> &nbsp; ';
            $returnData .= '<button type="button" class="btn btn-xs btn-default" onclick="delete_template_data('.$masterID.', \''.$masterType.'\')">'.$delete_span.'</button></td></tr>';
            ...
            if($masterType == 'H'){
                foreach ($subData as $sub_row){
                    ...
                    $returnData .= '<span class="glyphicon glyphicon-cog" style="color:green; font-size: 10px;"></span></button> &nbsp; ';
                    $returnData .= '<button type="button" class="btn btn-xs btn-default" onclick="edit_title('.$detID.', \''.$sub_row['description'].'\')">'.$edit_span.'</button>  &nbsp;  ';
                    $returnData .= '<button type="button" class="btn btn-xs btn-default" onclick="delete_template_data('.$detID.', \''.$type.'\')">'.$delete_span.'</button></td> </tr>';
                    ...
                    if($sub_row['itemType'] == 1){ /*Sub category*/
                        ...
                        foreach ($glData as $gl_row){
                            ...
                            $returnData .= '<td class="sub2"><button type="button" class="btn btn-xs btn-default pull-right" onclick="delete_template_data('.$gl_row['linkID'].', \'GL\')">';
...
if (!function_exists('rpt_template_det_type')) {
    function rpt_template_det_type($type)
    {
        $str = '';
        switch($type){
            case  1: $str = 'Sub Category'; break;
            case  2: $str = 'Header'; break;
            case  3: $str = 'Group Total'; break;
        }
        return $str;
    }
}
...
if (!function_exists('load_fm_template')) {
    function load_fm_template($temMasterID, $masterID, $dPlace, $print=0){
        $CI =& get_instance();
            ...
            if($row['itemType'] == 2){
                ...
                foreach ($subData as $sub_row){
                    ...
                    if($sub_row['itemType'] == 3){ /*Group*/
                        ...
                        $amount = number_format(0, $dPlace);
                        if(!empty($group_glData)){
                            $where_in_array = array_column($group_glData, 'glAutoID');
                            $where_in = implode(',', $where_in_array);
                            $amount = $CI->db->query("SELECT FORMAT( SUM(IFNULL(transactionAmount,0) * -1), transactionCurrencyDecimalPlaces) trAmount                                               
                        ...
                        $returnData .= '<td class="sub1 sub_total_rpt amount_td_rpt" id="subTot_'.$detID.'" style="text-align: right;">'.$amount.'</td></tr>';
                    }
                }
            }
            else{
                ...
                $returnData .= '<td class="sub1 total_black_rpt amount_td_rpt" id="subTot_'.$templateID.'" style="text-align: right;">'.$amount.'</td></tr>';
...
if (!function_exists('update_warehouse_items')) {
  ...
  round(sum(srp_erp_itemledger.transactionQTY/srp_erp_itemledger.convertionRate),2) as itemledQty,
  round(ifnull((srp_erp_warehouseitems.currentStock),0) - sum(srp_erp_itemledger.transactionQTY/srp_erp_itemledger.convertionRate),2) as diff
FROM
    srp_erp_warehouseitems
JOIN srp_erp_itemledger ON (srp_erp_warehouseitems.itemAutoID = srp_erp_itemledger.itemAutoID
...
if (!function_exists('update_item_master')) {
  ...
  round(sum(srp_erp_itemledger.transactionQTY/srp_erp_itemledger.convertionRate),2) as itemledQty,
  round(ifnull((srp_erp_itemmaster.currentStock),0) - sum(srp_erp_itemledger.transactionQTY/srp_erp_itemledger.convertionRate),2)as diff
FROM
    srp_erp_itemmaster
JOIN srp_erp_itemledger ON srp_erp_itemmaster.itemAutoID = srp_erp_itemledger.itemAutoID
...
if (!function_exists('drilldown_emp_location_drop')) {
    function drilldown_emp_location_drop()
    {
        ...
        $data = $CI->db->query("SELECT locationmaster.* FROM srp_erp_location locationmaster LEFT JOIN srp_erp_locationemployees emplocation on emplocation.locationID = locationmaster.locationID WHERE
  locationmaster.companyID = $companyID AND isCostLocation = 1 AND empID = $currentuser")->result_array();
        return $data;

    }
    }

if (!function_exists('all_financeyear_drop_location')) {//finance drop location wise codegeneration
...
if (!function_exists('document_status')) {
    function document_status($docID, $masterID, $isOn=0)
    {
        ...
        if($document_output['approvalVal'] == 1){
            return ['error' => 1, 'message' => 'This document already approved,<br/>Please refresh the page and check again.'];
        }
        ...
        if($document_output['confirmVal'] == 1){
            return ['error' => 1, 'message' => 'This document already confirmed,<br/>Please refresh the page and check again.'];
        }
...
if (!function_exists('load_invoice_action_margin')) {
    function load_invoice_action_margin($poID, $POConfirmedYN, $approved, $createdUserID, $confirmedYN, $isDeleted, $tempInvoiceID,$confirmedByEmpID)
    {
        $CI =& get_instance();
        $CI->load->library('session');
        $status = '<span class="pull-right">';
