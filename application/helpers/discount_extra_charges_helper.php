...
if (!function_exists('action_disc_and_extra')) {
    function action_disc_and_extra($discountExtraChargeID)
    {
        ...
        if (empty($discounttempid) && empty($extratempid) && empty($extraid) && empty($discountid)) {
            $status .= '<spsn class="pull-right"><a onclick="open_discount_edit_model('.$discountExtraChargeID.')"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp; | &nbsp;&nbsp;<a onclick="delete_discount_category('.$discountExtraChargeID.')"><span class="glyphicon glyphicon-trash" style="color:rgb(209, 91, 71);"></span></a></span>';
        }else{
