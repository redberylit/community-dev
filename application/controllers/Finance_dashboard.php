...
class Finance_dashboard extends ERP_Controller
{
    ...
    function fetch_financialPosition()
    {
        ...
        $this->datatables->select('CurrencyCode as bankCurrencyCode,srp_erp_chartofaccounts.GLDescription,(SUM(if(srp_erp_bankledger.transactionType = 1,srp_erp_bankledger.bankcurrencyAmount,0)) - SUM(if(srp_erp_bankledger.transactionType = 2,srp_erp_bankledger.bankcurrencyAmount,0))) as bookBalance,(SUM(if(srp_erp_bankledger.transactionType = 1 AND srp_erp_bankrecmaster.approvedYN = 1,srp_erp_bankledger.bankcurrencyAmount,0)) - SUM(if(srp_erp_bankledger.transactionType = 2 AND srp_erp_bankrecmaster.approvedYN = 1,srp_erp_bankledger.bankcurrencyAmount,0))) as bankBalance', false)
            ...
            ->edit_column('bookBalance', '<div class="text-right"><b> $1 </b></div>', 'dashboard_format_number(bookBalance)')
            ->edit_column('bankBalance', '<div class="text-right"><b> $1 </b></div>', 'dashboard_format_number(bankBalance)');
    ...
    function fetch_postdated_cheque_given()
    {
        ...
        if ($PostDatedChequeManagement == 1) {
            if ($this->input->post("dueDays") == 'all') {
                $this->datatables->select('t1.bankCurrencyAmount as bankCurrencyAmount,t1.dueDate as dueDate,t1.bankCurrency as bankCurrency,t1.dueDays as dueDays,t1.vendor as vendor,t1.remainIn as remainIn,', false)
...
AND "' . $endDate . '" AND il.companyID = "' . $this->common_data['company_data']['company_id'] . '" AND il.documentCode IN ("CINV","RV","POS") AND im.mainCategory = "Inventory"')
    ...
    function fetch_sales_log()
    {
        ...
        $this->datatables->select('IF(srp_erp_customermaster.customerName IS NULL OR srp_erp_customermaster.customerName = "","Sundry",srp_erp_customermaster.customerName) as customerName,SUM(if(documentDate >= \'' . $beginingDate . '\' AND documentDate <= \'' . $endDate . '\',companyReportingAmount,0))*-1 as currentYear,SUM(if(documentDate >= \'' . $beginingDateLast . '\' AND documentDate <= \'' . $endDateLast . '\',companyReportingAmount,0))*-1 as lastYear,DecimalPlaces', false)
            ->from('srp_erp_generalledger')
            ->join('srp_erp_chartofaccounts', 'srp_erp_generalledger.GLAutoID = srp_erp_chartofaccounts.GLAutoID AND srp_erp_chartofaccounts.masterCategory = "PL" AND srp_erp_chartofaccounts.companyID = ' . $this->common_data['company_data']['company_id'], 'INNER')
            ->join('srp_erp_customermaster', 'srp_erp_customermaster.customerAutoID = srp_erp_generalledger.partyAutoID', 'LEFT')
            ->join('srp_erp_currencymaster', 'srp_erp_currencymaster.currencyID = srp_erp_generalledger.companyReportingCurrencyID', 'LEFT')
            ->where('srp_erp_chartofaccounts.accountCategoryTypeID = 11 AND srp_erp_generalledger.companyID = ' . $this->common_data['company_data']['company_id'] . ' AND srp_erp_generalledger.partyType="CUS"')
            ...
            ->group_by('srp_erp_generalledger.partyAutoID');
        echo $this->datatables->generate();
        //echo $this->datatables->last_query();
    }
