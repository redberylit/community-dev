...
class MFQ_Template extends ERP_Controller
{
    ...
    function save_mfq_template_detail()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //if ($this->MFQ_Template_model->workflow_exist()) {
                //echo json_encode(array('e', 'Already work flow added to the template'));
            //} else {
    ...
    function delete_workprocess_attachment()
    {
        ...
        $url = base_url("attachments/MFQ");
        $link = "$url/$myFileName";
        $result = $this->db->delete('srp_erp_mfq_workflowattachments', array('attachmentID' => trim($attachmentID)));
        if ($result) {
