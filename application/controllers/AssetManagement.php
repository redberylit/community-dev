...
//        $this->form_validation->set_rules('dateexpired', 'dateexpired', 'trim|required');
    ...
    function load_disposal()
    {
        ...
        $this->datatables->select('assetdisposalMasterAutoID AS assetdisposalMasterAutoID,serialNo,segmentCode,companyCode,documentID,disposalDocumentCode,DATE_FORMAT(disposalDocumentDate,\'' . $convertFormat . '\') AS disposalDocumentDate,narration,confirmedYN AS confirmedYN,approvedYN AS approvedYN,confirmedByEmpID as confirmedByEmpID,createdUserID as createdUserID')
        ...
        $this->datatables->add_column('edit', '$1', 'disposal_action(assetdisposalMasterAutoID,confirmedYN,approvedYN,createdUserID,confirmedByEmpID)');
        echo $this->datatables->generate();
    }
...
")->row_array();
    ...
    {
        $this->datatables->select('locationID,locationName,IFNULL(locationCode,\'-\') as locationCode,concat(\'Add Employee (\',locationCode,\' - \',locationName,\')\') as loccodename', false)
            ->from('srp_erp_location')
            ->add_column('edit', '$1', 'action_asset_location_code(locationID,locationName,locationCode,loccodename)')
            ->where('companyID', current_companyID())
