...
FROM srp_erp_ngo_com_communitymaster AS t1
                                        ...
                                        WHERE t1.Com_MasterID = {$Com_MasterID} AND t1.companyID={$companyID}")->result_array();
...
FROM srp_erp_ngo_com_communitymaster AS t1
    ...
    function fetch_country_based_Area_Dropdown()
    {
        ...
        if (!empty($bornConutryId)) {
            $division = $this->db->query("SELECT stateID,Description FROM srp_erp_statemaster WHERE countyID = {$bornConutryId} AND type = 4 AND divisionTypeCode = 'MH'")->result_array();
        }
        ...
        if (!empty($division)) {
            foreach ($division as $row) {
                if ($relateArea == trim($row['stateID'])) {
                    echo '<option value="' . trim($row['stateID']) . '" selected="selected">' . trim($row['Description']) . '</option>';
                } else {
                    echo '<option value="' . trim($row['stateID']) . '">' . trim($row['Description']) . '</option>';
                }
    ...
    public function get_comMemBloodRelative()
    {
        ...
        $bldRelCom = $CI->db->query("SELECT srp_erp_ngo_com_communitymaster.Com_MasterID,CName_with_initials,DATE_FORMAT(CDOB,'{$convertFormat}') AS CDOB,CountryOfOrigin,CPlaceOfBirth,CBC_No,DATE_FORMAT(CBC_Date,'{$convertFormat}') AS CBC_Date,CNIC_No FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.Com_MasterID = '$Com_MasterID' AND comVerifiApproved='1' AND srp_erp_ngo_com_communitymaster.companyID='{$companyID}' GROUP BY srp_erp_ngo_com_communitymaster.Com_MasterID");
        $rowRelCom = $bldRelCom->row();

        $queryJob = $this->db->query("SELECT DISTINCT MemJobID,specializationID FROM srp_erp_ngo_com_memjobs WHERE srp_erp_ngo_com_memjobs.Com_MasterID='$Com_MasterID' AND srp_erp_ngo_com_memjobs.companyID='$companyID' ");
        $rowJob = $queryJob->row();
        if (!empty($rowJob)) {
...
<div class="canceldiv" id="editmemService_$1"><a id="" class="CA_Alter_btn"  onclick="edit_memService($1)"><span title="Edit" rel="tooltip" class="glyphicon glyphicon-pencil" style=""></span></a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a onclick="delete_memService($1)"><span title="Delete" rel="tooltip" class="glyphicon glyphicon-trash" style="color:rgb(209, 91, 71);"></span></a></div>', 'CmtMemServiceID');
    ...
    function get_totalFamHousing()
    {
        ...
        $this->load->model('CommunityNgo_model');
        $this->CommunityNgo_model->get_totalFamHousing($FamMasterID, $houseOwnshp, $houseType, $GS_DivisionArr, $RegionIDArr);
    }
...
WHERE $where AND (hEnr.FamHouseSt = '0' OR hEnr.FamHouseSt = NULL) $FamMasterIDS  " . " $houseOwnshpS $houseTypeS ORDER BY hEnr.hEnrollingID")->result_array();
