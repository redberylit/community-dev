...
class CommunityNgo extends ERP_Controller
{
    ...
    function load_communityMemberDetails()
    {
        ...
        $where = "srp_erp_ngo_com_communitymaster.companyID = " . $companyID . $search_string . $search_sorting . $deleted . $comVerifiApproved;
    ...
    function check_isNIC_available()
    {
        ...
        if ($Com_MasterID) {
            $result = $this->db->query("SELECT * FROM srp_erp_ngo_com_communitymaster
                                    WHERE Com_MasterID != {$Com_MasterID} AND CNIC_No = '$NIC' AND isDeleted='0'")->result_array();
        } else {
            $result = $this->db->query("SELECT * FROM srp_erp_ngo_com_communitymaster
                                    WHERE CNIC_No = '$NIC' AND isDeleted='0'")->result_array();
        }
    ...
    function loadEmployees()
    {
        ...
        $result = $this->db->query("SELECT Com_MasterID,CName_with_initials FROM srp_erp_ngo_com_communitymaster
                                    WHERE isDeleted = 0 AND srp_erp_ngo_com_communitymaster.comVerifiApproved='1' AND companyID = {$companyID} $GenderIDIN $GS_DivisionIN $RegionIDIN")->result_array();
        $data['members'] = $result;
    ...
    function fetch_all_member_details()
    {
        ...
        $where = "srp_erp_ngo_com_communitymaster.companyID = " . $companyID . $deleted . $comVerifiApproved;

        $this->datatables->select("Com_MasterID,MemberCode,CName_with_initials,CImage,CNIC_No,isActive,srp_erp_ngo_com_communitymaster.isDeleted,srp_erp_gender.name AS Gender,CONCAT(TP_Mobile,' | ',TP_home) AS PrimaryNumber,area.Description AS Region ,division.Description AS GS_Division");
    ...
    function load_memberDetailsView()
    {
        ...
        $where = "tCom.companyID = " . $companyID . " AND tCom.Com_MasterID = '" . $Com_MasterID . "' AND tCom.comVerifiApproved='1'";

        $data['master'] = $this->db->query("SELECT *,divisionc.stateID,divisionc.Description AS GS_DivisionSt,tCom.createdDateTime,tCom.createdUserName,tCom.modifiedDateTime 
...
FROM srp_erp_ngo_com_communitymaster AS tCom LEFT JOIN srp_titlemaster ON srp_titlemaster.TitleID = tCom.TitleID LEFT JOIN srp_erp_gender ON srp_erp_gender.genderID = tCom.GenderID LEFT JOIN srp_erp_statemaster divisionc ON divisionc.stateID = tCom.GS_Division LEFT JOIN srp_erp_statemaster area ON area.stateID = tCom.RegionID LEFT JOIN srp_erp_bloodgrouptype ON srp_erp_bloodgrouptype.BloodTypeID = tCom.BloodGroupID LEFT JOIN srp_erp_ngo_com_maritalstatus ON srp_erp_ngo_com_maritalstatus.maritalstatusID = tCom.CurrentStatus LEFT JOIN srp_erp_ngo_com_schools ON srp_erp_ngo_com_schools.schoolComID=tCom.SchoolAttended LEFT JOIN srp_erp_countrymaster ON srp_erp_countrymaster.countryID=tCom.CountryOfResidentID 
...
WHERE $where ")->row_array();
    ...
    function fetch_other_attachments()
    {
        ...
        if (!empty($data)) {
            foreach ($data as $val) {
                $path = "Community/$companyCode/Attachments/";
                $memOtherAttachment = get_all_community_uploads($val['myFileName'], $path, 'Member_Other_');
    ...
    function upload_other_attachments($description = true)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'Member_Other_' . $this->input->post('other_documentID') . '_' . $this->input->post('other_documentSystemCode') . '_' . (count($num) + 1);
            $file = $_FILES['other_document_file'];
            if ($file['error'] == 1) {
            ...
            $path = "Community/$companyCode/Attachments/$fileName.$ext";
    ...
    function load_member_all_attachments()
    {
        ...
        $whereComHelp = "srp_erp_ngo_attachments.companyID = " . $companyid . " AND documentID = '9' AND Com_MasterID = " . $Com_MasterID . "";
        $this->db->select('*');
        $this->db->join('srp_erp_ngo_com_memberhelprequirements', '(srp_erp_ngo_com_memberhelprequirements.memHelprID = srp_erp_ngo_attachments.documentAutoID)', 'left');
        $this->db->from('srp_erp_ngo_attachments');
        $this->db->where($whereComHelp);
        $data['needHelp_attachment'] = $this->db->get()->result_array();
        ...
        $whereSickness = "srp_erp_ngo_attachments.companyID = " . $companyid . " AND documentID = '12' AND Com_MasterID = " . $Com_MasterID . "";
        $this->db->select('*');
        ...
        $whereComProperty = "srp_erp_ngo_attachments.companyID = " . $companyid . " AND documentID = '13' AND Com_MasterID = " . $Com_MasterID . "";
        $this->db->select('*');
        $this->db->join('srp_erp_ngo_com_membervehicles', '(srp_erp_ngo_com_membervehicles.memVehicleID = srp_erp_ngo_attachments.documentAutoID)', 'left');
        $this->db->from('srp_erp_ngo_attachments');
        $this->db->where($whereComProperty);
        $data['property_attachment'] = $this->db->get()->result_array();

        $whereComInvolvement = "srp_erp_ngo_attachments.companyID = " . $companyid . " AND documentID = '14' AND Com_MasterID = " . $Com_MasterID . "";
        $this->db->select('*');
        $this->db->join('srp_erp_ngo_com_memberwillingtohelp', '(srp_erp_ngo_com_memberwillingtohelp.willingToHelpID = srp_erp_ngo_attachments.documentAutoID)', 'left');
        $this->db->from('srp_erp_ngo_attachments');
        $this->db->where($whereComInvolvement);
        $data['involvement_attachment'] = $this->db->get()->result_array();

        $whereSocialGrant = "srp_erp_ngo_attachments.companyID = " . $companyid . " AND documentID = '15' AND Com_MasterID = " . $Com_MasterID . "";
        $this->db->select('*');
        $this->db->join('srp_erp_ngo_com_membersocialgrants', '(srp_erp_ngo_com_membersocialgrants.MemSocialGrantID = srp_erp_ngo_attachments.documentAutoID)', 'left');
        $this->db->from('srp_erp_ngo_attachments');
        $this->db->where($whereSocialGrant);
        $data['socialGrants_attachment'] = $this->db->get()->result_array();
    ...
    function load_memberStatus_attachments()
    {
        ...
        if (!empty($data)) {
            foreach ($data as $val) {
                ...
                $statusAttachment = get_all_community_uploads($val['myFileName'], $path, 'Member_Status');
    ...
    function status_attachment_upload($description = true)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'Member_Status_' . $this->input->post('status_documentSystemCode') . '_' . (count($num) + 1);
    ...
    function delete_member_image()
    {
        ...
        if (!empty($myFileName)) {
            $result = $this->s3->deleteMyObject($myFileName);
        }
    ...
    function delete_member_attachment()
    {
        ...
        if (!empty($myFileName)) {
            ...
            $result = $this->s3->deleteMyObject($path . '/' . $myFileName);
        }
        /** end of AWS s3 delete object */
        if ($result) {
    ...
    function ngo_Memberattachement_upload()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'Mem_' . $this->input->post('document_name') . '_' . $this->input->post('documentID') . '_' . (count($num) + 1);
            $file = $_FILES['document_file'];
            if ($file['error'] == 1) {
                die(json_encode(['status' => 0, 'type' => 'e', 'message' => 'The file you are attempting to upload is larger than the permitted size. (maximum 5MB).']));
            }
    ...
    //member export to pdf
    function load_community_allMembers_details()
    {
        ...
        $where = "srp_erp_ngo_com_communitymaster.companyID = " . $companyID . $deleted . $active_filter . $division_filter . $region_filter . $gender_filter . $member_filter . $comVerifiApproved;

        $data['comMemberMas'] = $this->db->query("SELECT MemberCode,CName_with_initials,CNIC_No,CImage,srp_erp_ngo_com_communitymaster.isActive,srp_erp_gender.name AS Gender,division.Description AS GS_Division,area.Description AS Region,TP_Mobile  FROM srp_erp_ngo_com_communitymaster LEFT JOIN srp_erp_statemaster division ON division.stateID = srp_erp_ngo_com_communitymaster.GS_Division LEFT JOIN srp_titlemaster ON srp_titlemaster.TitleID = srp_erp_ngo_com_communitymaster.TitleID LEFT JOIN srp_erp_gender ON srp_erp_gender.genderID = srp_erp_ngo_com_communitymaster.GenderID LEFT JOIN srp_erp_statemaster area ON area.stateID = srp_erp_ngo_com_communitymaster.RegionID LEFT JOIN srp_erp_bloodgrouptype ON srp_erp_bloodgrouptype.BloodTypeID = srp_erp_ngo_com_communitymaster.BloodGroupID LEFT JOIN srp_erp_ngo_com_maritalstatus ON srp_erp_ngo_com_maritalstatus.maritalstatusID = srp_erp_ngo_com_communitymaster.CurrentStatus  WHERE $where ORDER BY Com_MasterID DESC ")->result_array();

        $html = $this->load->view('system/communityNgo/ajax/load_com_ngo_comMembers_pdf', $data, true);
    ...
    function check_memFamilyChain()
    {
        $companyID = current_companyID();
        $Com_MasterID = $this->input->post('Com_MasterID');
        $data = $this->db->query("SELECT srp_erp_ngo_com_familydetails.Com_MasterID FROM srp_erp_ngo_com_familydetails WHERE srp_erp_ngo_com_familydetails.Com_MasterID ={$Com_MasterID} AND srp_erp_ngo_com_familydetails.companyID={$companyID} AND (srp_erp_ngo_com_familydetails.relationshipID='4' OR srp_erp_ngo_com_familydetails.relationshipID='5')")->row_array();
        if (!empty($data)) {
    ...
    public function load_memberOtherDetails_View()
    {
        $Com_MasterID = $this->input->post('Com_MasterID');
        $companyID = current_companyID();
        $data['memberName'] = $this->db->query("SELECT MemberCode,CName_with_initials FROM srp_erp_ngo_com_communitymaster
                                        WHERE Com_MasterID = {$Com_MasterID} AND companyID={$companyID}")->row_array();
        ...
        $data['HelpReqComOther'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_memberhelprequirements AS t1
                                        JOIN srp_erp_ngo_com_helprequirements AS t2 ON t2.helpRequireID = t1.helpRequireID
                                        WHERE t1.companyID={$companyID} AND Com_MasterID = {$Com_MasterID} AND helpRequireType='OTHER' ")->result_array();
    ...
    public function load_memParentDetails_view()
    {
        $convertFormat = convert_date_format_sql();

        $Com_MasterID = $this->input->post('Com_MasterID');
        // $companyID = current_companyID();
        $data['memberParent'] = $this->db->query("SELECT t1.Com_MasterID,t1.cfComMasID,t1.cmComMasID,
fatherIsAlive,DATE_FORMAT(cfDateOfDeath,'{$convertFormat}') AS cfDateOfDeath,cfFullName,DATE_FORMAT(cFatherDOB,'{$convertFormat}') AS cFatherDOB,cfBornCountry,cfBornArea,cfBC_No,DATE_FORMAT(cfBCDate,'{$convertFormat}') AS cfBCDate,cfOccupationId,cfNIC_No,cfDC_No,DATE_FORMAT(cfDCDate,'{$convertFormat}') AS cfDCDate,
motherIsAlive,DATE_FORMAT(cmDateOfDeath,'{$convertFormat}') AS cmDateOfDeath,cmFullName,DATE_FORMAT(cMotherDOB,'{$convertFormat}') AS cMotherDOB,cmBornCountry,cmBornArea,cmBC_No,DATE_FORMAT(cmBCDate,'{$convertFormat}') AS cmBCDate,cmOccupationId,cmNIC_No,cmDC_No,DATE_FORMAT(cmDCDate,'{$convertFormat}') AS cmDCDate
FROM srp_erp_ngo_com_communitymaster AS t1
                                        LEFT JOIN srp_erp_countrymaster AS t2 ON t2.countryID = t1.cfBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t3 ON t3.stateID = t1.cfBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t6 ON t6.specializationID = t1.cfOccupationId
                                        LEFT JOIN srp_erp_countrymaster AS t8 ON t8.countryID = t1.cmBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t9 ON t9.stateID = t1.cmBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t11 ON t11.specializationID = t1.cmOccupationId                     
                                        WHERE t1.Com_MasterID = {$Com_MasterID}")->result_array();

        $this->load->view('system/communityNgo/ajax/load_ngo_mo_memberParent', $data);
    }

    public function load_grandParentDetails_view()
    {
        $convertFormat = convert_date_format_sql();

        $Com_MasterID = $this->input->post('Com_MasterID');
        // $companyID = current_companyID();
        $data['memGrandParents'] = $this->db->query("SELECT t1.Com_MasterID,t1.cf_GdFComMasID,t1.cf_GdMComMasID,t1.cm_GdFComMasID,t1.cm_GdMComMasID,
cf_GrandFIsAlive,DATE_FORMAT(cf_GrandFDateOfDeath,'{$convertFormat}') AS cf_GrandFDateOfDeath,cf_GrandFFullName,DATE_FORMAT(cf_GrandFDOB,'{$convertFormat}') AS cf_GrandFDOB,cf_GrandFBornCountry,cf_GrandFBornArea,cf_GrandFBC_No,DATE_FORMAT(cf_GrandFBCDate,'{$convertFormat}') AS cf_GrandFBCDate,cf_GrandFOccupationId,cf_GrandFNIC_No,cf_GrandFDC_No,DATE_FORMAT(cf_GrandFDCDate,'{$convertFormat}') AS cf_GrandFDCDate,
cf_GrandMIsAlive,DATE_FORMAT(cf_GrandMDateOfDeath,'{$convertFormat}') AS cf_GrandMDateOfDeath,cf_GrandMFullName,DATE_FORMAT(cf_GrandMDOB,'{$convertFormat}') AS cf_GrandMDOB,cf_GrandMBornCountry,cf_GrandMBornArea,cf_GrandMBC_No,DATE_FORMAT(cf_GrandMBCDate,'{$convertFormat}') AS cf_GrandMBCDate,cf_GrandMOccupationId,cf_GrandMNIC_No,cf_GrandMDC_No,DATE_FORMAT(cf_GrandMDCDate,'{$convertFormat}') AS cf_GrandMDCDate,
cm_GrandFIsAlive,DATE_FORMAT(cm_GrandFDateOfDeath,'{$convertFormat}') AS cm_GrandFDateOfDeath,cm_GrandFFullName,DATE_FORMAT(cm_GrandFDOB,'{$convertFormat}') AS cm_GrandFDOB,cm_GrandFBornCountry,cm_GrandFBornArea,cm_GrandFBC_No,DATE_FORMAT(cm_GrandFBCDate,'{$convertFormat}') AS cm_GrandFBCDate,cm_GrandFOccupationId,cm_GrandFNIC_No,cm_GrandFDC_No,DATE_FORMAT(cm_GrandFDCDate,'{$convertFormat}') AS cm_GrandFDCDate,
cm_GrandMIsAlive,DATE_FORMAT(cm_GrandMDateOfDeath,'{$convertFormat}') AS cm_GrandMDateOfDeath,cm_GrandMFullName,DATE_FORMAT(cm_GrandMDOB,'{$convertFormat}') AS cm_GrandMDOB,cm_GrandMBornCountry,cm_GrandMBornArea,cm_GrandMBC_No,DATE_FORMAT(cm_GrandMBCDate,'{$convertFormat}') AS cm_GrandMBCDate,cm_GrandMOccupationId,cm_GrandMNIC_No,cm_GrandMDC_No,DATE_FORMAT(cm_GrandMDCDate,'{$convertFormat}') AS cm_GrandMDCDate
FROM srp_erp_ngo_com_communitymaster AS t1
                                        LEFT JOIN srp_erp_countrymaster AS t2 ON t2.countryID = t1.cf_GrandFBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t3 ON t3.stateID = t1.cf_GrandFBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t6 ON t6.specializationID = t1.cf_GrandFOccupationId
                                        LEFT JOIN srp_erp_countrymaster AS t8 ON t8.countryID = t1.cf_GrandMBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t9 ON t9.stateID = t1.cf_GrandMBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t11 ON t11.specializationID = t1.cf_GrandMOccupationId                     
                                        LEFT JOIN srp_erp_countrymaster AS t4 ON t4.countryID = t1.cm_GrandFBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t5 ON t5.stateID = t1.cm_GrandFBornArea                                       
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t7 ON t7.specializationID = t1.cm_GrandFOccupationId
                                        LEFT JOIN srp_erp_countrymaster AS t12 ON t12.countryID = t1.cm_GrandMBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t13 ON t13.stateID = t1.cm_GrandMBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t14 ON t14.specializationID = t1.cm_GrandMOccupationId                     
                                        WHERE t1.Com_MasterID = {$Com_MasterID}")->result_array();

        $this->load->view('system/communityNgo/ajax/load_ngo_mo_memGrandParents', $data);
    }

    public function load_grt_grandParentDetails_view()
    {
        $convertFormat = convert_date_format_sql();

        $Com_MasterID = $this->input->post('Com_MasterID');
        // $companyID = current_companyID();
        $data['mem_grtGrandParents'] = $this->db->query("SELECT t1.Com_MasterID,t1.cf_grt_GdFComMasID,t1.cf_grt_GdMComMasID,t1.cm_grt_GdFComMasID,t1.cm_grt_GdMComMasID,
cf_grt_GrandFIsAlive,DATE_FORMAT(cf_grt_GrandFDateOfDeath,'{$convertFormat}') AS cf_grt_GrandFDateOfDeath,cf_grt_GrandFFullName,DATE_FORMAT(cf_grt_GrandFDOB,'{$convertFormat}') AS cf_grt_GrandFDOB,cf_grt_GrandFBornCountry,cf_grt_GrandFBornArea,cf_grt_GrandFBC_No,DATE_FORMAT(cf_grt_GrandFBCDate,'{$convertFormat}') AS cf_grt_GrandFBCDate,cf_grt_GrandFOccupationId,cf_grt_GrandFNIC_No,cf_grt_GrandFDC_No,DATE_FORMAT(cf_grt_GrandFDCDate,'{$convertFormat}') AS cf_grt_GrandFDCDate,
cf_grt_GrandMIsAlive,DATE_FORMAT(cf_grt_GrandMDateOfDeath,'{$convertFormat}') AS cf_grt_GrandMDateOfDeath,cf_grt_GrandMFullName,DATE_FORMAT(cf_grt_GrandMDOB,'{$convertFormat}') AS cf_grt_GrandMDOB,cf_grt_GrandMBornCountry,cf_grt_GrandMBornArea,cf_grt_GrandMBC_No,DATE_FORMAT(cf_grt_GrandMBCDate,'{$convertFormat}') AS cf_grt_GrandMBCDate,cf_grt_GrandMOccupationId,cf_grt_GrandMNIC_No,cf_grt_GrandMDC_No,DATE_FORMAT(cf_grt_GrandMDCDate,'{$convertFormat}') AS cf_grt_GrandMDCDate,
cm_grt_GrandFIsAlive,DATE_FORMAT(cm_grt_GrandFDateOfDeath,'{$convertFormat}') AS cm_grt_GrandFDateOfDeath,cm_grt_GrandFFullName,DATE_FORMAT(cm_grt_GrandFDOB,'{$convertFormat}') AS cm_grt_GrandFDOB,cm_grt_GrandFBornCountry,cm_grt_GrandFBornArea,cm_grt_GrandFBC_No,DATE_FORMAT(cm_grt_GrandFBCDate,'{$convertFormat}') AS cm_grt_GrandFBCDate,cm_grt_GrandFOccupationId,cm_grt_GrandFNIC_No,cm_grt_GrandFDC_No,DATE_FORMAT(cm_grt_GrandFDCDate,'{$convertFormat}') AS cm_grt_GrandFDCDate,
cm_grt_GrandMIsAlive,DATE_FORMAT(cm_grt_GrandMDateOfDeath,'{$convertFormat}') AS cm_grt_GrandMDateOfDeath,cm_grt_GrandMFullName,DATE_FORMAT(cm_grt_GrandMDOB,'{$convertFormat}') AS cm_grt_GrandMDOB,cm_grt_GrandMBornCountry,cm_grt_GrandMBornArea,cm_grt_GrandMBC_No,DATE_FORMAT(cm_grt_GrandMBCDate,'{$convertFormat}') AS cm_grt_GrandMBCDate,cm_grt_GrandMOccupationId,cm_grt_GrandMNIC_No,cm_grt_GrandMDC_No,DATE_FORMAT(cm_grt_GrandMDCDate,'{$convertFormat}') AS cm_grt_GrandMDCDate
FROM srp_erp_ngo_com_communitymaster AS t1
                                        LEFT JOIN srp_erp_countrymaster AS t2 ON t2.countryID = t1.cf_grt_GrandFBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t3 ON t3.stateID = t1.cf_grt_GrandFBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t6 ON t6.specializationID = t1.cf_grt_GrandFOccupationId
                                        LEFT JOIN srp_erp_countrymaster AS t8 ON t8.countryID = t1.cf_grt_GrandMBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t9 ON t9.stateID = t1.cf_grt_GrandMBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t11 ON t11.specializationID = t1.cf_grt_GrandMOccupationId                     
                                        LEFT JOIN srp_erp_countrymaster AS t4 ON t4.countryID = t1.cm_grt_GrandFBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t5 ON t5.stateID = t1.cm_grt_GrandFBornArea                                       
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t7 ON t7.specializationID = t1.cm_grt_GrandFOccupationId
                                        LEFT JOIN srp_erp_countrymaster AS t12 ON t12.countryID = t1.cm_grt_GrandMBornCountry
                                        LEFT JOIN srp_erp_statemaster AS t13 ON t13.stateID = t1.cm_grt_GrandMBornArea
                                        LEFT JOIN srp_erp_ngo_com_jobspecialization AS t14 ON t14.specializationID = t1.cm_grt_GrandMOccupationId                     
                                        WHERE t1.Com_MasterID = {$Com_MasterID}")->result_array();

        $this->load->view('system/communityNgo/ajax/load_ngo_mo_memGreatGrandParents', $data);
    }
    ...
    function fetch_newMem_language()
    {
        ...
        $this->form_validation->set_rules('memNewLanguage', 'Language', 'required');
        ...
        $Com_MasterID = $this->input->post('Com_MasterID');
        $memNewLanguage = $this->input->post('memNewLanguage');

        $search_string = '';
        if (isset($memNewLanguage) && !empty($memNewLanguage)) {
            $search_string = " ((description Like '%" . $memNewLanguage . "%'))";
        }
        $chkNewLanguage = $this->db->query("SELECT * FROM srp_erp_lang_languages WHERE {$search_string}")->row();

        if (isset($chkNewLanguage)) {

            $this->db->query("UPDATE srp_erp_lang_languages SET isActive=1 WHERE {$search_string}");
            $LanguageID = $chkNewLanguage->languageID;
        } else {
            $langData['systemDescription'] = strtolower($memNewLanguage);
            $langData['description'] = $memNewLanguage;
            $langData['isActive'] = 1;
            $insertLang = $this->db->insert('srp_erp_lang_languages', $langData);
    ...
    function save_vehicleStatus()
    {
        ...
        $this->form_validation->set_rules('PropertyID', 'Property Type', 'required');

        if ($this->input->post('PropertyID') == 1) {

            $this->form_validation->set_rules('vehicleAutoID', 'Vehicle Detail', 'required');
        }
        ...
        $vehiDescription = str_replace("'", "\'", $this->input->post('vehiDescription'));
        ...
        $companyID = current_companyID();
        $isExist = $this->db->query("SELECT memVehicleID FROM srp_erp_ngo_com_membervehicles WHERE companyID={$companyID} AND Com_MasterID = '$Com_MasterID' AND vehicleAutoID = '$vehicleAutoID' AND vehiDescription='$vehiDescription' AND vehiStatus='$vehiStatus' AND propertyValue='$propertyValue'")->row('vehicleAutoID');
    ...
    {
        $this->form_validation->set_rules('helpCategoryID', 'Type Of Involvement ', 'required');
    ...
    /*start of social grant details*/
    function save_memSocialGrant()
    {
        $this->form_validation->set_rules('SocialGrantID', 'Social Grant', 'required');

        if ($this->form_validation->run() == FALSE) {
            die(json_encode(['e', validation_errors()]));
        }

        $id = $this->input->post('hidden-id-socialGrant');
        $Com_MasterID = $this->input->post('Com_MasterID');
        $SocialGrantID = $this->input->post('SocialGrantID');
        $PeriodTypeID = $this->input->post('PeriodTypeID');
        $GrantAmount = $this->input->post('GrantAmount');
        $SocialGrantRemark = $this->input->post('SocialGrantRemark');

        $data = array(
            'Com_MasterID' => $Com_MasterID,
            'SocialGrantID' => $SocialGrantID,
            'PeriodTypeID' => $PeriodTypeID,
            'GrantAmount' => $GrantAmount,
            'SocialGrantRemark' => $SocialGrantRemark,
            'companyID' => current_companyID(),
            'createdUserID' => current_userID(),
            'createdPCID' => current_pc(),
            'createdDateTime' => current_date()
        ...
        if (empty($id)) {
            ...
            $up = $this->db->where('MemSocialGrantID', $id)->update('srp_erp_ngo_com_membersocialgrants', $data_update);
    ...
    function edit_memSocialGrant()
    {
        $id = $this->input->post('id');
        $companyID = current_companyID();

        $data = $this->db->query("SELECT * FROM srp_erp_ngo_com_membersocialgrants LEFT JOIN srp_erp_ngo_com_socialgrants ON srp_erp_ngo_com_membersocialgrants.SocialGrantID = srp_erp_ngo_com_socialgrants.SocialGrantID LEFT JOIN srp_erp_ngo_com_financialperiodtypes ON srp_erp_ngo_com_financialperiodtypes.PeriodTypeID = srp_erp_ngo_com_membersocialgrants.PeriodTypeID WHERE MemSocialGrantID ={$id} AND companyID = {$companyID}")->row_array();
        echo json_encode($data);
    }

    function delete_memSocialGrant()
    {
        $id = $this->input->post('id');
        $del = $this->db->where('MemSocialGrantID', $id)->delete('srp_erp_ngo_com_membersocialgrants');
    ...
    /*end of social grant details*/

    /* start of community parent */
    function save_communityParent()
    {
        echo json_encode($this->CommunityNgo_model->save_communityParent());
    }
    /* end of community parent */

    /* start of mem grandparents */
    public function save_memFatherParents()
    {
        echo json_encode($this->CommunityNgo_model->save_memFatherParents());
    }
    public function save_memMotherParents()
    {
        echo json_encode($this->CommunityNgo_model->save_memMotherParents());
    }
    /* end of mem grandparents */

    /* start of mem grt-grandparents */
    public function save_memGrtFatherParents()
    {
        echo json_encode($this->CommunityNgo_model->save_memGrtFatherParents());
    }
    public function save_memGrtMotherParents()
    {
        echo json_encode($this->CommunityNgo_model->save_memGrtMotherParents());
    }
    /* end of mem grt-grandparents */
    ...
    function fetch_district_based_jammiyaDropdown()
    {
        ...
        $dataStGet = $this->db->query("SELECT srp_erp_statemaster.countyID,srp_erp_ngo_com_regionmaster.stateID,srp_erp_statemaster.type,srp_erp_statemaster.divisionTypeCode,srp_erp_ngo_com_regionmaster.jammiyahDivisionID FROM srp_erp_ngo_com_regionmaster INNER JOIN srp_erp_statemaster ON srp_erp_ngo_com_regionmaster.stateID=srp_erp_statemaster.stateID WHERE srp_erp_ngo_com_regionmaster.companyID = {$companyID}");
        $stateGet = $dataStGet->row();

        if (!empty($masterID)) {
            $division = $this->db->query("SELECT srp_erp_statemaster.stateID,srp_erp_statemaster.Description,srp_erp_ngo_com_regionmaster.jammiyahDivisionID FROM srp_erp_statemaster INNER JOIN srp_erp_ngo_com_regionmaster ON srp_erp_ngo_com_regionmaster.jammiyahDivisionID=srp_erp_statemaster.stateID WHERE srp_erp_statemaster.masterID = {$masterID} AND srp_erp_statemaster.type = 3 AND srp_erp_statemaster.divisionTypeCode = 'JD' AND srp_erp_ngo_com_regionmaster.companyID = {$companyID}")->result_array();
        }

        // echo '<option value="">Select a Jammiyah Division</option>';
        if (!empty($division)) {
            foreach ($division as $row) {
                if ((!empty($stateGet) && $stateGet->type == 3 && $stateGet->divisionTypeCode == 'JD') && (!empty($row['jammiyahDivisionID']))) {
                    echo '<option value="' . trim($row['stateID']) . '" selected="selected">' . trim($row['Description']) . '</option>';
                } else {
                    echo '<option value="' . trim($row['stateID']) . '">' . trim($row['Description']) . '</option>';
                }
    ...
    function fetch_division_based_division_Area_Dropdown()
    {
        $masterID = $this->input->post('masterID');
        if (!empty($masterID)) {
            $comArea = $this->db->query("SELECT stateID,Description FROM srp_erp_statemaster WHERE masterID = {$masterID} AND type = 4 AND divisionTypeCode = 'MH'")->result_array();
        }

        echo '<option value="">Select An Area</option>';
        if (!empty($comArea)) {
            foreach ($comArea as $row) {
                echo '<option value="' . trim($row['stateID']) . '">' . trim($row['Description']) . '</option>';
            }
        }
    }

    function fetch_area_based_village_Dropdown()
    {
        $masterID = $this->input->post('masterID');
        if (!empty($masterID)) {
            $comVillage = $this->db->query("SELECT srp_erp_statemaster.VillageID,srp_erp_ngo_com_villagemaster.VillageDescription FROM srp_erp_statemaster INNER JOIN srp_erp_ngo_com_villagemaster ON srp_erp_ngo_com_villagemaster.VillageID=srp_erp_statemaster.VillageID WHERE srp_erp_statemaster.stateID = {$masterID}")->result_array();
        }
        if (!empty($comVillage)) {
            foreach ($comVillage as $row) {
                echo '<option value="' . trim($row['VillageID']) . '" selected="selected">' . trim($row['VillageDescription']) . '</option>';
            }
        }
    }

    //New coutry - Area 
    function fetch_newMember_Area()
    {

        $this->form_validation->set_rules('memberNewArea', 'Area', 'required');

        if ($this->form_validation->run() == FALSE) {
            die(json_encode(['e', validation_errors()]));
        }

        $bornConutryId = $this->input->post('bornConutryId');
        $memberNewArea = $this->input->post('memberNewArea');
        ...
        if (isset($chkNewMem_area)) {
            ...
            $memAreaData['isActive'] = 1;
            $insertArea = $this->db->insert('srp_erp_ngo_com_countryareas', $memAreaData);
            if ($insertArea) {
                echo json_encode(['s', 'Inserted successfully']);
            } else {
                $common_failed = $this->lang->line('common_failed');/* 'failed'*/
            ...
            // $CountryAreaID = $this->db->insert_id();
        }
    }

    function fetch_country_based_Area_Dropdown()
    {
        ...
        $bornConutryId = $this->input->post('bornConutryId');
        $Com_MasterID = $this->input->post('Com_MasterID');
        $areaFor = $this->input->post('areaFor');
        $bldComMasID = $this->input->post('bldComMasID');

        if (!empty($bornConutryId)) {
            $division = $this->db->query("SELECT CountryAreaID,AreaDes FROM srp_erp_ngo_com_countryareas WHERE countryID = {$bornConutryId}")->result_array();
        }

        $relateArea = '';
        if (!empty($bldComMasID)) {
            $bldMember = $this->db->query("SELECT CPlaceOfBirth FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$bldComMasID} AND companyID = {$companyID}");
            $rowBldMember = $bldMember->row();
            $relateArea = $rowBldMember->CPlaceOfBirth;
        } else {
            if ($areaFor == 'member' && !empty($Com_MasterID)) {
                $Cmember = $this->db->query("SELECT CPlaceOfBirth FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowMember = $Cmember->row();
                $relateArea = $rowMember->CPlaceOfBirth;
            } else if ($areaFor == 'memFtr' && !empty($Com_MasterID)) {
                $memFather = $this->db->query("SELECT cfBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowFather = $memFather->row();
                $relateArea = $rowFather->cfBornArea;
            } else if ($areaFor == 'memMtr' && !empty($Com_MasterID)) {
                $memMother = $this->db->query("SELECT cmBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowMother = $memMother->row();
                $relateArea = $rowMother->cmBornArea;
            } else if ($areaFor == 'ftrGftr' && !empty($Com_MasterID)) {
                $memGFftr = $this->db->query("SELECT cf_GrandFBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowGFftr = $memGFftr->row();
                $relateArea = $rowGFftr->cf_GrandFBornArea;
            } else if ($areaFor == 'ftrGmtr' && !empty($Com_MasterID)) {
                $memGFmtr = $this->db->query("SELECT cf_GrandMBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowGFmtr = $memGFmtr->row();
                $relateArea = $rowGFmtr->cf_GrandMBornArea;
            } else if ($areaFor == 'mtrGftr' && !empty($Com_MasterID)) {
                $memGMftr = $this->db->query("SELECT cm_GrandFBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowGMftr = $memGMftr->row();
                $relateArea = $rowGMftr->cm_GrandFBornArea;
            } else if ($areaFor == 'mtrGmtr' && !empty($Com_MasterID)) {
                $memGMmtr = $this->db->query("SELECT cm_GrandMBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $rowGMmtr = $memGMmtr->row();
                $relateArea = $rowGMmtr->cm_GrandMBornArea;
            } else if ($areaFor == 'gftrGftr' && !empty($Com_MasterID)) {
                $gmemGFftr = $this->db->query("SELECT cf_grt_GrandFBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $growGFftr = $gmemGFftr->row();
                $relateArea = $growGFftr->cf_grt_GrandFBornArea;
            } else if ($areaFor == 'gftrGmtr' && !empty($Com_MasterID)) {
                $gmemGFmtr = $this->db->query("SELECT cf_grt_GrandMBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $growGFmtr = $gmemGFmtr->row();
                $relateArea = $growGFmtr->cf_grt_GrandMBornArea;
            } else if ($areaFor == 'gmtrGftr' && !empty($Com_MasterID)) {
                $gmemGMftr = $this->db->query("SELECT cm_grt_GrandFBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $growGMftr = $gmemGMftr->row();
                $relateArea = $growGMftr->cm_grt_GrandFBornArea;
            } else if ($areaFor == 'gmtrGmtr' && !empty($Com_MasterID)) {
                $gmemGMmtr = $this->db->query("SELECT cm_grt_GrandMBornArea FROM srp_erp_ngo_com_communitymaster WHERE Com_MasterID={$Com_MasterID} AND companyID = {$companyID}");
                $growGMmtr = $gmemGMmtr->row();
                $relateArea = $growGMmtr->cm_grt_GrandMBornArea;
            }
        }
        echo '<option value="0">Select An Area</option>';
        if (!empty($division)) {
            foreach ($division as $row) {
                if ($relateArea == trim($row['CountryAreaID'])) {
                    echo '<option value="' . trim($row['CountryAreaID']) . '" selected="selected">' . trim($row['AreaDes']) . '</option>';
                } else {
                    echo '<option value="' . trim($row['CountryAreaID']) . '">' . trim($row['AreaDes']) . '</option>';
                }
            }
        }
    }
    ...
    public function get_comMemBloodRelative()
    {
        ...
        $Com_MasterID = $_POST['famComMasterID'];

        $CI = &get_instance();

        $bldRelCom = $CI->db->query("SELECT srp_erp_ngo_com_communitymaster.Com_MasterID,CName_with_initials,DATE_FORMAT(CDOB,'{$convertFormat}') AS CDOB,CountryOfOrigin,CPlaceOfBirth,CBC_No,DATE_FORMAT(CBC_Date,'{$convertFormat}') AS CBC_Date,CNIC_No FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.Com_MasterID = '$Com_MasterID' AND comVerifiApproved='1' GROUP BY srp_erp_ngo_com_communitymaster.Com_MasterID");
        $rowRelCom = $bldRelCom->row();

        $queryJob = $this->db->query("SELECT DISTINCT MemJobID,specializationID FROM srp_erp_ngo_com_memjobs WHERE srp_erp_ngo_com_memjobs.Com_MasterID='$Com_MasterID'");
        $rowJob = $queryJob->row();
        if (!empty($rowJob)) {
            $comMemJobId = $rowJob->specializationID;
        } else {
            $comMemJobId = '';
        }
        ...
        echo json_encode(
            array(
                ...
                'name' => $rowRelCom->CName_with_initials,
                'CDOB' => $rowRelCom->CDOB,
                'CountryOfOrigin' => $rowRelCom->CountryOfOrigin,
                'CPlaceOfBirth' => $rowRelCom->CPlaceOfBirth,
                'CBC_No' => $rowRelCom->CBC_No,
                'CBC_Date' => $rowRelCom->CBC_Date,
                'comMemJobId' => $comMemJobId,
                'CNIC_No' => $rowRelCom->CNIC_No,
    ...
    function get_collection_member_details()
    {
        ...
        $data['members'] = $this->db->query("SELECT * FROM srp_erp_ngo_com_communitymaster
                                        WHERE companyID={$companyID} AND isDeleted = 0 AND srp_erp_ngo_com_communitymaster.comVerifiApproved='1' AND isActive = 1 $gender_filter")->result_array();

        $data["companyFinanceYearID"] = $detail['companyFinanceYearID'];
        $data["companyFinanceYear"] = $detail['companyFinanceYear'];
    ...
    function get_selected_members()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo '<lable style="color: red;"> Member is required !</lable>';
        } else {
    ...
    {
        $this->form_validation->set_rules("Com_MasterID2[]", 'Member', 'trim|required');
        $this->form_validation->set_rules("Amount[]", 'Amount', 'trim|required');
    ...
    function get_member_for_collection()
    {
        ...
        $MemberAraeID = $this->input->post("MemberAraeID");
        $OccTypeID = $this->input->post("OccTypeID");
        ...
        if ($CollectionType == 1) {
            ...
            $whereMemberArea = "";
            if (!empty($MemberAraeID)) {
                $whereMemberArea = "AND cm.RegionID IN(" . join(',', $MemberAraeID) . ")";
            }
        ...
        $CollectionDes = $master{
            'CollectionDes'};
        $PeriodTypeID = $master{
            'PeriodTypeID'};
        $companyFinanceYearID = $master{
            'companyFinanceYearID'};
        $companyFinanceYear = $master{
            'companyFinanceYear'};
...
WHERE  cm.isDeleted = 0 AND cm.comVerifiApproved='1' AND cm.isActive = 1 AND cm.companyID = {$companyID} AND cm.Com_MasterID NOT IN ($in_memFeePrt)
$whereMemberArea $whereGender $whereAge $whereOccupation $gender_filter
...
WHERE  cm.isDeleted = 0 AND cm.comVerifiApproved='1' AND cm.isActive = 1 AND cm.companyID = {$companyID} AND cm.Com_MasterID NOT IN ($in_memFeePrt)
 ...
 ORDER BY cm.Com_MasterID ASC ")->result_array();
    ...
    function delete_familyMemDetail()
    {
        ...
        $isFamilyHeadCheck = $this->db->query("SELECT FamDel_ID FROM srp_erp_ngo_com_familydetails WHERE FamDel_ID={$FamDel_ID} AND isLeader = '1'")->row('FamDel_ID');

        if (empty($isFamilyHeadCheck)) {
            ...
            echo json_encode(array('e', 'The head of the family can not be deleted !'));
        }
    }
    ...
    function fetch_item_detail()
    {
        ...
        $data = $this->db->query("select FamDel_ID,FamMasterID ,srp_erp_ngo_com_familydetails.Com_MasterID,srp_erp_ngo_com_familydetails.relationshipID,isMove,relationship, DATE_FORMAT(FamMemAddedDate,'{$convertFormat}') AS FamMemAddedDate,cfComMasID,cmComMasID FROM srp_erp_ngo_com_familydetails INNER JOIN srp_erp_ngo_com_communitymaster ON srp_erp_ngo_com_familydetails.Com_MasterID=srp_erp_ngo_com_communitymaster.Com_MasterID LEFT JOIN srp_erp_family_relationship ON srp_erp_family_relationship.relationshipID=srp_erp_ngo_com_familydetails.relationshipID WHERE FamDel_ID={$FamDel_ID} ")->row_array();
        echo json_encode($data);
    }
    ...
    function load_community_member_details()
    {
        $Com_MasterID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('Com_MasterID'));
        $data['extra'] = $this->CommunityNgo_model->fetch_commMember_confirmation($Com_MasterID);
        $html = $this->load->view('system/communityNgo/ngo_mo_memberDetails_print', $data, TRUE);

        if ($this->input->post('html')) {
            echo $html;
        } else {
            $this->load->library('pdf');
            $pdf = $this->pdf->printed($html, 'A4', $data['extra']['master']['isActive']);
        }
    }
    ...
    function load_familyMasterDetails()
    {
        ...
        $this->db->order_by('isLeader', 'DESC');
        $this->db->order_by('CDOB', 'ASC');
...
WHERE hEnr.FamMasterID=$FamMasterID ORDER BY hEnr.hEnrollingID")->row_array();
    ...
    function familyMaster_attachement_upload()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'Fem_' . $this->input->post('document_name') . '_' . $this->input->post('documentID') . '_' . (count($num) + 1);
            $file = $_FILES['document_file'];
            if ($file['error'] == 1) {
                die(json_encode(['status' => 0, 'type' => 'e', 'message' => 'The file you are attempting to upload is larger than the permitted size. (maximum 5MB).']));
            }
    ...
    function exportFamily_excel()
    {
        ...
        $this->excel->getActiveSheet()->getStyle('A2')->applyFromArray($styleArray);
        $this->excel->getActiveSheet()->getStyle('A4:Q4')->applyFromArray($styleArray);
        $this->excel->getActiveSheet()->fromArray($header, null, 'A4');
    ...
    function get_communityMem_status_report()
    {
        ...
        $where = "srp_erp_ngo_com_communitymaster.companyID = " . $companyID . $deleted . $isActive . $comVerifiApproved . $srch_string;
    ...
    function get_communityMem_otrReport()
    {
        ...
        $where = "srp_erp_ngo_com_communitymaster.companyID = " . $companyID . $deleted . $isActive . $comVerifiApproved . $srch_age . $srch_string;
    ...
    function get_communityMem_otrReport_pdf()
    {
        ...
        if ($genType != '-9' && ($IsAbroad == '-7' || $IsAbroad == '') && ($isConvert == '-8' || $isConvert == '') && ($stateId == '-6' || $stateId == '')) {
            ...
            $data['commReport'] = $this->db->query("SELECT MemberCode,CName_with_initials,CNIC_No,srp_erp_gender.name AS Gender,TP_Mobile,DATE_FORMAT(CDOB,'{$convertFormat}') AS CDOB,areac.Description AS Region,divisionc.stateID,divisionc.Description AS divDescription,familyr.LeaderID,familyr.FamilySystemCode,familyr.FamMasterID,familyr.FamilyName,familDel.isMove,srp_erp_ngo_com_permanent_sickness.sickAutoID,srp_erp_ngo_com_permanent_sickness.sickDescription,srp_erp_ngo_com_maritalstatus.maritalstatus FROM srp_erp_ngo_com_communitymaster LEFT JOIN srp_titlemaster ON srp_titlemaster.TitleID = srp_erp_ngo_com_communitymaster.TitleID LEFT JOIN srp_erp_gender ON srp_erp_gender.genderID = srp_erp_ngo_com_communitymaster.GenderID LEFT JOIN srp_erp_statemaster divisionc ON divisionc.stateID= srp_erp_ngo_com_communitymaster.GS_Division LEFT JOIN srp_erp_statemaster areac ON areac.stateID = srp_erp_ngo_com_communitymaster.RegionID LEFT JOIN srp_erp_ngo_com_familydetails familDel ON familDel.Com_MasterID=srp_erp_ngo_com_communitymaster.Com_MasterID LEFT JOIN srp_erp_ngo_com_familymaster familyr ON  familyr.FamMasterID =familDel.FamMasterID LEFT JOIN srp_erp_ngo_com_memberpersickness sicknessr ON sicknessr.Com_MasterID=srp_erp_ngo_com_communitymaster.Com_MasterID LEFT JOIN srp_erp_ngo_com_permanent_sickness ON srp_erp_ngo_com_permanent_sickness.sickAutoID=sicknessr.sickAutoID LEFT JOIN srp_erp_ngo_com_maritalstatus ON srp_erp_ngo_com_maritalstatus.maritalstatusID = srp_erp_ngo_com_communitymaster.CurrentStatus LEFT JOIN srp_erp_ngo_com_maritalstatus ON srp_erp_ngo_com_maritalstatus.maritalstatusID = srp_erp_ngo_com_communitymaster.CurrentStatus WHERE srp_erp_ngo_com_communitymaster.GenderID='" . $genType . "' AND srp_erp_ngo_com_communitymaster.IsAbroad='" . $IsAbroad . "' AND $where $BloodGroupID $sickAutoID ORDER BY srp_erp_ngo_com_communitymaster.Com_MasterID DESC ")->result_array();
        } else if (($IsAbroad == '-7' || $IsAbroad == '') && ($isConvert == '-8' || $isConvert == '') && $genType != '-9' && ($stateId != '-6' || $stateId != '')) {
    ...
    function get_comMem_bothHelp_report()
    {
        ...
        $where = "comHlpingMas.companyID = " . $companyID . $deleted . $isActive . $comVerifiApproved . $srch_string;
    ...
    function load_comBeneficiaryManage_view()
    {
        ...
        if (isset($status) && $status != '') {
            if ($status == 1) {
                $filter_status = " AND bfm.confirmedYN = 0";
            } elseif ($status == 2) {
                $filter_status = " AND bfm.confirmedYN = 1";
            }
        }
        $province_filter = '';
    ...
    function comBeneficiary_familyImg_upload()
    {
        ...
        $Com_MasterID = trim($this->input->post('empfamilyComID'));
        ...
        if ($size > 5) {
            ...
            echo json_encode(array('e', 'The file you are attempting to upload is larger than the permitted size. (maximum 5MB)' . $this->upload->display_errors()));
        }
        ...
        if (!$s3Upload) {
            echo json_encode(array('e', 'Error in document upload location configuration !' . $this->upload->display_errors()));
        } else {
            $data['CImage'] = $path;
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                echo json_encode(array('e', 'Member Image Upload Failed !' . $this->upload->display_errors()));
            } else {
                $this->db->trans_commit();
    ...
    function comMemBen_attachment_upload()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'MemBene_' . $this->input->post('document_name') . '_' . $this->input->post('documentID') . '_' . (count($num) + 1);
            $file = $_FILES['document_file'];
            if ($file['error'] == 1) {
                die(json_encode(['status' => 0, 'type' => 'e', 'message' => 'The file you are attempting to upload is larger than the permitted size. (maximum 5MB).']));
            }
    ...
    function getAdded_members()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo '<lable style="color: red;"> Family Member is required !</lable>';
        } else {
            ...
            // $where = "companyID = " . $companyid . "";
            $this->db->select('Com_MasterID,CName_with_initials,GenderID,cfComMasID,cmComMasID');
            $this->db->from('srp_erp_ngo_com_communitymaster');
            $this->db->where($where);
            $this->db->order_by('Com_MasterID', 'desc');
            $data['Com_MemID'] = $this->db->get()->result_array();
...
<div class="canceldiv" id="editmemService_$1"><a id="" class="CA_Alter_btn"  onclick="edit_memService($1)"><span title="Edit" rel="tooltip" class="glyphicon glyphicon-pencil" style=""></span></a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a onclick="delete_memService($1)"><span title="Delete" rel="tooltip" class="glyphicon glyphicon-trash" style="color:rgb(209, 91, 71);"></span></a></div>', 'CmtMemServiceID');
    ...
    function fetch_familyOfGSdivision()
    {
    ...
    function get_houseEnrolling_del()
    {
        ...
        $division_filter = "";
        if (!empty($GS_DivisionArr)) {
            $division_filter = 'AND cm.GS_Division IN (' . join(",", $GS_DivisionArr) . ' )';
        }

        $region_filter = "";
        if (!empty($RegionIDArr)) {
            $region_filter = 'AND cm.RegionID IN (' . join(",", $RegionIDArr) . ' )';
        }
        $FamMasterIDS = "";
        ...
        $famEconStID = "";
        if (!empty($famEconStatus)) {
            $famEconStID = "AND fm.ComEconSteID  = $famEconStatus";
        }
...
WHERE $where AND (hEnr.FamHouseSt = '0' OR hEnr.FamHouseSt IS NULL) AND (fm.isDeleted = '0' OR fm.isDeleted IS NULL) $FamMasterIDS  " . " $houseOwnshpS $houseTypeS $famHodState $famHdGenderId $famEconStID ORDER BY hEnr.hEnrollingID")->result_array();
    ...
    function get_committees_report()
    {
        ...
        $text = $this->input->post("committeeText");
        ...
        if (isset($text) && !empty($text)) {
            ...
            $srch_string = " AND ((startDate Like '%" . $text . "%') OR (endDate Like '%" . $text . "%') OR (CommitteeAreawiseDes Like '%" . $text . "%') OR(CommitteeDes Like '%" . $text . "%'))";
        }

        $where = "srp_erp_ngo_com_committeesmaster.companyID = " . $companyID . $deleted . $srch_string;
    ...
    function fetch_famLog_del()
    {
        ...
        $chkLogFamExt = $this->db->query('SELECT FamUsername FROM srp_erp_ngo_com_familymaster WHERE srp_erp_ngo_com_familymaster.companyID="' . $companyID . '" AND srp_erp_ngo_com_familymaster.FamMasterID="' . $FamMasterID . '"');
        $resLogFamExt = $chkLogFamExt->row();
        ...
        if ($resLogFamExt->FamUsername == "" || $resLogFamExt->FamUsername == NULL) {
            $this->datatables->add_column('isLoginActive', '', '');
...
&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a onclick="delete_famLogDel($1)"><span title="Delete" rel="tooltip" class="glyphicon glyphicon-trash" style="color:rgb(209, 91, 71);"></span></a>', 'FamMasterID');
    ...
    function save_com_proj_proposal_attachments()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->CommunityNgo_model->save_com_proj_proposal_attachments());
        }
    }
    ...
    function delete_com_proj_proposal_attachment()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->CommunityNgo_model->delete_com_proj_proposal_attachment());
        }
    }
    ...
    function load_proposal_family_del()
    {
        ...
        $this->db->select("srp_erp_ngo_com_familydetails.Com_MasterID,srp_erp_ngo_com_familydetails.FamMasterID,FamDel_ID,srp_erp_ngo_com_familydetails.isLeader,FamilyName,CFullName,CName_with_initials,CurrentStatus,DATE_FORMAT(FamMemAddedDate,'{$convertFormat}') AS FamMemAddedDate,name,Age,relationship,isMove,DATE_FORMAT(CDOB,'{$convertFormat}') AS CDOB,srp_erp_ngo_com_communitymaster.isActive,srp_erp_ngo_com_communitymaster.DeactivatedFor,srp_erp_ngo_com_maritalstatus.maritalstatusID,srp_erp_ngo_com_maritalstatus.maritalstatus");
        $this->db->from('srp_erp_ngo_com_familydetails');
        $this->db->join('srp_erp_ngo_com_familymaster', 'srp_erp_ngo_com_familymaster.FamMasterID=srp_erp_ngo_com_familydetails.FamMasterID', 'left');
        $this->db->join('srp_erp_ngo_com_communitymaster', 'srp_erp_ngo_com_communitymaster.Com_MasterID=srp_erp_ngo_com_familydetails.Com_MasterID', 'left');
        $this->db->join('srp_erp_gender', 'srp_erp_gender.genderID=srp_erp_ngo_com_communitymaster.GenderID', 'left');
  ...
  LEFT JOIN srp_erp_ngo_com_familymaster fm ON fm.FamMasterID=fd.FamMasterID LEFT JOIN srp_erp_ngo_com_communitymaster cm ON cm.Com_MasterID=fd.Com_MasterID WHERE fd.companyID='" . $companyID . "' AND fd.FamMasterID='" . $rowPbene->FamMasterID . "' AND cm.isActive='1' AND fd.isMove='0'");
    ...
    function get_com_uploadsInfo()
    {
        ...
        $ComUploadExpire = $this->input->post('ComUploadExpire');
        ...
        if ($isSubmited == '-3' || $isSubmited == NULL || $isSubmited == '') {
            $isSubmiteds = '';
        } else {
    ...
    function get_user_uploadsInfo()
    {
        ...
        $userUploadExpire = $this->input->post('upldUserExpire');
        ...
        if ($userUploadExpire == 0) {
            $urdate = '';
        } else if ($userUploadExpire == 1) {
            $urdate = " AND ( ComUploadExpireDate >= '" . $dateto . "')";
        } else if ($userUploadExpire == 2) {
            $urdate = " AND ( ComUploadExpireDate BETWEEN '" . $datefrom . "' AND '" . $dateto . " ')";
        }

        $companyID = $this->common_data['company_data']['company_id'];
        $convertFormat = convert_date_format_sql();

        $filter_ne = array("AND (srp_erp_ngo_com_uploads.ComUploadType=" . $ComUploadTps . ")" => $ComUploadTps);
        $set_filter_ne = array_filter($filter_ne);
        $where_uploads = join(" ", array_keys($set_filter_ne));

        $data['userUploadByDate'] = $this->db->query("SELECT srp_erp_ngo_com_uploads.ComUploadID,familyUplaod, srp_erp_ngo_com_uploads.ComUploadType,(DATE_FORMAT(UploadPublishedDate,'{$convertFormat}'))as UploadPublishedDt,ComUploadSubject,ComUploadDescription, (DATE_FORMAT(ComUploadExpireDate,'{$convertFormat}'))as ComUploadExpireDate,  ComUploadSubmited FROM srp_erp_ngo_com_uploads WHERE srp_erp_ngo_com_uploads.companyID = {$companyID} AND familyUplaod='1' AND srp_erp_ngo_com_uploads.UploadPublishedDate >='" . $date_from . "' AND srp_erp_ngo_com_uploads.UploadPublishedDate <='" . $date_To . "' $urdate $where_uploads GROUP BY srp_erp_ngo_com_uploads.UploadPublishedDate ORDER BY srp_erp_ngo_com_uploads.UploadPublishedDate DESC")->result_array();

        $data['ComUploadType'] = $ComUploadType;
        $data['userUploadExpire'] = $userUploadExpire;
    ...
    public function save_user_UpldData_masCom()
    {
        ...
        if ($ComUploadType == 1 || $ComUploadType == 2) {
            ...
            $this->form_validation->set_rules('ComUploadFileName[]', 'Document Upload', 'trim|required');
        }
    ...
    function comDonor_attachment_upload()
    {
        $this->form_validation->set_rules('attachmentDescription', 'Attachment Description', 'trim|required');
        $this->form_validation->set_rules('documentID', 'documentID', 'trim|required');
        $this->form_validation->set_rules('documentAutoID', 'Document Auto ID', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'comDonor_' . $this->input->post('document_name') . '_' . $this->input->post('documentID') . '_' . (count($num) + 1);
            $file = $_FILES['document_file'];
            if ($file['error'] == 1) {
                die(json_encode(['status' => 0, 'type' => 'e', 'message' => 'The file you are attempting to upload is larger than the permitted size. (maximum 5MB).']));
            }
...
dcd.collectionAutoId,dcd.commitmentAutoID,dcd.projectID")->result_array();
    ...
    public function searchCommunity_donor()
    {
        ...
        $querycOM1 = $CI->db->query("SELECT srp_erp_ngo_com_communitymaster.Com_MasterID,srp_erp_ngo_com_communitymaster.postalCode,srp_erp_statemaster.stateID,C_Address,srp_erp_statemaster.countyID FROM srp_erp_ngo_com_communitymaster INNER JOIN srp_erp_statemaster ON srp_erp_ngo_com_communitymaster.RegionID = srp_erp_statemaster.stateID WHERE srp_erp_ngo_com_communitymaster.companyID='{$companyID}' AND srp_erp_ngo_com_communitymaster.Com_MasterID = '$Com_MasterID' AND srp_erp_ngo_com_communitymaster.comVerifiApproved='1'");
        $rescoM1 = $querycOM1->row();
    ...
    {
        $this->form_validation->set_rules('attachmentDescription', 'Attachment Description', 'trim|required');
        $this->form_validation->set_rules('NoticeID', 'NoticeID', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $fileName = $companyCode . '_' . 'Announcement_' . $this->input->post('NoticeID') . '_' . (count($num) + 1);
            $file = $_FILES['document_file'];
            if ($file['error'] == 1) {
                die(json_encode(['status' => 0, 'type' => 'e', 'message' => 'The file you are attempting to upload is larger than the permitted size. (maximum 5MB).']));
            }
    ...
    public function memApprovalsFilter()
    {
        echo json_encode(memApprovalsPagination());
    ...
    public function memDoc_app_details()
    {
        $appMemID = trim($this->input->post('appMemID'));
        $appMemName = $this->input->post('appMemName');
        $VerificationDocID = trim($this->input->post('VerificationDocID'));
        $memDocDescription = $this->input->post('memDocDescription');
        $this->load->model('CommunityNgo_model');
        $this->CommunityNgo_model->memDoc_app_detail($appMemID, $appMemName, $VerificationDocID, $memDocDescription);
    }

    public function memApprovalBody()
    {
        $docType = trim($this->input->post('docType'));
        $FamMasterID = trim($this->input->post('FamMasterID'));
        $this->load->model('CommunityNgo_model');
        $this->CommunityNgo_model->memApprovalBody($docType, $FamMasterID);
    }

    function save_mApproval_del()
    {
        $this->form_validation->set_rules('description', 'Approval Remark', 'trim|required');

        if ($this->form_validation->run() == FALSE) {
            echo json_encode(array('e', validation_errors()));
        } else {
            echo json_encode($this->CommunityNgo_model->save_mApproval_del());
        }
    }
    ...
    function apply_memberSNFormat()
    {
        ...
        $mem_applySN = $this->db->query("SELECT Com_MasterID,SerialNo,RegionID,countyID FROM srp_erp_ngo_com_communitymaster WHERE srp_erp_ngo_com_communitymaster.companyID = {$companyID}")->result_array();
        if (!empty($mem_applySN)) {
            foreach ($mem_applySN as $mem_masGet) {

                /*   //get member mahallah
                $getAreaCode = $this->db->query("SELECT memArea.masterID,memArea.shortCode FROM srp_erp_statemaster memArea WHERE memArea.stateID={$mem_masGet['RegionID']} AND memArea.countyID = {$mem_masGet['countyID']}");
                $memAreaCode = $getAreaCode->row();
                $rowAreaCode = $memAreaCode->shortCode; */

                $MemberCode = ($defsProvince . '/' . $defsDistrict . '/' . $defsDistrictDivision . '/' . $company_code . '/' . 'MEM' . str_pad($mem_masGet['SerialNo'], 6, '0', STR_PAD_LEFT));

                $this->db->query("UPDATE srp_erp_ngo_com_communitymaster SET MemberCode='" . $MemberCode . "' WHERE srp_erp_ngo_com_communitymaster.companyID = {$companyID} AND SerialNo='" . $mem_masGet['SerialNo'] . "' AND Com_MasterID='" . $mem_masGet['Com_MasterID'] . "'");
            }
            echo json_encode(array('s', ' Member Serial Number Format Update Successfully.'));
        }
    }
    ...
    function apply_serialNoFormat()
    {
        ...
        $fam_applySN = $this->db->query("SELECT FamMasterID,FamilySerialNo,LeaderID FROM srp_erp_ngo_com_familymaster WHERE srp_erp_ngo_com_familymaster.companyID = {$companyID}")->result_array();
        if (!empty($fam_applySN)) {
            foreach ($fam_applySN as $fam_masGet) {

                //get member mahallah
                $qryMember = $this->db->query('SELECT RegionID,countyID FROM srp_erp_ngo_com_communitymaster WHERE companyID="' . $companyID . '" AND Com_MasterID="' . $fam_masGet['LeaderID'] . '"');
                ...
                $FamilyCode = 'FAM' . '/' . $defsProvince . '/' . $defsDistrict . '/' . $defsDistrictDivision . '/' . $rowAreaCode;
                $FamilySystemCode = ($defsProvince . '/' . $defsDistrict . '/' . $defsDistrictDivision . '/' . $company_code . '/' . 'FAM' . str_pad($fam_masGet['FamilySerialNo'], 6, '0', STR_PAD_LEFT));

                $this->db->query("UPDATE srp_erp_ngo_com_familymaster SET FamilyCode = '" . $FamilyCode . "',FamilySystemCode='" . $FamilySystemCode . "' WHERE srp_erp_ngo_com_familymaster.companyID = {$companyID} AND FamilySerialNo='" . $fam_masGet['FamilySerialNo'] . "' AND FamMasterID='" . $fam_masGet['FamMasterID'] . "'");
            }
            echo json_encode(array('s', ' Family Serial Number Format Update Successfully.'));
        }
    }

    function UpdateFamily_dels()
    {

        $CI = &get_instance();
        $companyID = $CI->common_data['company_data']['company_id'];

        $mis_familyDel = $this->db->query("SELECT FamMasterID,LeaderID,FamilyAddedDate,companyID,createdPCID,createdUserID,createdUserName,createdDateTime,srp_erp_ngo_com_familymaster.isDeleted FROM srp_erp_ngo_com_familymaster WHERE srp_erp_ngo_com_familymaster.companyID = {$companyID} AND srp_erp_ngo_com_familymaster.FamMasterID NOT IN (SELECT srp_erp_ngo_com_familydetails.FamMasterID FROM srp_erp_ngo_com_familydetails)")->result_array();
        if (!empty($mis_familyDel)) {
            foreach ($mis_familyDel as $row_misGet) {

                if (!empty($row_misGet['FamMasterID'])) {
                    $updateFemDel = $this->db->query("INSERT INTO srp_erp_ngo_com_familydetails (FamMasterID,Com_MasterID,isLeader,FamMemAddedDate,companyID,createdPCID,createdUserID,createdUserName,createdDateTime,isDeleted) VALUES ('" . $row_misGet['FamMasterID'] . "','" . $row_misGet['LeaderID'] . "','1','" . $row_misGet['FamilyAddedDate'] . "','" . $companyID . "','" . $row_misGet['createdPCID'] . "','" . $row_misGet['createdUserID'] . "','" . $row_misGet['createdUserName'] . "','" . $row_misGet['createdDateTime'] . "','" . $row_misGet['isDeleted'] . "')");
                }
            }
            echo json_encode(array('s', ' Family Update Successfully.'));
        } else {
            echo json_encode(array('w', ' Family already in upto date.'));
        }
    }


    //Advertisement Member Approval
    public function addApprovalsFilter()
    {
        echo json_encode(addApprovalsPagination());
        //echo json_encode( load_employee_data() );
        /*$empList = load_employee_data();
        employeeListCreate($empList);*/
        //echo '<pre>'; print_r($empList); echo '</pre>';
    ...
    public function addDoc_app_details()
    {
        $advertiseId = trim($this->input->post('advertiseId'));
        $adMemName = $this->input->post('adMemName');
        $adType_id = trim($this->input->post('adType_id'));
        $sub_category_name = $this->input->post('sub_category_name');
        $this->load->model('CommunityNgo_model');
        $this->CommunityNgo_model->addDoc_app_detail($advertiseId, $adMemName, $adType_id, $sub_category_name);
    }

    public function addApprovalBody()
    {
        $advertiseId = trim($this->input->post('advertiseId'));
        $this->load->model('CommunityNgo_model');
        $this->CommunityNgo_model->addApprovalBody($advertiseId);
    }

    function save_addApproval_del()
    {
        $this->form_validation->set_rules('adApproval_remarks', 'Approval Remark', 'trim|required');

        if ($this->form_validation->run() == FALSE) {
            echo json_encode(array('e', validation_errors()));
        } else {
            echo json_encode($this->CommunityNgo_model->save_addApproval_del());
        }
    }
    public function load_uploadAddVideo()
    {

        $uploadAdd_url = $this->input->post('uploadAdd_url');

        $shortUrlRegex = '/youtu.be\/([a-zA-Z0-9_]+)\??/i';
        $longUrlRegex = '/youtube.com\/((?:embed)|(?:watch))((?:\?v\=)|(?:\/))(\w+)/i';

        if (preg_match($longUrlRegex, $uploadAdd_url, $matches)) {
            $youtube_id = $matches[count($matches) - 1];
        }

        if (preg_match($shortUrlRegex, $uploadAdd_url, $matches)) {
            $youtube_id = $matches[count($matches) - 1];
        }
        echo 'https://www.youtube.com/embed/' . $youtube_id;
    }
