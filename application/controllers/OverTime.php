...
class OverTime extends ERP_Controller{
    ...
    function fetch_general_ot_master_table()
    {
        ...
        $this->datatables->select('generalOTMasterID,otCode,description,srp_erp_generalotmaster.currencyID,confirmedYN,approvedYN,srp_erp_currencymaster.CurrencyCode as currency,DATE_FORMAT(documentDate,"'.$convertFormat.'") AS documentDate')
        ...
        $this->datatables->add_column('edit', '$1', 'general_ot_action(generalOTMasterID,confirmedYN,approvedYN)');
    ...
    function getEmployeesDataTableShift()
    {
       ...
       // $str_lastOCGrade = '(SELECT ocGrade FROM srp_erp_sso_epfreportdetails WHERE empID = EIdNo AND companyID=' . $companyID . ' ORDER BY id DESC LIMIT 1)';
        ...
        $this->datatables->join("(SELECT EIdNo AS empID, dischargedDate, IF( isDischarged != 1, 0, 
                                  IF('{$documentDate}' <= LAST_DAY(lastWorkingDate), 0, 1)	)AS isDischargedStatus
