...
class MFQ_Template_model extends ERP_Model
{
    ...
    function load_work_process_template_detail()
    {
        ...
        $data = $this->db->query("SELECT srp_erp_mfq_templatedetail.sortOrder,srp_erp_mfq_templatedetail.description,pageNameLink,srp_erp_mfq_templatedetail.workFlowID,documentID,ws.status,srp_erp_mfq_templatedetail.templateDetailID,IFNULL(linkworkFlow,0) as linkworkFlow  FROM srp_erp_mfq_templatedetail LEFT JOIN srp_erp_mfq_workflowtemplate ON srp_erp_mfq_workflowtemplate.workFlowTemplateID = srp_erp_mfq_templatedetail.workFlowTemplateID LEFT JOIN srp_erp_mfq_systemworkflowcategory ON srp_erp_mfq_workflowtemplate.workFlowID = srp_erp_mfq_systemworkflowcategory.workFlowID LEFT JOIN (SELECT * FROM srp_erp_mfq_workflowstatus WHERE jobID = $jobID) ws ON ws.templateDetailID = srp_erp_mfq_templatedetail.templateDetailID  WHERE templateMasterID=$templateMasterID ORDER BY srp_erp_mfq_templatedetail.sortOrder")->result_array();
        //echo $this->db->last_query();
        return $data;
    }
    ...
    function load_custom_work_process_template_detail()
    {
        $templateMasterID = $this->input->post('templateMasterID');
        $jobID = $this->input->post('workProcessID');
        $data = $this->db->query("SELECT srp_erp_mfq_customtemplatedetail.sortOrder,srp_erp_mfq_customtemplatedetail.description,pageNameLink,srp_erp_mfq_customtemplatedetail.workFlowID,documentID,ws.status,srp_erp_mfq_customtemplatedetail.templateDetailID,IFNULL(linkworkFlow,0) as linkworkFlow  FROM srp_erp_mfq_customtemplatedetail LEFT JOIN srp_erp_mfq_workflowtemplate ON srp_erp_mfq_workflowtemplate.workFlowTemplateID = srp_erp_mfq_customtemplatedetail.workFlowTemplateID LEFT JOIN srp_erp_mfq_systemworkflowcategory ON srp_erp_mfq_workflowtemplate.workFlowID = srp_erp_mfq_systemworkflowcategory.workFlowID LEFT JOIN (SELECT * FROM srp_erp_mfq_workflowstatus WHERE jobID = $jobID) ws ON ws.templateDetailID = srp_erp_mfq_customtemplatedetail.templateDetailID WHERE templateMasterID=$templateMasterID AND srp_erp_mfq_customtemplatedetail.jobID = {$jobID} ORDER BY srp_erp_mfq_customtemplatedetail.sortOrder")->result_array();
        //echo $this->db->last_query();
        return $data;
    }
    ...
    function fetch_crew()
    {
        ...
        $data = $this->db->query('SELECT crewID,EIdNo,ECode,Ename1,EpTelephone,EEmail,DesDescription,name as gender,IFNULL(Ename1,"") AS "Match" FROM srp_erp_mfq_crews LEFT JOIN srp_designation ON DesignationID = EmpDesignationId LEFT JOIN srp_erp_gender ON genderID = Gender  WHERE (ECode LIKE "' . $search_string . '" OR Ename1 LIKE "' . $search_string . '")')->result_array();
        if (!empty($data)) {
            foreach ($data as $val) {
                $dataArr[] = array('value' => $val["Match"], 'data' => $val['ECode'], 'crewID' => $val['crewID'], 'EpTelephone' => $val['EpTelephone'], 'EEmail' => $val['EEmail'], 'DesDescription' => $val['DesDescription'], 'gender' => $val['gender']);
            }
        }
