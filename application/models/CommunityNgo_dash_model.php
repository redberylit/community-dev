...
class CommunityNgo_dash_model extends ERP_Model
{
    ...
    function communityDashSum_Count()
    {
        ...
        $where = "srp_erp_ngo_com_house_enrolling.companyID = " . $companyID . $deleted.$verifyDocApp;

                $member_count = $this->db->query("SELECT COUNT(*) as membersCount FROM srp_erp_ngo_com_communitymaster where companyID = '{$companyID}' AND (isDeleted IS NULL OR isDeleted = '' OR isDeleted = 0) AND isActive='1' AND comVerifiApproved $areaMemIdS  " . " $gsDivitnIdS")->row_array();
    ...
    public function get_family_details($FamMasterID){
        ...
        $where = "srp_erp_ngo_com_familymaster.companyID = " . $companyID . $deleted.$verifyDocApp;
        ...
        $familyDsh = $queryFam->result();
         if (($FamMasterID !=NULL || !empty($FamMasterID)) && !empty($familyDsh)) {
                ...
                foreach ($familyDsh as $valMas) {
                    ...
                    echo '<tr>
               <td>' . $f . '</td>
               <td><a onclick="get_familyMembers(' . $valMas->FamMasterID . ');">' . $valMas->FamilySystemCode . '</a></td>
               ...
               <td style="padding: 2px;text-align:center;"><a onclick="get_familyMembers(' . $valMas->FamMasterID . ');">' . $totFamMms . '</a></td>
...
//var_dump($date_from);
        ...
        foreach ($res as $row) {
            ...
            echo '<tr>
               <td>' . $a . '</td>
               <td><a onclick="documentPageView_modal(\'' . $row->documentID . '\',' . $row->receiptVoucherAutoId . ')">' . $row->RVcode . '</a></td>
