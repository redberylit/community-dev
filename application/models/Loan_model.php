...
class Loan_model extends ERP_Model{
    ...
    function fetchLoanCategory(){
        $query = $this->db->select('loanID, description, isInterestBased, interestPercentage', false)
                         ->from('srp_erp_pay_loan_category')
    ...
    function load_emp_loanDet($loanID){
        ...
        $con = "IFNULL(Ename2, '')";
        $query = $this->db->select('EIdNo, CONCAT('.$con.') AS "Employee", ECode, loanCode, amount, payCurrency, payCurrencyID,
                                    DecimalPlaces,DATE_FORMAT(loanDate,\''.$convertFormat.'\') AS loanDate, ID, loanCatID, loanDescription, interestPer, numberOfInstallment,
                                  DATE_FORMAT(deductionStartingDate,\''.$convertFormat.'\') AS  deductionStartingDate, DesDescription, loan_cat.description, confirmedYN, confirmedByName,
                                    approvedYN, approvedbyEmpName')
    ...
    function loan_conformation($loanID){
        ...
        if( $this->db->affected_rows() > 0 ){*/
            ...
            $approvals_status = $this->approvals->CreateApproval('LO',$loanID,$lo_data['loanCode'],'Loan Approval', 'srp_erp_pay_emploan', 'ID',0,$lo_data['loanDate']);
        ...
        /*}
            ...
            return array('e', 'Error in Loan Conformation ['.$lo_data['loanCode'].']');
        }*/
        ...
        if($this->form_validation->run()==FALSE){
            ...
            if( $errorInstallmentNo == '' ){
                ...
                foreach($skipDate as $scheduleDate) {
                    ...
                    $this->db->query("INSERT INTO srp_erp_pay_emploan_schedule ( empID, loanID, loanCode, amountPerInstallment, scheduleDate, installmentNo, interestPer,
                                      ...
                                      SELECT empID, loanID, loanCode, amountPerInstallment, '".$scheduleDate."', installmentNo, interestPer, isSetteled, 0, transactionCurrency,
