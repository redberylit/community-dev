...
class Receipt_voucher_model extends ERP_Model
{
    ...
    function fetch_rv_advance_detail()
    {
        ...
        $this->db->select('srp_erp_customerreceiptdetail.transactionAmount ,DATE_FORMAT(srp_erp_customerreceiptmaster.RVdate,\'' . $convertFormat . '\') AS RVdate , srp_erp_customerreceiptmaster.RVcode,sum(srp_erp_rvadvancematchdetails.transactionAmount) as paid,srp_erp_customerreceiptdetail.receiptVoucherDetailAutoID,srp_erp_customerreceiptmaster.transactionCurrencyDecimalPlaces as decimalplaces');
	...
	GROUP BY invoiceAutoID) genexcharg', 'srp_erp_customerinvoicemaster.invoiceAutoID = genexcharg.invoiceAutoID', 'left');
        ...
        for ($i = 0; $i < count($master_recode); $i++) {
            ...
            $data[$i]['due_amount'] = ($master_recode[$i]['transactionAmount'] - ($master_recode[$i]['receiptTotalAmount'] + $master_recode[$i]['advanceMatchedTotal'] + $master_recode[$i]['creditNoteTotalAmount']));
...
IF
	( LENGTH( srp_erp_itemmaster.partNo ), `srp_erp_itemmaster`.`partNo`, NULL )
       ...
       /* $this->db->select('*');
        ...
        $data['creditnote'] =  $this->db->query("SELECT
	* 
FROM
...
IF
    ...
    function save_direct_rv_detail()
    {
        ...
        foreach ($gl_auto_ids as $key => $gl_auto_id) {
            ...
            $data[$key]['transactionAmount'] = trim($amount[$key]-$data[$key]['discountAmount']);
...
GROUP BY receiptVoucherAutoId")->row_array();
            ...
            if ($totalamnt < 0) {
                ...
                if($totalamnt > $payAmount['payAmount']){
...
FROM
	...
	LEFT JOIN ( SELECT SUM( totalValue ) AS transactionAmount, salesReturnAutoID FROM srp_erp_salesreturndetails WHERE companyID = '$companyID' GROUP BY salesReturnAutoID ) detailTbl ON detailTbl.salesReturnAutoID = masterTbl.salesReturnAutoID
...
FROM
 ...
 CASE srp_erp_customerreceiptpayments.paymentType WHEN 1 THEN 'Cash' WHEN 2 THEN 'Cheque' WHEN 3 THEN 'Bank Transfer' 
    ...
    function fetch_receiptVocher_payment_detail()
    {
        ...
        $receiptPaymentID = $this->input->post('receiptPaymentID');
        $receiptVoucherAutoId = $this->input->post('receiptVoucherAutoId');
        $data = $this->db->query("SELECT receiptPaymentID, srp_erp_customerreceiptpayments.paymentType, bankGLAutoID, chequeNo, DATE_FORMAT(chequeDate,' . $convertFormat . ') AS chequeDate, memo, amount, ((totalamounttransaction + ((taxPercentage / 100)*totalamounttransaction) + amount) - (paidAmount)) as payablemount FROM srp_erp_customerreceiptpayments 
                     left join(
