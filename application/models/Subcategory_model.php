...
class Subcategory_model extends ERP_Model
{
    ...
    function save_sub_category()
    {
        //if (!$this->input->post('subcatregoryedit')) {
            ...
            $itemCategory = $this->db->query("SELECT itemCategoryID,description,companyID FROM srp_erp_itemcategory WHERE companyID = " . $this->common_data['company_data']['company_id'] . " AND description = '" . $this->input->post('subcategory') . "' AND masterID='" .$this->input->post('master'). "'")->row_array();
