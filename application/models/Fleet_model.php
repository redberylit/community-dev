...
class Fleet_model extends ERP_Model
{
    ...
    function Save_New_Vehicle()
    {
        ...
        if ($vehicleMasterID) {
            ...
            if (isset($isassetexist)) {
                ...
                $data['vehicleCode'] = ($company_code . '/' . 'VEH' . str_pad($data['SerialNo'], 6,
                        '0', STR_PAD_LEFT));
    ...
    function Save_New_Driver()
    {
        ...
        if ($driverMasID) {
            ...
            $data['driverCode'] = ($company_code . '/' . 'DRV' . str_pad($data['SerialNo'], 6,
                    '0', STR_PAD_LEFT));
        ...
        if ($fuelusageID) {
            ...
            if ($update) {
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    $this->db->trans_commit();
                    return array('s', 'Fuel Usage Updated successfully.', $last_id);
                }
            }
        } else {
            ...
            $data['documentCode'] = ($company_code . '/' . 'FU' . str_pad($data['SerialNo'], 6,
                    '0', STR_PAD_LEFT));
...
WHERE
	...
	AND meterreadingmaster.companyID = $companyID")->row_array();
        ...
        if ($vehiclemaintenaceid) {
            ...
            $data['serialNo'] = $serial['serialNo'];
            $data['maintenanceCode'] = ($company_code . '/' . 'MNT' . str_pad($data['serialNo'], 6,
                    '0', STR_PAD_LEFT));
            $data['createdUserGroup'] = $this->common_data['user_group'];
            $data['createdpc'] = $this->common_data['current_pc'];
            $data['createDateTime'] = $this->common_data['current_date'];
